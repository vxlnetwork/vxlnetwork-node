...
if [ -n "$DOCKER_PASSWORD" ]; then
    ...
    if [[ "$TRAVIS_BRANCH" == "master" || "$TRAVIS_BRANCH" == "docker_cache" ]] && [[ "${TRAVIS_BUILD_STAGE_NAME}" =~ 'Build' ]]; then
        ci_image_name="vxlnetworkcurrency/vxlnetwork-env:$TRAVIS_JOB_NAME"
    ...
    if [[ "$TRAVIS_BUILD_STAGE_NAME" == "Master_beta_docker" ]]; then
        ...
        "$scripts"/custom-timeout.sh 30 docker build --build-arg NETWORK=beta --build-arg CI_BUILD=true --build-arg TRAVIS_TAG="$TRAVIS_TAG" -f docker/node/Dockerfile -t vxlnetworkcurrency/vxlnetwork-beta:master --cache-from vxlnetworkcurrency/vxlnetwork-beta:master .
        "$scripts"/custom-timeout.sh 30 docker push vxlnetworkcurrency/vxlnetwork-beta:master
        ...
        if [[ "$TRAVIS_JOB_NAME" =~ "live" ]]; then
            ...
            cached="--cache-from=vxlnetworkcurrency/vxlnetwork-beta:master"
            docker pull vxlnetworkcurrency/vxlnetwork-beta:master
        fi
