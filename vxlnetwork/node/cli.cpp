...
namespace
{
void reset_confirmation_heights (vxlnetwork::write_transaction const & transaction, vxlnetwork::ledger_constants & constants, vxlnetwork::store & store);
...
std::string vxlnetwork::error_cli_messages::message (int ev) const
{
	switch (static_cast<vxlnetwork::error_cli> (ev))
	{
		case vxlnetwork::error_cli::generic:
			return "Unknown error";
		case vxlnetwork::error_cli::parse_error:
			return "Coud not parse command line";
		case vxlnetwork::error_cli::invalid_arguments:
			return "Invalid arguments";
		case vxlnetwork::error_cli::unknown_command:
			return "Unknown command";
		case vxlnetwork::error_cli::database_write_error:
			return "Database write error";
		case vxlnetwork::error_cli::reading_config:
			return "Config file read error";
		case vxlnetwork::error_cli::disable_all_network:
			return "Flags --disable_tcp_realtime and --disable_udp cannot be used together";
		case vxlnetwork::error_cli::ambiguous_udp_options:
			return "Flags --disable_udp and --enable_udp cannot be used together";
		case vxlnetwork::error_cli::ambiguous_pruning_voting_options:
...
void vxlnetwork::add_node_options (boost::program_options::options_description & description_a)
{
	// clang-format off
...
void vxlnetwork::add_node_flag_options (boost::program_options::options_description & description_a)
{
	// clang-format off
	description_a.add_options()
...
std::error_code vxlnetwork::update_flags (vxlnetwork::node_flags & flags_a, boost::program_options::variables_map const & vm)
{
	std::error_code ec;
	...
	if (config != vm.end ())
	{
		flags_a.config_overrides = vxlnetwork::config_overrides (config->second.as<std::vector<vxlnetwork::config_key_value_pair>> ());
	}
	auto rpcconfig (vm.find ("rpcconfig"));
	if (rpcconfig != vm.end ())
	{
		flags_a.rpc_config_overrides = vxlnetwork::config_overrides (rpcconfig->second.as<std::vector<vxlnetwork::config_key_value_pair>> ());
	}
	return ec;
}

std::error_code vxlnetwork::flags_config_conflicts (vxlnetwork::node_flags const & flags_a, vxlnetwork::node_config const & config_a)
{
	std::error_code ec;
	if (flags_a.enable_pruning && config_a.enable_voting)
	{
		ec = vxlnetwork::error_cli::ambiguous_pruning_voting_options;
	}
	return ec;
}
...
void database_write_lock_error (std::error_code & ec)
{
	...
	ec = vxlnetwork::error_cli::database_write_error;
}
...
std::error_code vxlnetwork::handle_node_options (boost::program_options::variables_map const & vm)
{
	std::error_code ec;
	boost::filesystem::path data_path = vm.count ("data_path") ? boost::filesystem::path (vm["data_path"].as<std::string> ()) : vxlnetwork::working_path ();
			...
			if (!wallet_id.decode_hex (vm["wallet"].as<std::string> ()))
			{
				...
				if (wallet != nullptr)
				{
					...
					else
					{
						...
						ec = vxlnetwork::error_cli::invalid_arguments;
					}
				}
				else
				{
		...
		try
		{
			...
			if (!ec)
			{
				...
				if (success)
				{
					...
					if (using_rocksdb)
					{
						vxlnetwork::remove_all_files_in_dir (backup_path);
						vxlnetwork::move_all_files_to_dir (source_path, backup_path);
						vxlnetwork::move_all_files_to_dir (vacuum_path, source_path);
	...
	{
		auto data_path = vm.count ("data_path") ? boost::filesystem::path (vm["data_path"].as<std::string> ()) : vxlnetwork::working_path ();
		auto node_flags = vxlnetwork::inactive_node_flag_defaults ();
			...
			if (account_it != vm.cend ())
			{
				...
				if (!account.decode_account (account_str))
				{
					...
					else
					{
						...
						ec = vxlnetwork::error_cli::generic;
					}
				}
				else
				{
	...
	else if (vm.count ("generate_config"))
	{
		...
		if (type == "node")
		{
			...
			vxlnetwork::daemon_config config{ data_path, network_params };
			config.serialize_toml (toml);
		}
		else if (type == "rpc")
		{
			valid_type = true;
			vxlnetwork::rpc_config config{ vxlnetwork::dev::network_params.network };
			config.serialize_toml (toml);
		}
		else if (type == "tls")
		{
			valid_type = true;
			vxlnetwork::tls_config config;
			config.serialize_toml (toml);
		}
		else
		{
		...
		if (valid_type)
		{
			std::cout << "# This is an example configuration file for Vxlnetwork. Visit https://docs.vxlnetwork.org/running-a-node/configuration/ for more information.\n#\n"
					  ...
					  << "# Additional information for notable configuration options is available in https://docs.vxlnetwork.org/running-a-node/configuration/#notable-configuration-options\n";
	...
	else if (vm.count ("diagnostics"))
	{
		...
		key.clear ();
		vxlnetwork::send_block send (0, 0, 0, key, 0, 0);
		std::cout << "Testing key derivation function" << std::endl;
		vxlnetwork::raw_key junk1;
		junk1.clear ();
		vxlnetwork::uint256_union junk2 (0);
		vxlnetwork::kdf kdf{ inactive_node->node->config.network_params.kdf_work };
		kdf.phs (junk1, "", junk2);
		std::cout << "Testing time retrieval latency... " << std::flush;
		vxlnetwork::timer<std::chrono::nanoseconds> timer (vxlnetwork::timer_state::started);
	...
	else if (vm.count ("wallet_create"))
	{
		...
		{
			auto inactive_node = vxlnetwork::default_inactive_node (data_path, vm);
			auto wallet_key = vxlnetwork::random_wallet_id ();
				...
				if (existing != inactive_node->node->wallets.items.end ())
				{
					...
					if (!existing->second->enter_password (transaction, password))
					{
						...
						for (auto i (existing->second->store.begin (transaction)), m (existing->second->store.end ()); i != m; ++i)
						{
							...
							if (vxlnetwork::pub_key (key) != account)
							{
						...
						else
						{
							...
							else
							{
								bool error (true);
								{
									...
									auto transaction (node->wallets.tx_begin_write ());
									vxlnetwork::wallet wallet (error, transaction, node->wallets, wallet_id.to_string (), contents.str ());
								}
								if (error)
								{
									std::cerr << "Unable to import wallet\n";
	...
	else if (vm.count ("wallet_list"))
	{
		...
		for (auto i (node->wallets.items.begin ()), n (node->wallets.items.end ()); i != n; ++i)
		{
			...
			{
				std::cout << vxlnetwork::account (j->first).to_account () << '\n';
			}
		}
	}
	...
	{
		ec = vxlnetwork::error_cli::unknown_command;
	}

	return ec;
}

std::unique_ptr<vxlnetwork::inactive_node> vxlnetwork::default_inactive_node (boost::filesystem::path const & path_a, boost::program_options::variables_map const & vm_a)
{
	auto node_flags = vxlnetwork::inactive_node_flag_defaults ();
	vxlnetwork::update_flags (node_flags, vm_a);
	return std::make_unique<vxlnetwork::inactive_node> (path_a, node_flags);
}
...
bool is_using_rocksdb (boost::filesystem::path const & data_path, boost::program_options::variables_map const & vm, std::error_code & ec)
{
	...
	{
		config_overrides = vxlnetwork::config_overrides (config_arg->second.as<std::vector<vxlnetwork::config_key_value_pair>> ());
	}

	// config override...
	auto error = vxlnetwork::read_node_config_toml (data_path, config, config_overrides);
	if (!error)
	{
	...
	{
		ec = vxlnetwork::error_cli::reading_config;
	}
