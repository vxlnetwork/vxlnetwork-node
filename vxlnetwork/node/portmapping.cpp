...
std::string vxlnetwork::mapping_protocol::to_string ()
{
	std::stringstream ss;
...
std::string vxlnetwork::port_mapping::get_config_port (std::string const & node_port_a)
{
	return node.config.external_port != 0 ? std::to_string (node.config.external_port) : node_port_a;
}

std::string vxlnetwork::port_mapping::to_string ()
{
	std::stringstream ss;
...
void vxlnetwork::port_mapping::refresh_devices ()
{
...
vxlnetwork::endpoint vxlnetwork::port_mapping::external_address ()
{
	vxlnetwork::endpoint result_l (boost::asio::ip::address_v6{}, 0);
	vxlnetwork::lock_guard<vxlnetwork::mutex> guard_l (mutex);
	for (auto & protocol : protocols | boost::adaptors::filtered ([] (auto const & p) { return p.enabled; }))
	{
		if (protocol.external_port != 0)
		{
			result_l = vxlnetwork::endpoint (protocol.external_address, protocol.external_port);
		}
	}
	return result_l;
}

void vxlnetwork::port_mapping::refresh_mapping ()
{
	...
	if (on)
	{
		...
		for (auto & protocol : protocols | boost::adaptors::filtered ([] (auto const & p) { return p.enabled; }))
		{
			auto upnp_description = std::string ("Vxlnetwork Node (") + node.network_params.network.get_current_network_as_string () + ")";
...
bool vxlnetwork::port_mapping::check_lost_or_old_mapping ()
{
...
void vxlnetwork::port_mapping::check_mapping_loop ()
{
...
void vxlnetwork::port_mapping::stop ()
{
	on = false;
	vxlnetwork::lock_guard<vxlnetwork::mutex> guard_l (mutex);
	for (auto & protocol : protocols | boost::adaptors::filtered ([] (auto const & p) { return p.enabled; }))
...
std::string vxlnetwork::upnp_state::to_string ()
{
	std::stringstream ss;
...
vxlnetwork::upnp_state::~upnp_state ()
{
	if (devices)
	{
...
vxlnetwork::upnp_state & vxlnetwork::upnp_state::operator= (vxlnetwork::upnp_state && other_a)
{
	if (this == &other_a)
	{
		return *this;
