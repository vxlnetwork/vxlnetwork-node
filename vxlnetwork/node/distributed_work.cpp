...
#include <vxlnetwork/node/distributed_work.hpp>
#include <vxlnetwork/node/node.hpp>
#include <vxlnetwork/node/websocket.hpp>

#include <boost/algorithm/string/erase.hpp>

std::shared_ptr<request_type> vxlnetwork::distributed_work::peer_request::get_prepared_json_request (std::string const & request_string_a) const
{
...
vxlnetwork::distributed_work::distributed_work (vxlnetwork::node & node_a, vxlnetwork::work_request const & request_a, std::chrono::seconds const & backoff_a) :
	node (node_a),
	...
	elapsed (vxlnetwork::timer_state::started, "distributed work generation timer")
{
	debug_assert (!finished);
	debug_assert (status == work_generation_status::ongoing);
}

vxlnetwork::distributed_work::~distributed_work ()
{
	debug_assert (status != work_generation_status::ongoing);
	if (auto node_l = node_w.lock ())
	{
		if (!node_l->stopped && node_l->websocket_server && node_l->websocket_server->any_subscriber (vxlnetwork::websocket::topic::work))
		{
...
void vxlnetwork::distributed_work::start ()
{
	...
	for (auto const & peer : need_resolve)
	{
		...
		if (!ec)
		{
			do_request (vxlnetwork::tcp_endpoint (parsed_address, peer.second));
		}
		else
		{
			...
			node.network.resolver.async_resolve (boost::asio::ip::udp::resolver::query (peer.first, std::to_string (peer.second)), [peer, this_l, &extra = resolved_extra] (boost::system::error_code const & ec, boost::asio::ip::udp::resolver::iterator i_a) {
				if (!ec)
				{
					this_l->do_request (vxlnetwork::tcp_endpoint (i_a->endpoint ().address (), i_a->endpoint ().port ()));
					++i_a;
					for (auto & i : boost::make_iterator_range (i_a, {}))
					{
						++extra;
						this_l->do_request (vxlnetwork::tcp_endpoint (i.endpoint ().address (), i.endpoint ().port ()));
					}
				}
				else
				{
...
void vxlnetwork::distributed_work::start_local ()
{
	auto this_l (shared_from_this ());
...
void vxlnetwork::distributed_work::do_request (vxlnetwork::tcp_endpoint const & endpoint_a)
{
	auto this_l (shared_from_this ());
	...
	[this_l, connection] (boost::system::error_code const & ec) {
		if (!ec && !this_l->stopped)
		{
			std::string request_string;
			{
				...
				rpc_request.put ("difficulty", vxlnetwork::to_string_hex (this_l->request.difficulty));
...
void vxlnetwork::distributed_work::do_cancel (vxlnetwork::tcp_endpoint const & endpoint_a)
{
	auto this_l (shared_from_this ());
...
void vxlnetwork::distributed_work::success (std::string const & body_a, vxlnetwork::tcp_endpoint const & endpoint_a)
{
	bool error = true;
	try
	{
		...
		uint64_t work;
		if (!vxlnetwork::from_string_hex (work_text, work))
		{
			if (vxlnetwork::dev::network_params.work.difficulty (request.version, request.root, work) >= request.difficulty)
			{
				error = false;
			...
			{
				node.logger.try_log (boost::str (boost::format ("Incorrect work response from %1%:%2% for root %3% with diffuculty %4%: %5%") % endpoint_a.address () % endpoint_a.port () % request.root.to_string () % vxlnetwork::to_string_hex (request.difficulty) % work_text));
			}
		}
		else
		{
...
void vxlnetwork::distributed_work::stop_once (bool const local_stop_a)
{
...
void vxlnetwork::distributed_work::set_once (uint64_t const work_a, std::string const & source_a)
{
	if (!finished.exchange (true))
	{
		...
		if (node.config.logging.work_generation_time ())
		{
			...
			auto multiplier_text_l (vxlnetwork::to_string (vxlnetwork::difficulty::to_multiplier (request.difficulty, node.default_difficulty (request.version)), 2));
			node.logger.try_log (boost::str (unformatted_l % request.root.to_string () % vxlnetwork::to_string_hex (request.difficulty) % multiplier_text_l % elapsed.value ().count ()));
		}
	}
}

void vxlnetwork::distributed_work::cancel ()
{
	if (!finished.exchange (true))
	{
		elapsed.stop ();
...
void vxlnetwork::distributed_work::failure ()
{
...
void vxlnetwork::distributed_work::handle_failure ()
{
	if (!finished)
	{
		...
		if (!local_generation_started && !finished.exchange (true))
		{
			...
			std::weak_ptr<vxlnetwork::node> node_weak (node.shared ());
...
void vxlnetwork::distributed_work::add_bad_peer (vxlnetwork::tcp_endpoint const & endpoint_a)
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> guard (mutex);
