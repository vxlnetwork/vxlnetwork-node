...
class peer_exclusion final
{
	...
	public:
		...
		decltype (std::declval<vxlnetwork::tcp_endpoint> ().address ()) address;
		uint64_t score;
	};
...
public:
	...
	uint64_t add (vxlnetwork::tcp_endpoint const &, std::size_t const);
	bool check (vxlnetwork::tcp_endpoint const &);
	void remove (vxlnetwork::tcp_endpoint const &);
