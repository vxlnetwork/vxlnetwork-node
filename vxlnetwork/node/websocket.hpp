...
#include <vxlnetwork/node/websocket_stream.hpp>
...
namespace websocket
{
	...
	{
	public:
		message (vxlnetwork::websocket::topic topic_a) :
			topic (topic_a)
		{
		}
		message (vxlnetwork::websocket::topic topic_a, boost::property_tree::ptree & tree_a) :
			topic (topic_a), contents (tree_a)
		{
		}

		std::string to_string () const;
		vxlnetwork::websocket::topic topic;
	...
	{
	public:
		message block_confirmed (std::shared_ptr<vxlnetwork::block> const & block_a, vxlnetwork::account const & account_a, vxlnetwork::amount const & amount_a, std::string subtype, bool include_block, vxlnetwork::election_status const & election_status_a, std::vector<vxlnetwork::vote_with_weight_info> const & election_votes_a, vxlnetwork::websocket::confirmation_options const & options_a);
		message stopped_election (vxlnetwork::block_hash const & hash_a);
		message vote_received (std::shared_ptr<vxlnetwork::vote> const & vote_a, vxlnetwork::vote_code code_a);
		message work_generation (vxlnetwork::work_version const version_a, vxlnetwork::block_hash const & root_a, uint64_t const work_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::string const & peer_a, std::vector<std::string> const & bad_peers_a, bool const completed_a = true, bool const cancelled_a = false);
		message work_cancelled (vxlnetwork::work_version const version_a, vxlnetwork::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a);
		message work_failed (vxlnetwork::work_version const version_a, vxlnetwork::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a);
		...
		message telemetry_received (vxlnetwork::telemetry_data const &, vxlnetwork::endpoint const &);
		message new_block_arrived (vxlnetwork::block const & block_a);
	...
	{
	public:
		confirmation_options (vxlnetwork::wallets & wallets_a);
		confirmation_options (boost::property_tree::ptree const & options_a, vxlnetwork::wallets & wallets_a, vxlnetwork::logger_mt & logger_a);
	...
	private:
		...
		boost::optional<vxlnetwork::logger_mt &> logger;
	...
	{
	public:
		vote_options (boost::property_tree::ptree const & options_a, vxlnetwork::logger_mt & logger_a);
	...
	public:
		...
		explicit session (vxlnetwork::websocket::listener & listener_a, socket_type socket_a, boost::asio::ssl::context & ctx_a);
#endif
		/** Constructor that takes ownership over \p socket_a */
		explicit session (vxlnetwork::websocket::listener & listener_a, socket_type socket_a);
		...
		void write (vxlnetwork::websocket::message message_a);

	private:
		/** The owning listener */
		vxlnetwork::websocket::listener & ws_listener;
		/** Websocket stream, supporting both plain and tls connections */
		vxlnetwork::websocket::stream ws;
		...
		vxlnetwork::mutex subscriptions_mutex;
	...
	{
	public:
		listener (std::shared_ptr<vxlnetwork::tls_config> const & tls_config_a, vxlnetwork::logger_mt & logger_a, vxlnetwork::wallets & wallets_a, boost::asio::io_context & io_ctx_a, boost::asio::ip::tcp::endpoint endpoint_a);
		...
		void broadcast_confirmation (std::shared_ptr<vxlnetwork::block> const & block_a, vxlnetwork::account const & account_a, vxlnetwork::amount const & amount_a, std::string const & subtype, vxlnetwork::election_status const & election_status_a, std::vector<vxlnetwork::vote_with_weight_info> const & election_votes_a);

		/** Broadcast \p message to all session subscribing to the message topic. */
		void broadcast (vxlnetwork::websocket::message message_a);

		vxlnetwork::logger_mt & get_logger () const
		{
			return logger;
		}
		...
		vxlnetwork::wallets & get_wallets () const
		{
			return wallets;
		}
		 ...
		 */
		bool any_subscriber (vxlnetwork::websocket::topic const & topic_a) const
		{
			return subscriber_count (topic_a) > 0;
		}
		/** Getter for subscriber count of a specific topic*/
		std::size_t subscriber_count (vxlnetwork::websocket::topic const & topic_a) const
		{
	...
	private:
		...
		friend vxlnetwork::websocket::session;

		/** Adds to subscription count of a specific topic*/
		void increase_subscriber_count (vxlnetwork::websocket::topic const & topic_a);
		/** Removes from subscription count of a specific topic*/
		void decrease_subscriber_count (vxlnetwork::websocket::topic const & topic_a);
		...
		socket_type socket;
		vxlnetwork::mutex sessions_mutex;
