...
vxlnetwork::bootstrap_attempt_legacy::bootstrap_attempt_legacy (std::shared_ptr<vxlnetwork::node> const & node_a, uint64_t const incremental_id_a, std::string const & id_a, uint32_t const frontiers_age_a, vxlnetwork::account const & start_account_a) :
	vxlnetwork::bootstrap_attempt (node_a, vxlnetwork::bootstrap_mode::legacy, incremental_id_a, id_a),
...
bool vxlnetwork::bootstrap_attempt_legacy::consume_future (std::future<bool> & future_a)
{
	bool result;
	try
	{
...
void vxlnetwork::bootstrap_attempt_legacy::stop ()
{
...
void vxlnetwork::bootstrap_attempt_legacy::request_push (vxlnetwork::unique_lock<vxlnetwork::mutex> & lock_a)
{
	bool error (false);
	lock_a.unlock ();
	...
	if (connection_l)
	{
		std::future<bool> future;
		{
			...
			auto client (std::make_shared<vxlnetwork::bulk_push_client> (connection_l, this_l));
			client->start ();
			push = client;
...
void vxlnetwork::bootstrap_attempt_legacy::add_frontier (vxlnetwork::pull_info const & pull_a)
{
...
void vxlnetwork::bootstrap_attempt_legacy::add_bulk_push_target (vxlnetwork::block_hash const & head, vxlnetwork::block_hash const & end)
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> lock (mutex);
	bulk_push_targets.emplace_back (head, end);
}

bool vxlnetwork::bootstrap_attempt_legacy::request_bulk_push_target (std::pair<vxlnetwork::block_hash, vxlnetwork::block_hash> & current_target_a)
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> lock (mutex);
...
void vxlnetwork::bootstrap_attempt_legacy::set_start_account (vxlnetwork::account const & start_account_a)
{
...
bool vxlnetwork::bootstrap_attempt_legacy::request_frontier (vxlnetwork::unique_lock<vxlnetwork::mutex> & lock_a, bool first_attempt)
{
	auto result (true);
	lock_a.unlock ();
	...
	if (connection_l && !stopped)
	{
		...
		{
			auto this_l (shared_from_this ());
			auto client (std::make_shared<vxlnetwork::frontier_req_client> (connection_l, this_l));
		...
		{
			account_count = vxlnetwork::narrow_cast<unsigned int> (frontier_pulls.size ());
			// Shuffle pulls
			...
			if (!frontier_pulls.empty ())
			{
				...
				{
					auto k = vxlnetwork::random_pool::generate_word32 (0, i);
		...
		if (node->config.logging.network_logging ())
		{
			...
			{
				node->stats.inc (vxlnetwork::stat::type::error, vxlnetwork::stat::detail::frontier_req, vxlnetwork::stat::dir::out);
			}
		}
	}
	return result;
...
void vxlnetwork::bootstrap_attempt_legacy::run_start (vxlnetwork::unique_lock<vxlnetwork::mutex> & lock_a)
{
...
void vxlnetwork::bootstrap_attempt_legacy::run ()
{
	...
	while (still_pulling ())
	{
		...
		lock.lock ();
		if (start_account.number () != std::numeric_limits<vxlnetwork::uint256_t>::max ())
		{
...
void vxlnetwork::bootstrap_attempt_legacy::get_information (boost::property_tree::ptree & tree_a)
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> lock (mutex);
