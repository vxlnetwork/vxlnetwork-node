...
namespace ipc
{
	...
	{
	public:
		std::unordered_set<vxlnetwork::ipc::access_permission, enum_hash> permissions;
	...
	public:
		...
		std::string id;
		std::vector<vxlnetwork::ipc::access_role> roles;
		void clear () override;
	};
	...
	{
	public:
		bool has_access (std::string const & credentials_a, vxlnetwork::ipc::access_permission permission_a) const;
		bool has_access_to_all (std::string const & credentials_a, std::initializer_list<vxlnetwork::ipc::access_permission> permissions_a) const;
		bool has_access_to_oneof (std::string const & credentials_a, std::initializer_list<vxlnetwork::ipc::access_permission> permissions_a) const;
		vxlnetwork::error deserialize_toml (vxlnetwork::tomlconfig &);

	private:
		/** Process allow and deny entries for the given subject */
		void set_effective_permissions (vxlnetwork::ipc::access_subject & subject_a, std::shared_ptr<cpptoml::table> const & config_subject_a);

		/** Clear current users, roles and default permissions */
		void clear ();

		std::unordered_map<std::string, vxlnetwork::ipc::access_user> users;
		std::unordered_map<std::string, vxlnetwork::ipc::access_role> roles;
