...
vxlnetwork::ipc::access_permission from_string (std::string permission)
{
	if (permission == "unrestricted")
		return vxlnetwork::ipc::access_permission::unrestricted;
	if (permission == "api_account_weight")
		return vxlnetwork::ipc::access_permission::api_account_weight;
	if (permission == "api_service_register")
		return vxlnetwork::ipc::access_permission::api_service_register;
	if (permission == "api_service_stop")
		return vxlnetwork::ipc::access_permission::api_service_stop;
	if (permission == "api_topic_service_stop")
		return vxlnetwork::ipc::access_permission::api_topic_service_stop;
	if (permission == "api_topic_confirmation")
		return vxlnetwork::ipc::access_permission::api_topic_confirmation;
	if (permission == "account_query")
		return vxlnetwork::ipc::access_permission::account_query;
	if (permission == "epoch_upgrade")
		return vxlnetwork::ipc::access_permission::epoch_upgrade;
	if (permission == "service")
		return vxlnetwork::ipc::access_permission::service;
	if (permission == "wallet")
		return vxlnetwork::ipc::access_permission::wallet;
	if (permission == "wallet_read")
		return vxlnetwork::ipc::access_permission::wallet_read;
	if (permission == "wallet_write")
		return vxlnetwork::ipc::access_permission::wallet_write;
	if (permission == "wallet_seed_change")
		return vxlnetwork::ipc::access_permission::wallet_seed_change;

	return vxlnetwork::ipc::access_permission::invalid;
}
}

void vxlnetwork::ipc::access::set_effective_permissions (vxlnetwork::ipc::access_subject & subject_a, std::shared_ptr<cpptoml::table> const & config_subject_a)
{
	...
	for (auto const & permission : allow_strings_l)
	{
		if (!permission.empty ())
		{
			...
			if (permission_enum != vxlnetwork::ipc::access_permission::invalid)
			{
...
void vxlnetwork::ipc::access::clear ()
{
	users.clear ();
	roles.clear ();
	...
	default_user.permissions.insert (vxlnetwork::ipc::access_permission::api_account_weight);
}

vxlnetwork::error vxlnetwork::ipc::access::deserialize_toml (vxlnetwork::tomlconfig & toml)
{
	...
	if (toml.has_key ("role"))
	{
		auto get_role = [this] (std::shared_ptr<cpptoml::table> const & role_a) {
			vxlnetwork::ipc::access_role role;
	...
	if (!error && toml.has_key ("user"))
	{
		auto get_user = [this, &error] (std::shared_ptr<cpptoml::table> const & user_a) {
			vxlnetwork::ipc::access_user user;
...
bool vxlnetwork::ipc::access::has_access (std::string const & credentials_a, vxlnetwork::ipc::access_permission permssion_a) const
{
	vxlnetwork::unique_lock<vxlnetwork::mutex> lock (mutex);
	...
	if (user != users.end ())
	{
		...
		if (!permitted)
		{
			permitted = user->second.permissions.find (vxlnetwork::ipc::access_permission::unrestricted) != user->second.permissions.end ();
		}
	}
	return permitted;
}

bool vxlnetwork::ipc::access::has_access_to_all (std::string const & credentials_a, std::initializer_list<vxlnetwork::ipc::access_permission> permissions_a) const
{
	vxlnetwork::unique_lock<vxlnetwork::mutex> lock (mutex);
	bool permitted = false;
	auto user = users.find (credentials_a);
...
bool vxlnetwork::ipc::access::has_access_to_oneof (std::string const & credentials_a, std::initializer_list<vxlnetwork::ipc::access_permission> permissions_a) const
{
	vxlnetwork::unique_lock<vxlnetwork::mutex> lock (mutex);
	bool permitted = false;
	auto user = users.find (credentials_a);
...
void vxlnetwork::ipc::access_subject::clear ()
{
	permissions.clear ();
}

void vxlnetwork::ipc::access_user::clear ()
{
...
namespace ipc
{
	vxlnetwork::error read_access_config_toml (boost::filesystem::path const & data_path_a, vxlnetwork::ipc::access & config_a)
	{
		vxlnetwork::error error;
		auto toml_config_path = vxlnetwork::get_access_toml_config_path (data_path_a);
