...
namespace
{
auto handler_map = vxlnetwork::ipc::action_handler::handler_map ();
...
vxlnetwork::ipc::flatbuffers_handler::flatbuffers_handler (vxlnetwork::node & node_a, vxlnetwork::ipc::ipc_server & ipc_server_a, std::shared_ptr<vxlnetwork::ipc::subscriber> const & subscriber_a, vxlnetwork::ipc::ipc_config const & ipc_config_a) :
	node (node_a),
...
std::shared_ptr<flatbuffers::Parser> vxlnetwork::ipc::flatbuffers_handler::make_flatbuffers_parser (vxlnetwork::ipc::ipc_config const & ipc_config_a)
{
	...
	if (!api_path)
	{
		throw vxlnetwork::error ("Internal IPC error: unable to find api path");
	}

	char const * include_directories[] = { api_path->string ().c_str (), nullptr };
	std::string schemafile;
	if (!flatbuffers::LoadFile ((*api_path / "vxlnetworkapi.fbs").string ().c_str (), false, &schemafile))
	{
		throw vxlnetwork::error ("Internal IPC error: unable to load schema file");
	}
	...
	if (!parse_success)
	{
		...
		throw vxlnetwork::error (parser_error);
	}
	return parser;
}

void vxlnetwork::ipc::flatbuffers_handler::process_json (uint8_t const * message_buffer_a, std::size_t buffer_size_a,
...
void vxlnetwork::ipc::flatbuffers_handler::process (uint8_t const * message_buffer_a, std::size_t buffer_size_a,
std::function<void (std::shared_ptr<flatbuffers::FlatBufferBuilder> const &)> const & response_handler)
{
	...
	try
	{
		...
		if (ipc_config.flatbuffers.verify_buffers)
		{
			...
			{
				throw vxlnetwork::error ("Envelope buffer did not pass verifier");
			}
		}

		auto incoming = vxlnetworkapi::GetEnvelope (message_buffer_a);
		if (incoming == nullptr)
		{
			vxlnetwork::error err ("Invalid message");
		...
		{
			vxlnetwork::error err ("Unknown message type");
			actionhandler->make_error (err.error_code_as_int (), err.get_message ());
		}
	}
	catch (vxlnetwork::error const & err)
