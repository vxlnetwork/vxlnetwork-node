...
class stream_wrapper : public vxlnetwork::websocket::websocket_stream_concept
{
public:
	...
	void async_write (vxlnetwork::shared_const_buffer const & buffer_a, std::function<void (boost::system::error_code, std::size_t)> callback_a) override
	{
...
#ifdef VXLNETWORK_SECURE_RPC
vxlnetwork::websocket::stream::stream (socket_type socket_a, boost::asio::ssl::context & ctx_a)
{
	impl = std::make_unique<stream_wrapper<wss_type>> (std::move (socket_a), ctx_a);
}
#endif
vxlnetwork::websocket::stream::stream (socket_type socket_a)
{
	impl = std::make_unique<stream_wrapper<ws_type>> (std::move (socket_a));
}

[[nodiscard]] boost::asio::strand<boost::asio::io_context::executor_type> & vxlnetwork::websocket::stream::get_strand ()
{
	return impl->get_strand ();
}

void vxlnetwork::websocket::stream::handshake (std::function<void (boost::system::error_code const & ec)> callback_a)
{
	impl->handshake (callback_a);
}

void vxlnetwork::websocket::stream::close (boost::beast::websocket::close_reason const & reason_a, boost::system::error_code & ec_a)
{
	impl->close (reason_a, ec_a);
}

void vxlnetwork::websocket::stream::async_write (vxlnetwork::shared_const_buffer const & buffer_a, std::function<void (boost::system::error_code, std::size_t)> callback_a)
{
	impl->async_write (buffer_a, callback_a);
}

void vxlnetwork::websocket::stream::async_read (boost::beast::multi_buffer & buffer_a, std::function<void (boost::system::error_code, std::size_t)> callback_a)
{
