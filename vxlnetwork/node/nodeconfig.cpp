...
std::string const default_live_peer_network = vxlnetwork::get_env_or_default ("VXLNETWORK_DEFAULT_PEER", "live.vxl.network");//
std::string const default_beta_peer_network = vxlnetwork::get_env_or_default ("VXLNETWORK_DEFAULT_PEER", "beta.vxl.network");
std::string const default_test_peer_network = vxlnetwork::get_env_or_default ("VXLNETWORK_DEFAULT_PEER", "test.vxl.network");
}

vxlnetwork::node_config::node_config (vxlnetwork::network_params & network_params) :
	node_config (std::nullopt, vxlnetwork::logging (), network_params)
{
}

vxlnetwork::node_config::node_config (const std::optional<uint16_t> & peering_port_a, vxlnetwork::logging const & logging_a, vxlnetwork::network_params & network_params) :
	...
	external_address{ boost::asio::ip::address_v6{}.to_string () }
{
	...
	switch (network_params.network.network ())
	{
		...
		case vxlnetwork::networks::vxlnetwork_beta_network:
		{
			...
			vxlnetwork::account offline_representative;
			release_assert (!offline_representative.decode_account ("vxlc_1beta5mgf3rn4nyrheqnzbrsbcytcg6kfsootzocfb3gmdpdsd45x91zm4xx"));
		...
		case vxlnetwork::networks::vxlnetwork_live_network:
			...
			// SET AT LEAST TWO REPRESENTATIVES
			preconfigured_representatives.emplace_back ("6196C33418B5E39D3E8940D968E252E0C04EBB23AF6E03EDE3871F1A4A1BF969");//vxlc_1repret3jfh5mnzaki8sf5j77r81btxk9dug1hpy93rz5b73qydbxw94oih6
			//preconfigured_representatives.emplace_back ("6196C33A5AF1CAC771B8E277BBFBD2CB75A366681F8F330F80B8B38874044357");//vxlc_1reprex7owgcrxrujrmqqhxx7kuonfm8i9wh8e9r3g7mj3t1aitqbthmttki
			//preconfigured_representatives.emplace_back ("6196C339D568AB5A3736BA30E939DCC1E010F5BFA5A5B2A343FA3F965C856BB2");//vxlc_1reprewxct7ddaumfgjix6wxsih145tuzbf7pcjn9yjzksgactxktpt7bmwf
			break;
...
vxlnetwork::error vxlnetwork::node_config::serialize_toml (vxlnetwork::tomlconfig & toml) const
{
	...
	vxlnetwork::tomlconfig experimental_l;
	...
	vxlnetwork::tomlconfig callback_l;
...
vxlnetwork::error vxlnetwork::node_config::deserialize_toml (vxlnetwork::tomlconfig & toml)
{
	try
	{
		...
		if (frontiers_confirmation == vxlnetwork::frontiers_confirmation_mode::invalid)
		{
...
std::string vxlnetwork::node_config::serialize_frontiers_confirmation (vxlnetwork::frontiers_confirmation_mode mode_a) const
{
	switch (mode_a)
	{
		case vxlnetwork::frontiers_confirmation_mode::always:
			return "always";
		case vxlnetwork::frontiers_confirmation_mode::automatic:
			return "auto";
		case vxlnetwork::frontiers_confirmation_mode::disabled:
			return "disabled";
		default:
			return "auto";
	}
...
vxlnetwork::frontiers_confirmation_mode vxlnetwork::node_config::deserialize_frontiers_confirmation (std::string const & string_a)
{
	if (string_a == "always")
	{
		return vxlnetwork::frontiers_confirmation_mode::always;
	}
	else if (string_a == "auto")
	{
		return vxlnetwork::frontiers_confirmation_mode::automatic;
	}
	else if (string_a == "disabled")
	{
		return vxlnetwork::frontiers_confirmation_mode::disabled;
	}
	else
	{
		return vxlnetwork::frontiers_confirmation_mode::invalid;
	}
}

void vxlnetwork::node_config::deserialize_address (std::string const & entry_a, std::vector<std::pair<std::string, uint16_t>> & container_a) const
{
...
vxlnetwork::account vxlnetwork::node_config::random_representative () const
{
	debug_assert (!preconfigured_representatives.empty ());
	std::size_t index (vxlnetwork::random_pool::generate_word32 (0, static_cast<CryptoPP::word32> (preconfigured_representatives.size () - 1)));
