...
{
public:
	explicit matches_txn (vxlnetwork::transaction_impl const * transaction_impl_a) :
		transaction_impl (transaction_impl_a)
	{
	}

	bool operator() (vxlnetwork::mdb_txn_stats const & mdb_txn_stats)
	{
...
vxlnetwork::read_mdb_txn::read_mdb_txn (vxlnetwork::mdb_env const & environment_a, vxlnetwork::mdb_txn_callbacks txn_callbacks_a) :
...
vxlnetwork::read_mdb_txn::~read_mdb_txn ()
{
...
void vxlnetwork::read_mdb_txn::reset ()
{
	mdb_txn_reset (handle);
	txn_callbacks.txn_end (this);
}

void vxlnetwork::read_mdb_txn::renew ()
{
...
void * vxlnetwork::read_mdb_txn::get_handle () const
{
	return handle;
}

vxlnetwork::write_mdb_txn::write_mdb_txn (vxlnetwork::mdb_env const & environment_a, vxlnetwork::mdb_txn_callbacks txn_callbacks_a) :
	env (environment_a),
	txn_callbacks (txn_callbacks_a)
{
	renew ();
...
vxlnetwork::write_mdb_txn::~write_mdb_txn ()
{
	commit ();
}

void vxlnetwork::write_mdb_txn::commit ()
{
	if (active)
	{
		auto status (mdb_txn_commit (handle));
...
void vxlnetwork::write_mdb_txn::renew ()
{
...
void * vxlnetwork::write_mdb_txn::get_handle () const
{
	return handle;
}

bool vxlnetwork::write_mdb_txn::contains (vxlnetwork::tables table_a) const
{
	// LMDB locks on every write
	return true;
}

vxlnetwork::mdb_txn_tracker::mdb_txn_tracker (vxlnetwork::logger_mt & logger_a, vxlnetwork::txn_tracking_config const & txn_tracking_config_a, std::chrono::milliseconds block_processor_batch_max_time_a) :
	logger (logger_a),
...
void vxlnetwork::mdb_txn_tracker::serialize_json (boost::property_tree::ptree & json, std::chrono::milliseconds min_read_time, std::chrono::milliseconds min_write_time)
{
	...
	for (std::size_t i = 0; i < times_since_start.size (); ++i)
	{
		...
		{
			vxlnetwork::jsonconfig mdb_lock_config;
			...
			{
				vxlnetwork::jsonconfig frame_json;
			...
			vxlnetwork::jsonconfig stack (stacktrace_config);
...
void vxlnetwork::mdb_txn_tracker::log_if_held_long_enough (vxlnetwork::mdb_txn_stats const & mdb_txn_stats) const
{
	...
	bool is_blk_processing_thread = mdb_txn_stats.thread_name == vxlnetwork::thread_role::get_string (vxlnetwork::thread_role::name::block_processing);
...
void vxlnetwork::mdb_txn_tracker::add (vxlnetwork::transaction_impl const * transaction_impl)
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> guard (mutex);
...
void vxlnetwork::mdb_txn_tracker::erase (vxlnetwork::transaction_impl const * transaction_impl)
{
...
vxlnetwork::mdb_txn_stats::mdb_txn_stats (vxlnetwork::transaction_impl const * transaction_impl) :
	transaction_impl (transaction_impl),
	thread_name (vxlnetwork::thread_role::get_string ()),
	stacktrace (std::make_shared<boost::stacktrace::stacktrace> ())
{
	timer.start ();
}

bool vxlnetwork::mdb_txn_stats::is_write () const
{
	return (dynamic_cast<vxlnetwork::write_transaction_impl const *> (transaction_impl) != nullptr);
}
