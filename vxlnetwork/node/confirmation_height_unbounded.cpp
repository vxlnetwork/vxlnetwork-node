...
#include <vxlnetwork/node/confirmation_height_unbounded.hpp>
...
void vxlnetwork::confirmation_height_unbounded::process (std::shared_ptr<vxlnetwork::block> original_block)
{
	if (pending_empty ())
	{
		clear_process_vars ();
	...
	std::vector<vxlnetwork::block_hash> orig_block_callback_data;
	...
	do
	{
		...
		if (first_iter)
		{
			...
			vxlnetwork::lock_guard<vxlnetwork::mutex> guard (block_cache_mutex);
		...
		std::vector<vxlnetwork::block_hash> block_callback_datas_required;
...
void vxlnetwork::confirmation_height_unbounded::collect_unconfirmed_receive_and_sources_for_account (uint64_t block_height_a, uint64_t confirmation_height_a, std::shared_ptr<vxlnetwork::block> const & block_a, vxlnetwork::block_hash const & hash_a, vxlnetwork::account const & account_a, vxlnetwork::read_transaction const & transaction_a, std::vector<receive_source_pair> & receive_source_pairs_a, std::vector<vxlnetwork::block_hash> & block_callback_data_a, std::vector<vxlnetwork::block_hash> & orig_block_callback_data_a, std::shared_ptr<vxlnetwork::block> original_block)
{
	...
	while ((num_to_confirm > 0) && !hash.is_zero () && !stopped)
	{
		...
		if (block)
		{
			...
			if (!source.is_zero () && !ledger.is_epoch_link (source) && ledger.store.block.exists (transaction_a, source))
			{
				...
				receive_source_pairs_a.emplace_back (std::make_shared<conf_height_details> (account_a, hash, block_height, 1, std::vector<vxlnetwork::block_hash>{ hash }), source);
			}
...
void vxlnetwork::confirmation_height_unbounded::prepare_iterated_blocks_for_cementing (preparation_data & preparation_data_a)
{
 ...
 */
void vxlnetwork::confirmation_height_unbounded::cement_blocks (vxlnetwork::write_guard & scoped_write_guard_a)
{
	vxlnetwork::timer<std::chrono::milliseconds> cemented_batch_timer;
	...
	auto error = false;
	{
		...
		while (!pending_writes.empty ())
		{
			...
			if (pending.height > confirmation_height)
			{
				...
				ledger.stats.add (vxlnetwork::stat::type::confirmation_height, vxlnetwork::stat::detail::blocks_confirmed, vxlnetwork::stat::dir::in, pending.height - confirmation_height);
				ledger.stats.add (vxlnetwork::stat::type::confirmation_height, vxlnetwork::stat::detail::blocks_confirmed_unbounded, vxlnetwork::stat::dir::in, pending.height - confirmation_height);
...
std::shared_ptr<vxlnetwork::block> vxlnetwork::confirmation_height_unbounded::get_block_and_sideband (vxlnetwork::block_hash const & hash_a, vxlnetwork::transaction const & transaction_a)
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> guard (block_cache_mutex);
...
bool vxlnetwork::confirmation_height_unbounded::pending_empty () const
{
	return pending_writes.empty ();
}

void vxlnetwork::confirmation_height_unbounded::clear_process_vars ()
{
...
bool vxlnetwork::confirmation_height_unbounded::has_iterated_over_block (vxlnetwork::block_hash const & hash_a) const
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> guard (block_cache_mutex);
	return block_cache.count (hash_a) == 1;
}

uint64_t vxlnetwork::confirmation_height_unbounded::block_cache_size () const
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> guard (block_cache_mutex);
	return block_cache.size ();
}

vxlnetwork::confirmation_height_unbounded::conf_height_details::conf_height_details (vxlnetwork::account const & account_a, vxlnetwork::block_hash const & hash_a, uint64_t height_a, uint64_t num_blocks_confirmed_a, std::vector<vxlnetwork::block_hash> const & block_callback_data_a) :
	account (account_a),
	hash (hash_a),
	height (height_a),
...
vxlnetwork::confirmation_height_unbounded::receive_source_pair::receive_source_pair (std::shared_ptr<conf_height_details> const & receive_details_a, const block_hash & source_a) :
	receive_details (receive_details_a),
	source_hash (source_a)
{
}

vxlnetwork::confirmation_height_unbounded::confirmed_iterated_pair::confirmed_iterated_pair (uint64_t confirmed_height_a, uint64_t iterated_height_a) :
	confirmed_height (confirmed_height_a),
	iterated_height (iterated_height_a)
{
}

std::unique_ptr<vxlnetwork::container_info_component> vxlnetwork::collect_container_info (confirmation_height_unbounded & confirmation_height_unbounded, std::string const & name_a)
{
