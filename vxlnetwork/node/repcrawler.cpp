...
vxlnetwork::rep_crawler::rep_crawler (vxlnetwork::node & node_a) :
	node (node_a)
{
	if (!node.flags.disable_rep_crawler)
	{
		node.observers.endpoint.add ([this] (std::shared_ptr<vxlnetwork::transport::channel> const & channel_a) {
			this->query (channel_a);
		});
	}
}

void vxlnetwork::rep_crawler::remove (vxlnetwork::block_hash const & hash_a)
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> lock (active_mutex);
	active.erase (hash_a);
}

void vxlnetwork::rep_crawler::start ()
{
	ongoing_crawl ();
}

void vxlnetwork::rep_crawler::validate ()
{
	...
	for (auto const & i : responses_l)
	{
		...
		if (channel->get_type () == vxlnetwork::transport::transport_type::loopback)
		{
		...
		vxlnetwork::uint128_t rep_weight = node.ledger.weight (vote->account);
		...
		auto inserted = false;
		auto updated = false;
		std::shared_ptr<vxlnetwork::transport::channel> prev_channel;

		vxlnetwork::unique_lock<vxlnetwork::mutex> lock (probable_reps_mutex);

		auto existing (probable_reps.find (vote->account));
		if (existing != probable_reps.end ())
		{
			probable_reps.modify (existing, [rep_weight, &updated, &vote, &channel, &prev_channel] (vxlnetwork::representative & info) {
		...
		{
			probable_reps.emplace (vxlnetwork::representative (vote->account, rep_weight, channel));
			inserted = true;
		}
...
void vxlnetwork::rep_crawler::ongoing_crawl ()
{
...
std::vector<std::shared_ptr<vxlnetwork::transport::channel>> vxlnetwork::rep_crawler::get_crawl_targets (vxlnetwork::uint128_t total_weight_a)
{
	...
	std::vector<std::shared_ptr<vxlnetwork::transport::channel>> result;
	result.insert (result.end (), random_peers.begin (), random_peers.end ());
	return result;
}

void vxlnetwork::rep_crawler::query (std::vector<std::shared_ptr<vxlnetwork::transport::channel>> const & channels_a)
{
...
void vxlnetwork::rep_crawler::query (std::shared_ptr<vxlnetwork::transport::channel> const & channel_a)
{
	std::vector<std::shared_ptr<vxlnetwork::transport::channel>> peers;
	peers.emplace_back (channel_a);
	query (peers);
}

void vxlnetwork::rep_crawler::throttled_remove (vxlnetwork::block_hash const & hash_a, uint64_t const target_finished_processed)
{
...
bool vxlnetwork::rep_crawler::is_pr (vxlnetwork::transport::channel const & channel_a) const
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> lock (probable_reps_mutex);
...
bool vxlnetwork::rep_crawler::response (std::shared_ptr<vxlnetwork::transport::channel> const & channel_a, std::shared_ptr<vxlnetwork::vote> const & vote_a)
{
	bool error = true;
	vxlnetwork::lock_guard<vxlnetwork::mutex> lock (active_mutex);
...
vxlnetwork::uint128_t vxlnetwork::rep_crawler::total_weight () const
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> lock (probable_reps_mutex);
...
void vxlnetwork::rep_crawler::on_rep_request (std::shared_ptr<vxlnetwork::transport::channel> const & channel_a)
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> lock (probable_reps_mutex);
	if (channel_a->get_tcp_endpoint ().address () != boost::asio::ip::address_v6::any ())
	{
		...
		{
			channel_ref_index.modify (itr_pair.first, [] (vxlnetwork::representative & value_a) {
...
void vxlnetwork::rep_crawler::cleanup_reps ()
{
	std::vector<std::shared_ptr<vxlnetwork::transport::channel>> channels;
	{
	...
	{
		bool equal (false);
		if (i->get_type () == vxlnetwork::transport::transport_type::tcp)
		{
			auto find_channel (node.network.tcp_channels.find_channel (i->get_tcp_endpoint ()));
			if (find_channel != nullptr && *find_channel == *static_cast<vxlnetwork::transport::channel_tcp *> (i.get ()))
			{
				equal = true;
			}
		}
		else if (i->get_type () == vxlnetwork::transport::transport_type::udp)
		{
			auto find_channel (node.network.udp_channels.channel (i->get_endpoint ()));
			if (find_channel != nullptr && *find_channel == *static_cast<vxlnetwork::transport::channel_udp *> (i.get ()))
			{
				equal = true;
			}
		}
...
void vxlnetwork::rep_crawler::update_weights ()
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> lock (probable_reps_mutex);
	for (auto i (probable_reps.get<tag_last_request> ().begin ()), n (probable_reps.get<tag_last_request> ().end ()); i != n;)
	{
		...
		if (weight > 0)
		{
			...
			{
				probable_reps.get<tag_last_request> ().modify (i, [weight] (vxlnetwork::representative & info) {
					info.weight = weight;
				});
			}
			++i;
...
std::vector<vxlnetwork::representative> vxlnetwork::rep_crawler::representatives (std::size_t count_a, vxlnetwork::uint128_t const weight_a, boost::optional<decltype (vxlnetwork::network_constants::protocol_version)> const & opt_version_min_a)
{
...
std::vector<vxlnetwork::representative> vxlnetwork::rep_crawler::principal_representatives (std::size_t count_a, boost::optional<decltype (vxlnetwork::network_constants::protocol_version)> const & opt_version_min_a)
{
	return representatives (count_a, node.minimum_principal_weight (), opt_version_min_a);
}

std::vector<std::shared_ptr<vxlnetwork::transport::channel>> vxlnetwork::rep_crawler::representative_endpoints (std::size_t count_a)
{
	std::vector<std::shared_ptr<vxlnetwork::transport::channel>> result;
...
std::size_t vxlnetwork::rep_crawler::representative_count ()
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> lock (probable_reps_mutex);
