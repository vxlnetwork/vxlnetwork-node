...
double constexpr vxlnetwork::node::price_max;
double constexpr vxlnetwork::node::free_cutoff;
std::size_t constexpr vxlnetwork::block_arrival::arrival_size_min;
std::chrono::seconds constexpr vxlnetwork::block_arrival::arrival_time_min;

namespace vxlnetwork
{
extern unsigned char vxlnetwork_bootstrap_weights_live[];
extern std::size_t vxlnetwork_bootstrap_weights_live_size;
extern unsigned char vxlnetwork_bootstrap_weights_beta[];
extern std::size_t vxlnetwork_bootstrap_weights_beta_size;
}

void vxlnetwork::node::keepalive (std::string const & address_a, uint16_t port_a)
{
	...
	network.resolver.async_resolve (boost::asio::ip::udp::resolver::query (address_a, std::to_string (port_a)), [node_l, address_a, port_a] (boost::system::error_code const & ec, boost::asio::ip::udp::resolver::iterator i_a) {
		if (!ec)
		{
			...
			{
				auto endpoint (vxlnetwork::transport::map_endpoint_to_v6 (i->endpoint ()));
				std::weak_ptr<vxlnetwork::node> node_w (node_l);
...
std::unique_ptr<vxlnetwork::container_info_component> vxlnetwork::collect_container_info (rep_crawler & rep_crawler, std::string const & name)
{
	std::size_t count;
	{
		vxlnetwork::lock_guard<vxlnetwork::mutex> guard (rep_crawler.active_mutex);
...
vxlnetwork::node::node (boost::asio::io_context & io_ctx_a, uint16_t peering_port_a, boost::filesystem::path const & application_path_a, vxlnetwork::logging const & logging_a, vxlnetwork::work_pool & work_a, vxlnetwork::node_flags flags_a, unsigned seq) :
	node (io_ctx_a, application_path_a, vxlnetwork::node_config (peering_port_a, logging_a), work_a, flags_a, seq)
{
}

vxlnetwork::node::node (boost::asio::io_context & io_ctx_a, boost::filesystem::path const & application_path_a, vxlnetwork::node_config const & config_a, vxlnetwork::work_pool & work_a, vxlnetwork::node_flags flags_a, unsigned seq) :
	...
	workers (std::max (3u, config.io_threads / 4), vxlnetwork::thread_role::name::worker),
	flags (flags_a),
	work (work_a),
	distributed_work (*this),
	logger (config_a.logging.min_time_between_log_output),
	store_impl (vxlnetwork::make_store (logger, application_path_a, network_params.ledger, flags.read_only, true, config_a.rocksdb_config, config_a.diagnostics_config.txn_tracking, config_a.block_processor_batch_max_time, config_a.lmdb_config, config_a.backup_before_upgrade)),
	store (*store_impl),
	unchecked{ store, flags.disable_block_processor_unchecked_deletion },
	wallets_store_impl (std::make_unique<vxlnetwork::mdb_wallets_store> (application_path_a / "wallets.ldb", config_a.lmdb_config)),
	...
	telemetry (std::make_shared<vxlnetwork::telemetry> (network, workers, observers.telemetry, stats, network_params, flags.disable_ongoing_telemetry_requests)),
	...
	node_seq (seq)
{
	unchecked.satisfied = [this] (vxlnetwork::unchecked_info const & info) {
	...
	if (!init_error ())
	{
		...
		{
			auto endpoint_l (vxlnetwork::tcp_endpoint (boost::asio::ip::make_address_v6 (config.websocket_config.address), config.websocket_config.port));
			websocket_server = std::make_shared<vxlnetwork::websocket::listener> (config.websocket_config.tls_config, logger, wallets, io_ctx, endpoint_l);
		...
		network.channel_observer = [this] (std::shared_ptr<vxlnetwork::transport::channel> const & channel_a) {
		...
		{
			observers.blocks.add ([this] (vxlnetwork::election_status const & status_a, std::vector<vxlnetwork::vote_with_weight_info> const & votes_a, vxlnetwork::account const & account_a, vxlnetwork::amount const & amount_a, bool is_state_send_a, bool is_state_epoch_a) {
				auto block_a (status_a.winner);
				if ((status_a.type == vxlnetwork::election_status_type::active_confirmed_quorum || status_a.type == vxlnetwork::election_status_type::active_confirmation_height) && this->block_arrival.recent (block_a->hash ()))
				{
					auto node_l (shared_from_this ());
					background ([node_l, block_a, account_a, amount_a, is_state_send_a, is_state_epoch_a] () {
						...
						// Subtype field
						else if (block_a->type () == vxlnetwork::block_type::state)
						{
						...
						resolver->async_resolve (boost::asio::ip::tcp::resolver::query (address, std::to_string (port)), [node_l, address, port, target, body, resolver] (boost::system::error_code const & ec, boost::asio::ip::tcp::resolver::iterator i_a) {
							...
							else
							{
								...
								node_l->stats.inc (vxlnetwork::stat::type::error, vxlnetwork::stat::detail::http_callback, vxlnetwork::stat::dir::out);
							}
						});
					});
				}
		...
		{
			observers.blocks.add ([this] (vxlnetwork::election_status const & status_a, std::vector<vxlnetwork::vote_with_weight_info> const & votes_a, vxlnetwork::account const & account_a, vxlnetwork::amount const & amount_a, bool is_state_send_a, bool is_state_epoch_a) {
				debug_assert (status_a.type != vxlnetwork::election_status_type::ongoing);

				if (this->websocket_server->any_subscriber (vxlnetwork::websocket::topic::confirmation))
				{
					auto block_a (status_a.winner);
					std::string subtype;
					if (is_state_send_a)
			...
			observers.active_stopped.add ([this] (vxlnetwork::block_hash const & hash_a) {
				if (this->websocket_server->any_subscriber (vxlnetwork::websocket::topic::stopped_election))
				{
			...
			observers.telemetry.add ([this] (vxlnetwork::telemetry_data const & telemetry_data, vxlnetwork::endpoint const & endpoint) {
				if (this->websocket_server->any_subscriber (vxlnetwork::websocket::topic::telemetry))
				{
					vxlnetwork::websocket::message_builder builder;
		...
		observers.blocks.add ([this] (vxlnetwork::election_status const & status_a, std::vector<vxlnetwork::vote_with_weight_info> const & votes_a, vxlnetwork::account const & account_a, vxlnetwork::amount const & amount_a, bool is_state_send_a, bool is_state_epoch_a) {
			...
			{
				case vxlnetwork::election_status_type::active_confirmed_quorum:
					this->stats.inc (vxlnetwork::stat::type::confirmation_observer, vxlnetwork::stat::detail::active_quorum, vxlnetwork::stat::dir::out);
					break;
				case vxlnetwork::election_status_type::active_confirmation_height:
					this->stats.inc (vxlnetwork::stat::type::confirmation_observer, vxlnetwork::stat::detail::active_conf_height, vxlnetwork::stat::dir::out);
					break;
				case vxlnetwork::election_status_type::inactive_confirmation_height:
					this->stats.inc (vxlnetwork::stat::type::confirmation_observer, vxlnetwork::stat::detail::inactive_conf_height, vxlnetwork::stat::dir::out);
					break;
				default:
					break;
			}
		});
		observers.endpoint.add ([this] (std::shared_ptr<vxlnetwork::transport::channel> const & channel_a) {
			if (channel_a->get_type () == vxlnetwork::transport::transport_type::udp)
			{
		...
		observers.vote.add ([this] (std::shared_ptr<vxlnetwork::vote> vote_a, std::shared_ptr<vxlnetwork::transport::channel> const & channel_a, vxlnetwork::vote_code code_a) {
			debug_assert (code_a != vxlnetwork::vote_code::invalid);
			// The vote_code::vote is handled inside the election
			if (code_a == vxlnetwork::vote_code::indeterminate)
			{
		...
		{
			observers.vote.add ([this] (std::shared_ptr<vxlnetwork::vote> vote_a, std::shared_ptr<vxlnetwork::transport::channel> const & channel_a, vxlnetwork::vote_code code_a) {
				if (this->websocket_server->any_subscriber (vxlnetwork::websocket::topic::vote))
				{
					vxlnetwork::websocket::message_builder builder;
		...
		observers.work_cancel.add ([this] (vxlnetwork::root const & root_a) {
			this->work.cancel (root_a);
			this->distributed_work.cancel (root_a);
		});

		logger.always_log ("Node starting, version: ", VXLNETWORK_VERSION_STRING);
		...
		if ((network_params.network.is_live_network () || network_params.network.is_beta_network ()) && !flags.inactive_node)
		{
			...
			if (use_bootstrap_weight)
			{
				...
				{
					logger.always_log ("Using bootstrap rep weight: ", rep.first.to_account (), " -> ", vxlnetwork::uint128_union (rep.second).format_balance (Mxrb_ratio, 0, true), " XRB");
				}
			}
...
void vxlnetwork::node::do_rpc_callback (boost::asio::ip::tcp::resolver::iterator i_a, std::string const & address, uint16_t port, std::shared_ptr<std::string> const & target, std::shared_ptr<std::string> const & body, std::shared_ptr<boost::asio::ip::tcp::resolver> const & resolver)
{
					...
					if (!ec)
					{
						...
						boost::beast::http::async_read (*sock, *sb, *resp, [node_l, sb, resp, sock, address, port, i_a, target, body, resolver] (boost::system::error_code const & ec, std::size_t bytes_transferred) mutable {
							if (!ec)
							{
								...
								{
									node_l->stats.inc (vxlnetwork::stat::type::http_callback, vxlnetwork::stat::detail::initiate, vxlnetwork::stat::dir::out);
								}
								else
								{
									if (node_l->config.logging.callback_logging ())
...
bool vxlnetwork::node::copy_with_compaction (boost::filesystem::path const & destination)
{
	return store.copy_db (destination);
}

std::unique_ptr<vxlnetwork::container_info_component> vxlnetwork::collect_container_info (node & node, std::string const & name)
{
	auto composite = std::make_unique<container_info_composite> (name);
...
void vxlnetwork::node::process_active (std::shared_ptr<vxlnetwork::block> const & incoming)
{
	block_arrival.add (incoming->hash ());
	block_processor.add (incoming);
}

vxlnetwork::process_return vxlnetwork::node::process (vxlnetwork::block & block_a)
{
...
vxlnetwork::process_return vxlnetwork::node::process_local (std::shared_ptr<vxlnetwork::block> const & block_a)
{
	...
	auto const transaction (store.tx_begin_write ({ tables::accounts, tables::blocks, tables::frontiers, tables::pending }));
	return block_processor.process_one (transaction, post_events, info, false, vxlnetwork::block_origin::local);
}

void vxlnetwork::node::process_local_async (std::shared_ptr<vxlnetwork::block> const & block_a)
{
	// Add block hash as recently arrived to trigger automatic rebroadcast and election
	block_arrival.add (block_a->hash ());
	// Set current time to trigger automatic rebroadcast and election
...
void vxlnetwork::node::start ()
{
	...
	wallets.start ();
	if (config.frontiers_confirmation != vxlnetwork::frontiers_confirmation_mode::disabled)
	{
...
void vxlnetwork::node::keepalive_preconfigured (std::vector<std::string> const & peers_a)
{
...
vxlnetwork::block_hash vxlnetwork::node::latest (vxlnetwork::account const & account_a)
{
	auto const transaction (store.tx_begin_read ());
	return ledger.latest (transaction, account_a);
}

vxlnetwork::uint128_t vxlnetwork::node::balance (vxlnetwork::account const & account_a)
{
	auto const transaction (store.tx_begin_read ());
	return ledger.account_balance (transaction, account_a);
}

std::shared_ptr<vxlnetwork::block> vxlnetwork::node::block (vxlnetwork::block_hash const & hash_a)
{
	auto const transaction (store.tx_begin_read ());
	return store.block.get (transaction, hash_a);
}

std::pair<vxlnetwork::uint128_t, vxlnetwork::uint128_t> vxlnetwork::node::balance_pending (vxlnetwork::account const & account_a, bool only_confirmed_a)
{
	std::pair<vxlnetwork::uint128_t, vxlnetwork::uint128_t> result;
	auto const transaction (store.tx_begin_read ());
...
vxlnetwork::uint128_t vxlnetwork::node::weight (vxlnetwork::account const & account_a)
{
	return ledger.weight (account_a);
}

vxlnetwork::block_hash vxlnetwork::node::rep_block (vxlnetwork::account const & account_a)
{
	auto const transaction (store.tx_begin_read ());
...
vxlnetwork::uint128_t vxlnetwork::node::minimum_principal_weight ()
{
	return minimum_principal_weight (online_reps.trended ());
}

vxlnetwork::uint128_t vxlnetwork::node::minimum_principal_weight (vxlnetwork::uint128_t const & online_stake)
{
	return online_stake / network_params.network.principal_weight_factor;
}

void vxlnetwork::node::long_inactivity_cleanup ()
{
...
void vxlnetwork::node::ongoing_rep_calculation ()
{
	auto now (std::chrono::steady_clock::now ());
	vote_processor.calculate_weights ();
	std::weak_ptr<vxlnetwork::node> node_w (shared_from_this ());
...
void vxlnetwork::node::ongoing_bootstrap ()
{
	...
	auto previous_bootstrap_count (stats.count (vxlnetwork::stat::type::bootstrap, vxlnetwork::stat::detail::initiate, vxlnetwork::stat::dir::out) + stats.count (vxlnetwork::stat::type::bootstrap, vxlnetwork::stat::detail::initiate_legacy_age, vxlnetwork::stat::dir::out));
	/* 
...
void vxlnetwork::node::ongoing_peer_store ()
{
...
void vxlnetwork::node::backup_wallet ()
{
	...
	for (auto i (wallets.items.begin ()), n (wallets.items.end ()); i != n; ++i)
	{
		...
		vxlnetwork::set_secure_perm_directory (backup_path, error_chmod);
...
void vxlnetwork::node::search_receivable_all ()
{
...
void vxlnetwork::node::bootstrap_wallet ()
{
	std::deque<vxlnetwork::account> accounts;
	{
		vxlnetwork::lock_guard<vxlnetwork::mutex> lock (wallets.mutex);
		...
		for (auto i (wallets.items.begin ()), n (wallets.items.end ()); i != n && accounts.size () < 128; ++i)
		{
			...
			vxlnetwork::lock_guard<std::recursive_mutex> wallet_lock (wallet.store.mutex);
			for (auto j (wallet.store.begin (transaction)), m (wallet.store.end ()); j != m && accounts.size () < 128; ++j)
			{
				vxlnetwork::account account (j->first);
...
void vxlnetwork::node::unchecked_cleanup ()
{
	std::vector<vxlnetwork::uint128_t> digests;
	std::deque<vxlnetwork::unchecked_key> cleaning_list;
	...
	{
		auto const now (vxlnetwork::seconds_since_epoch ());
		auto const transaction (store.tx_begin_read ());
		// Max 1M records to clean, max 2 minutes reading to prevent slow i/o systems issues
		for (auto [i, n] = unchecked.full_range (transaction); i != n && cleaning_list.size () < 1024 * 1024 && vxlnetwork::seconds_since_epoch () - now < 120; ++i)
		{
...
void vxlnetwork::node::ongoing_unchecked_cleanup ()
{
	unchecked_cleanup ();
...
void vxlnetwork::node::ongoing_backlog_population ()
{
	populate_backlog ();
...
bool vxlnetwork::node::collect_ledger_pruning_targets (std::deque<vxlnetwork::block_hash> & pruning_targets_a, vxlnetwork::account & last_account_a, uint64_t const batch_read_size_a, uint64_t const max_depth_a, uint64_t const cutoff_time_a)
{
	...
	for (auto i (store.confirmation_height.begin (transaction, last_account_a)), n (store.confirmation_height.end ()); i != n && !finish_transaction;)
	{
		...
		vxlnetwork::block_hash hash (i->second.frontier);
		uint64_t depth (0);
...
void vxlnetwork::node::ledger_pruning (uint64_t const batch_size_a, bool bootstrap_weight_reached_a, bool log_to_cout_a)
{
	uint64_t const max_depth (config.max_pruning_depth != 0 ? config.max_pruning_depth : std::numeric_limits<uint64_t>::max ());
	uint64_t const cutoff_time (bootstrap_weight_reached_a ? vxlnetwork::seconds_since_epoch () - config.max_pruning_age.count () : std::numeric_limits<uint64_t>::max ());
	uint64_t pruned_count (0);
	uint64_t transaction_write_count (0);
	vxlnetwork::account last_account (1); // 0 Burn account is never opened. So it can be used to break loop
	std::deque<vxlnetwork::block_hash> pruning_targets;
	...
	while ((transaction_write_count != 0 || !target_finished) && !stopped)
	{
		...
		{
			auto scoped_write_guard = write_database_queue.wait (vxlnetwork::writer::pruning);
...
void vxlnetwork::node::ongoing_ledger_pruning ()
{
	auto bootstrap_weight_reached (ledger.cache.block_count >= ledger.bootstrap_weight_max_blocks);
...
int vxlnetwork::node::price (vxlnetwork::uint128_t const & balance_a, int amount_a)
{
	debug_assert (balance_a >= amount_a * vxlnetwork::Gxrb_ratio);
	auto balance_l (balance_a);
	double result (0.0);
	for (auto i (0); i < amount_a; ++i)
	{
		balance_l -= vxlnetwork::Gxrb_ratio;
		auto balance_scaled ((balance_l / vxlnetwork::Mxrb_ratio).convert_to<double> ());
...
uint64_t vxlnetwork::node::default_difficulty (vxlnetwork::work_version const version_a) const
{
...
uint64_t vxlnetwork::node::default_receive_difficulty (vxlnetwork::work_version const version_a) const
{
	uint64_t result{ std::numeric_limits<uint64_t>::max () };
	switch (version_a)
	{
...
uint64_t vxlnetwork::node::max_work_generate_difficulty (vxlnetwork::work_version const version_a) const
{
	return vxlnetwork::difficulty::from_multiplier (config.max_work_generate_multiplier, default_difficulty (version_a));
}

bool vxlnetwork::node::local_work_generation_enabled () const
{
	return config.work_threads > 0 || work.opencl;
}

bool vxlnetwork::node::work_generation_enabled () const
{
	return work_generation_enabled (config.work_peers);
}

bool vxlnetwork::node::work_generation_enabled (std::vector<std::pair<std::string, uint16_t>> const & peers_a) const
{
	return !peers_a.empty () || local_work_generation_enabled ();
}

boost::optional<uint64_t> vxlnetwork::node::work_generate_blocking (vxlnetwork::block & block_a, uint64_t difficulty_a)
{
...
void vxlnetwork::node::work_generate (vxlnetwork::work_version const version_a, vxlnetwork::root const & root_a, uint64_t difficulty_a, std::function<void (boost::optional<uint64_t>)> callback_a, boost::optional<vxlnetwork::account> const & account_a, bool secondary_work_peers_a)
{
...
boost::optional<uint64_t> vxlnetwork::node::work_generate_blocking (vxlnetwork::work_version const version_a, vxlnetwork::root const & root_a, uint64_t difficulty_a, boost::optional<vxlnetwork::account> const & account_a)
{
...
boost::optional<uint64_t> vxlnetwork::node::work_generate_blocking (vxlnetwork::block & block_a)
{
	debug_assert (network_params.network.is_dev_network ());
	return work_generate_blocking (block_a, default_difficulty (vxlnetwork::work_version::work_1));
}

boost::optional<uint64_t> vxlnetwork::node::work_generate_blocking (vxlnetwork::root const & root_a)
{
	debug_assert (network_params.network.is_dev_network ());
	return work_generate_blocking (root_a, default_difficulty (vxlnetwork::work_version::work_1));
}

boost::optional<uint64_t> vxlnetwork::node::work_generate_blocking (vxlnetwork::root const & root_a, uint64_t difficulty_a)
{
	debug_assert (network_params.network.is_dev_network ());
	return work_generate_blocking (vxlnetwork::work_version::work_1, root_a, difficulty_a);
}

void vxlnetwork::node::add_initial_peers ()
{
	...
	{
		vxlnetwork::endpoint endpoint (boost::asio::ip::address_v6 (i->first.address_bytes ()), i->first.port ());
...
void vxlnetwork::node::block_confirm (std::shared_ptr<vxlnetwork::block> const & block_a)
{
...
bool vxlnetwork::node::block_confirmed (vxlnetwork::block_hash const & hash_a)
{
	auto transaction (store.tx_begin_read ());
	return ledger.block_confirmed (transaction, hash_a);
}

bool vxlnetwork::node::block_confirmed_or_being_confirmed (vxlnetwork::transaction const & transaction_a, vxlnetwork::block_hash const & hash_a)
{
	return confirmation_height_processor.is_processing_block (hash_a) || ledger.block_confirmed (transaction_a, hash_a);
}

void vxlnetwork::node::ongoing_online_weight_calculation_queue ()
{
	std::weak_ptr<vxlnetwork::node> node_w (shared_from_this ());
...
bool vxlnetwork::node::online () const
{
	return rep_crawler.total_weight () > online_reps.delta ();
}

void vxlnetwork::node::ongoing_online_weight_calculation ()
{
	online_reps.sample ();
	ongoing_online_weight_calculation_queue ();
}

void vxlnetwork::node::receive_confirmed (vxlnetwork::transaction const & block_transaction_a, vxlnetwork::block_hash const & hash_a, vxlnetwork::account const & destination_a)
{
	vxlnetwork::unique_lock<vxlnetwork::mutex> lk (wallets.mutex);
	...
	for ([[maybe_unused]] auto const & [id, wallet] : wallets_l)
	{
		if (wallet->store.exists (wallet_transaction, destination_a))
		{
			...
			auto error (store.pending.get (block_transaction_a, vxlnetwork::pending_key (destination_a, hash_a), pending));
			if (!error)
			{
				auto amount (pending.amount.number ());
				wallet->receive_async (hash_a, representative, amount, destination_a, [] (std::shared_ptr<vxlnetwork::block> const &) {});
			}
			else
			{
...
void vxlnetwork::node::process_confirmed_data (vxlnetwork::transaction const & transaction_a, std::shared_ptr<vxlnetwork::block> const & block_a, vxlnetwork::block_hash const & hash_a, vxlnetwork::account & account_a, vxlnetwork::uint128_t & amount_a, bool & is_state_send_a, bool & is_state_epoch_a, vxlnetwork::account & pending_account_a)
{
	...
	{
		amount_a = vxlnetwork::dev::constants.genesis_amount;
	}
	if (auto state = dynamic_cast<vxlnetwork::state_block *> (block_a.get ()))
	{
	...
	if (auto send = dynamic_cast<vxlnetwork::send_block *> (block_a.get ()))
	{
		pending_account_a = send->hashables.destination;
	}
}

void vxlnetwork::node::process_confirmed (vxlnetwork::election_status const & status_a, uint64_t iteration_a)
{
	...
	{
		iteration_a++;
		std::weak_ptr<vxlnetwork::node> node_w (shared ());
...
bool vxlnetwork::block_arrival::add (vxlnetwork::block_hash const & hash_a)
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> lock (mutex);
	auto now (std::chrono::steady_clock::now ());
	auto inserted (arrival.get<tag_sequence> ().emplace_back (vxlnetwork::block_arrival_info{ now, hash_a }));
	auto result (!inserted.second);
	return result;
}

bool vxlnetwork::block_arrival::recent (vxlnetwork::block_hash const & hash_a)
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> lock (mutex);
	auto now (std::chrono::steady_clock::now ());
...
std::unique_ptr<vxlnetwork::container_info_component> vxlnetwork::collect_container_info (block_arrival & block_arrival, std::string const & name)
{
	std::size_t count = 0;
	{
		vxlnetwork::lock_guard<vxlnetwork::mutex> guard (block_arrival.mutex);
...
std::shared_ptr<vxlnetwork::node> vxlnetwork::node::shared ()
{
	return shared_from_this ();
}

int vxlnetwork::node::store_version ()
{
	auto transaction (store.tx_begin_read ());
	return store.version.get (transaction);
}

bool vxlnetwork::node::init_error () const
{
	return store.init_error () || wallets_store.init_error ();
}

bool vxlnetwork::node::epoch_upgrader (vxlnetwork::raw_key const & prv_a, vxlnetwork::epoch epoch_a, uint64_t count_limit, uint64_t threads)
{
	...
	if (!error)
	{
		...
		if (!error)
		{
			*epoch_upgrade = std::async (std::launch::async, &vxlnetwork::node::epoch_upgrader_impl, this, prv_a, epoch_a, count_limit, threads);
		}
	}
	return error;
}

void vxlnetwork::node::set_bandwidth_params (std::size_t limit, double ratio)
{
...
void vxlnetwork::node::epoch_upgrader_impl (vxlnetwork::raw_key const & prv_a, vxlnetwork::epoch epoch_a, uint64_t count_limit, uint64_t threads)
{
	vxlnetwork::thread_role::set (vxlnetwork::thread_role::name::epoch_upgrader);
	auto upgrader_process = [] (vxlnetwork::node & node_a, std::atomic<uint64_t> & counter, std::shared_ptr<vxlnetwork::block> const & epoch, uint64_t difficulty, vxlnetwork::public_key const & signer_a, vxlnetwork::root const & root_a, vxlnetwork::account const & account_a) {
		epoch->block_work_set (node_a.work_generate_blocking (vxlnetwork::work_version::work_1, root_a, difficulty).value_or (0));
		bool valid_signature (!vxlnetwork::validate_message (signer_a, epoch->hash (), epoch->block_signature ()));
		bool valid_work (node_a.network_params.work.difficulty (*epoch) >= difficulty);
		vxlnetwork::process_result result (vxlnetwork::process_result::old);
		if (valid_signature && valid_work)
		{
			result = node_a.process_local (epoch).code;
		}
		if (result == vxlnetwork::process_result::progress)
		{
			++counter;
		}
		else
		{
			bool fork (result == vxlnetwork::process_result::fork);
	...
	vxlnetwork::raw_key raw_key;
	raw_key = prv_a;
	auto signer (vxlnetwork::pub_key (prv_a));
	debug_assert (signer == ledger.epoch_signer (link));

	vxlnetwork::mutex upgrader_mutex;
	vxlnetwork::condition_variable upgrader_condition;
	...
	boost::multi_index::indexed_by<
		...
		boost::multi_index::hashed_unique<boost::multi_index::tag<account_tag>,
			boost::multi_index::member<account_upgrade_item, vxlnetwork::account, &account_upgrade_item::account>>>>
	accounts_list;
	// clang-format on
	...
	while (!finished_upgrade && !stopped)
	{
		...
		{
			{
				...
				for (auto i (store.account.begin (transaction)), n (store.account.end ()); i != n && accounts_list.size () < count_limit; ++i)
				{
					...
					{
						release_assert (vxlnetwork::epochs::is_sequential (info.epoch (), epoch_a));
			...
			for (auto i (accounts_list.get<modified_tag> ().begin ()), n (accounts_list.get<modified_tag> ().end ()); i != n && attempts < upgrade_batch_size && attempts < count_limit && !stopped; ++i)
			{
				auto transaction (store.tx_begin_read ());
				vxlnetwork::account_info info;
				vxlnetwork::account const & account (i->account);
				if (!store.account.get (transaction, account, info) && info.epoch () < epoch_a)
				{
					++attempts;
					auto difficulty (network_params.work.threshold (vxlnetwork::work_version::work_1, vxlnetwork::block_details (epoch_a, false, false, true)));
					vxlnetwork::root const & root (info.head);
					std::shared_ptr<vxlnetwork::block> epoch = builder.state ()
														 .account (account)
														 .previous (info.head)
					...
					{
						{
							vxlnetwork::unique_lock<vxlnetwork::mutex> lock (upgrader_mutex);
							++workers;
						...
						this->workers.push_task ([node_l = shared_from_this (), &upgrader_process, &upgrader_mutex, &upgrader_condition, &upgraded_accounts, &workers, epoch, difficulty, signer, root, account] () {
							...
							{
								vxlnetwork::lock_guard<vxlnetwork::mutex> lock (upgrader_mutex);
								--workers;
							}
		...
		while (!finished_pending && count_limit != 0 && !stopped)
		{
			...
			uint64_t attempts (0);
			auto transaction (store.tx_begin_read ());
			for (auto i (store.pending.begin (transaction, vxlnetwork::pending_key (1, 0))), n (store.pending.end ()); i != n && attempts < upgrade_batch_size && attempts < count_limit && !stopped;)
			{
				...
				if (!store.account.exists (transaction, key.account))
				{
					...
					if (info.epoch < epoch_a)
					{
						...
						release_assert (vxlnetwork::epochs::is_sequential (info.epoch, epoch_a));
						auto difficulty (network_params.work.threshold (vxlnetwork::work_version::work_1, vxlnetwork::block_details (epoch_a, false, false, true)));
						vxlnetwork::root const & root (key.account);
						vxlnetwork::account const & account (key.account);
						std::shared_ptr<vxlnetwork::block> epoch = builder.state ()
															 .account (key.account)
															 .previous (0)
															 .representative (0)
				...
				if (to_next_account)
				{
					...
					if (key.account.number () == std::numeric_limits<vxlnetwork::uint256_t>::max ())
					{
						break;
					}
					else
					{
						i = store.pending.begin (transaction, vxlnetwork::pending_key (key.account.number () + 1, 0));
					}
				}
				else
				{
...
std::pair<uint64_t, decltype (vxlnetwork::ledger::bootstrap_weights)> vxlnetwork::node::get_bootstrap_weights () const
{
	std::unordered_map<vxlnetwork::account, vxlnetwork::uint128_t> weights;
	uint8_t const * weight_buffer = network_params.network.is_live_network () ? vxlnetwork_bootstrap_weights_live : vxlnetwork_bootstrap_weights_beta;
	std::size_t weight_size = network_params.network.is_live_network () ? vxlnetwork_bootstrap_weights_live_size : vxlnetwork_bootstrap_weights_beta_size;
	vxlnetwork::bufferstream weight_stream ((uint8_t const *)weight_buffer, weight_size);
	vxlnetwork::uint128_union block_height;
	uint64_t max_blocks = 0;
	if (!vxlnetwork::try_read (weight_stream, block_height))
	{
		max_blocks = vxlnetwork::narrow_cast<uint64_t> (block_height.number ());
		while (true)
		{
			vxlnetwork::account account;
			if (vxlnetwork::try_read (weight_stream, account.bytes))
			{
				break;
			}
			vxlnetwork::amount weight;
			if (vxlnetwork::try_read (weight_stream, weight.bytes))
			{
				break;
			}
...
void vxlnetwork::node::populate_backlog ()
{
	auto done = false;
	uint64_t const chunk_size = 65536;
	vxlnetwork::account next = 0;
	uint64_t total = 0;
	while (!stopped && !done)
	{
		auto transaction = store.tx_begin_read ();
...
uint64_t vxlnetwork::node::get_confirmation_height (vxlnetwork::transaction const & transaction_a, vxlnetwork::account & account_a)
{
...
vxlnetwork::node_wrapper::node_wrapper (boost::filesystem::path const & path_a, boost::filesystem::path const & config_path_a, vxlnetwork::node_flags const & node_flags_a) :
	network_params{ vxlnetwork::network_constants::active_network },
	...
	work{ network_params.network, 1 }
{
	...
	vxlnetwork::set_secure_perm_directory (path_a, error_chmod);
	vxlnetwork::daemon_config daemon_config{ path_a, network_params };
	auto error = vxlnetwork::read_node_config_toml (config_path_a, daemon_config, node_flags_a.config_overrides);
	if (error)
	{
	...
	node = std::make_shared<vxlnetwork::node> (*io_context, path_a, node_config, work, node_flags_a);
}
...
vxlnetwork::inactive_node::inactive_node (boost::filesystem::path const & path_a, boost::filesystem::path const & config_path_a, vxlnetwork::node_flags const & node_flags_a) :
...
vxlnetwork::inactive_node::inactive_node (boost::filesystem::path const & path_a, vxlnetwork::node_flags const & node_flags_a) :
	inactive_node (path_a, path_a, node_flags_a)
{
}

vxlnetwork::node_flags const & vxlnetwork::inactive_node_flag_defaults ()
{
	static vxlnetwork::node_flags node_flags;
...
std::unique_ptr<vxlnetwork::store> vxlnetwork::make_store (vxlnetwork::logger_mt & logger, boost::filesystem::path const & path, vxlnetwork::ledger_constants & constants, bool read_only, bool add_db_postfix, vxlnetwork::rocksdb_config const & rocksdb_config, vxlnetwork::txn_tracking_config const & txn_tracking_config_a, std::chrono::milliseconds block_processor_batch_max_time_a, vxlnetwork::lmdb_config const & lmdb_config_a, bool backup_before_upgrade)
{
	if (rocksdb_config.enable)
	{
		return std::make_unique<vxlnetwork::rocksdb_store> (logger, add_db_postfix ? path / "rocksdb" : path, constants, rocksdb_config, read_only);
	}

	return std::make_unique<vxlnetwork::mdb_store> (logger, add_db_postfix ? path / "data.ldb" : path, constants, txn_tracking_config_a, block_processor_batch_max_time_a, lmdb_config_a, backup_before_upgrade);
}
