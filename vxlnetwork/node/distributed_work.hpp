...
#include <vxlnetwork/boost/beast/http/string_body.hpp>
...
{
	vxlnetwork::work_version version;
	vxlnetwork::root root;
	uint64_t difficulty;
	boost::optional<vxlnetwork::account> const account;
 ...
 */
class distributed_work final : public std::enable_shared_from_this<vxlnetwork::distributed_work>
{
	...
	{
	public:
		peer_request (boost::asio::io_context & io_ctx_a, vxlnetwork::tcp_endpoint const & endpoint_a) :
			endpoint (endpoint_a),
			socket (io_ctx_a)
		{
		}
		std::shared_ptr<request_type> get_prepared_json_request (std::string const &) const;
		vxlnetwork::tcp_endpoint const endpoint;
...
public:
	distributed_work (vxlnetwork::node &, vxlnetwork::work_request const &, std::chrono::seconds const &);
	~distributed_work ();
	void start ();
	void cancel ();
...
private:
	...
	void do_request (vxlnetwork::tcp_endpoint const & endpoint_a);
	/** Send a work_cancel message using a new connection to \p endpoint_a */
	void do_cancel (vxlnetwork::tcp_endpoint const & endpoint_a);
	/** Called on a successful peer response, validates the reply */
	void success (std::string const &, vxlnetwork::tcp_endpoint const &);
	...
	void failure ();
	void handle_failure ();
	void add_bad_peer (vxlnetwork::tcp_endpoint const &);
	...
	std::weak_ptr<vxlnetwork::node> node_w;
	vxlnetwork::work_request request;
	...
	vxlnetwork::timer<std::chrono::milliseconds> elapsed; // logging only
