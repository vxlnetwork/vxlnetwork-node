...
#include <vxlnetwork/lib/rate_limiting.hpp>
...
private:
	vxlnetwork::rate::token_bucket bucket;
};

namespace transport
{
	vxlnetwork::endpoint map_endpoint_to_v6 (vxlnetwork::endpoint const &);
	vxlnetwork::endpoint map_tcp_to_endpoint (vxlnetwork::tcp_endpoint const &);
	vxlnetwork::tcp_endpoint map_endpoint_to_tcp (vxlnetwork::endpoint const &);
	...
	bool reserved_address (vxlnetwork::endpoint const &, bool = false);
	...
	{
	public:
		explicit channel (vxlnetwork::node &);
		virtual ~channel () = default;
		virtual std::size_t hash_code () const = 0;
		virtual bool operator== (vxlnetwork::transport::channel const &) const = 0;
		void send (vxlnetwork::message & message_a, std::function<void (boost::system::error_code const &, std::size_t)> const & callback_a = nullptr, vxlnetwork::buffer_drop_policy policy_a = vxlnetwork::buffer_drop_policy::limiter);
		// TODO: investigate clang-tidy warning about default parameters on virtual/override functions
		//
		virtual void send_buffer (vxlnetwork::shared_const_buffer const &, std::function<void (boost::system::error_code const &, std::size_t)> const & = nullptr, vxlnetwork::buffer_drop_policy = vxlnetwork::buffer_drop_policy::limiter) = 0;
		virtual std::string to_string () const = 0;
		virtual vxlnetwork::endpoint get_endpoint () const = 0;
		virtual vxlnetwork::tcp_endpoint get_tcp_endpoint () const = 0;
		virtual vxlnetwork::transport::transport_type get_type () const = 0;
		...
		boost::optional<vxlnetwork::account> get_node_id_optional () const
		{
			vxlnetwork::lock_guard<vxlnetwork::mutex> lk (channel_mutex);
			return node_id;
		}

		vxlnetwork::account get_node_id () const
		{
			vxlnetwork::lock_guard<vxlnetwork::mutex> lk (channel_mutex);
		...
		void set_node_id (vxlnetwork::account node_id_a)
		{
			vxlnetwork::lock_guard<vxlnetwork::mutex> lk (channel_mutex);
			node_id = node_id_a;
		}
		...
		mutable vxlnetwork::mutex channel_mutex;
	...
	private:
		...
		boost::optional<vxlnetwork::account> node_id{ boost::none };
	...
	class channel_loopback final : public vxlnetwork::transport::channel
	{
	public:
		explicit channel_loopback (vxlnetwork::node &);
		...
		bool operator== (vxlnetwork::transport::channel_loopback const & other_a) const
		{
		...
		{
			return vxlnetwork::transport::transport_type::loopback;
		}

	private:
		vxlnetwork::endpoint const endpoint;
	};
} // namespace transport
} // namespace vxlnetwork
...
{
template <>
struct hash<::vxlnetwork::transport::channel>
{
	std::size_t operator() (::vxlnetwork::transport::channel const & channel_a) const
	{
		return channel_a.hash_code ();
	}
};
template <>
struct equal_to<std::reference_wrapper<::vxlnetwork::transport::channel const>>
{
	bool operator() (std::reference_wrapper<::vxlnetwork::transport::channel const> const & lhs, std::reference_wrapper<::vxlnetwork::transport::channel const> const & rhs) const
	{
...
struct hash<::vxlnetwork::transport::channel>
{
	std::size_t operator() (::vxlnetwork::transport::channel const & channel_a) const
	{
		std::hash<::vxlnetwork::transport::channel> hash;
		return hash (channel_a);
	}
};
template <>
struct hash<std::reference_wrapper<::vxlnetwork::transport::channel const>>
{
	std::size_t operator() (std::reference_wrapper<::vxlnetwork::transport::channel const> const & channel_a) const
	{
		std::hash<::vxlnetwork::transport::channel> hash;
