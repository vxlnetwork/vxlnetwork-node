...
vxlnetwork::transport::channel_tcp::channel_tcp (vxlnetwork::node & node_a, std::weak_ptr<vxlnetwork::socket> socket_a) :
	channel (node_a),
	socket (std::move (socket_a))
{
}

vxlnetwork::transport::channel_tcp::~channel_tcp ()
{
...
std::size_t vxlnetwork::transport::channel_tcp::hash_code () const
{
	std::hash<::vxlnetwork::tcp_endpoint> hash;
	return hash (get_tcp_endpoint ());
}

bool vxlnetwork::transport::channel_tcp::operator== (vxlnetwork::transport::channel const & other_a) const
{
	bool result (false);
	auto other_l (dynamic_cast<vxlnetwork::transport::channel_tcp const *> (&other_a));
	if (other_l != nullptr)
	{
		return *this == *other_l;
	}
...
void vxlnetwork::transport::channel_tcp::send_buffer (vxlnetwork::shared_const_buffer const & buffer_a, std::function<void (boost::system::error_code const &, std::size_t)> const & callback_a, vxlnetwork::buffer_drop_policy policy_a)
{
	if (auto socket_l = socket.lock ())
	{
		if (!socket_l->max () || (policy_a == vxlnetwork::buffer_drop_policy::no_socket_drop && !socket_l->full ()))
		{
			socket_l->async_write (
			buffer_a, [endpoint_a = socket_l->remote_endpoint (), node = std::weak_ptr<vxlnetwork::node> (node.shared ()), callback_a] (boost::system::error_code const & ec, std::size_t size_a) {
		...
		{
			if (policy_a == vxlnetwork::buffer_drop_policy::no_socket_drop)
			{
				node.stats.inc (vxlnetwork::stat::type::tcp, vxlnetwork::stat::detail::tcp_write_no_socket_drop, vxlnetwork::stat::dir::out);
			}
			else
			{
				node.stats.inc (vxlnetwork::stat::type::tcp, vxlnetwork::stat::detail::tcp_write_drop, vxlnetwork::stat::dir::out);
			}
			if (callback_a)
			{
...
std::string vxlnetwork::transport::channel_tcp::to_string () const
{
	return boost::str (boost::format ("%1%") % get_tcp_endpoint ());
}

void vxlnetwork::transport::channel_tcp::set_endpoint ()
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> lk (channel_mutex);
	debug_assert (endpoint == vxlnetwork::tcp_endpoint (boost::asio::ip::address_v6::any (), 0)); // Not initialized endpoint value
...
vxlnetwork::transport::tcp_channels::tcp_channels (vxlnetwork::node & node, std::function<void (vxlnetwork::message const &, std::shared_ptr<vxlnetwork::transport::channel> const &)> sink) :
	node{ node },
	sink{ std::move (sink) }
{
}

bool vxlnetwork::transport::tcp_channels::insert (std::shared_ptr<vxlnetwork::transport::channel_tcp> const & channel_a, std::shared_ptr<vxlnetwork::socket> const & socket_a, std::shared_ptr<vxlnetwork::bootstrap_server> const & bootstrap_server_a)
{
	auto endpoint (channel_a->get_tcp_endpoint ());
	debug_assert (endpoint.address ().is_v6 ());
	auto udp_endpoint (vxlnetwork::transport::map_tcp_to_endpoint (endpoint));
	bool error (true);
...
void vxlnetwork::transport::tcp_channels::erase (vxlnetwork::tcp_endpoint const & endpoint_a)
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> lock (mutex);
	channels.get<endpoint_tag> ().erase (endpoint_a);
}

std::size_t vxlnetwork::transport::tcp_channels::size () const
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> lock (mutex);
	return channels.size ();
}

std::shared_ptr<vxlnetwork::transport::channel_tcp> vxlnetwork::transport::tcp_channels::find_channel (vxlnetwork::tcp_endpoint const & endpoint_a) const
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> lock (mutex);
...
std::unordered_set<std::shared_ptr<vxlnetwork::transport::channel>> vxlnetwork::transport::tcp_channels::random_set (std::size_t count_a, uint8_t min_version, bool include_temporary_channels_a) const
{
...
void vxlnetwork::transport::tcp_channels::random_fill (std::array<vxlnetwork::endpoint, 8> & target_a) const
{
...
bool vxlnetwork::transport::tcp_channels::store_all (bool clear_peers)
{
	...
	std::vector<vxlnetwork::endpoint> endpoints;
	{
		...
		std::back_inserter (endpoints), [] (auto const & channel) { return vxlnetwork::transport::map_tcp_to_endpoint (channel.endpoint ()); });
	}
	bool result (false);
	if (!endpoints.empty ())
	{
		...
		{
			node.store.peer.put (transaction, vxlnetwork::endpoint_key{ endpoint.address ().to_v6 ().to_bytes (), endpoint.port () });
		}
		result = true;
	}
	return result;
...
std::shared_ptr<vxlnetwork::transport::channel_tcp> vxlnetwork::transport::tcp_channels::find_node_id (vxlnetwork::account const & node_id_a)
{
	std::shared_ptr<vxlnetwork::transport::channel_tcp> result;
	vxlnetwork::lock_guard<vxlnetwork::mutex> lock (mutex);
...
vxlnetwork::tcp_endpoint vxlnetwork::transport::tcp_channels::bootstrap_peer (uint8_t connection_protocol_version_min)
{
...
void vxlnetwork::transport::tcp_channels::process_messages ()
{
	while (!stopped)
	{
...
void vxlnetwork::transport::tcp_channels::process_message (vxlnetwork::message const & message_a, vxlnetwork::tcp_endpoint const & endpoint_a, vxlnetwork::account const & node_id_a, std::shared_ptr<vxlnetwork::socket> const & socket_a)
{
	auto type_a = socket_a->type ();
	if (!stopped && message_a.header.version_using >= node.network_params.network.protocol_version_min)
	{
		auto channel (node.network.find_channel (vxlnetwork::transport::map_tcp_to_endpoint (endpoint_a)));
		if (channel)
		{
		...
		else
		{
			...
			else if (!node.network.excluded_peers.check (endpoint_a))
			{
				if (!node_id_a.is_zero ())
				{
					...
					auto temporary_channel (std::make_shared<vxlnetwork::transport::channel_tcp> (node, socket_a));
					...
					debug_assert (type_a == vxlnetwork::socket::type_t::realtime || type_a == vxlnetwork::socket::type_t::realtime_response_server);
					// Don't insert temporary channels for response_server
					if (type_a == vxlnetwork::socket::type_t::realtime)
					{
				...
				else
				{
					...
					debug_assert (message_a.header.type == vxlnetwork::message_type::node_id_handshake);
...
void vxlnetwork::transport::tcp_channels::start ()
{
	ongoing_keepalive ();
}

void vxlnetwork::transport::tcp_channels::stop ()
{
	stopped = true;
	vxlnetwork::unique_lock<vxlnetwork::mutex> lock (mutex);
...
bool vxlnetwork::transport::tcp_channels::max_ip_connections (vxlnetwork::tcp_endpoint const & endpoint_a)
{
	...
	if (result)
	{
		node.stats.inc (vxlnetwork::stat::type::tcp, vxlnetwork::stat::detail::tcp_max_per_ip, vxlnetwork::stat::dir::out);
	}
	return result;
}

bool vxlnetwork::transport::tcp_channels::max_subnetwork_connections (vxlnetwork::tcp_endpoint const & endpoint_a)
{
	...
	if (result)
	{
		node.stats.inc (vxlnetwork::stat::type::tcp, vxlnetwork::stat::detail::tcp_max_per_subnetwork, vxlnetwork::stat::dir::out);
	}
	return result;
}

bool vxlnetwork::transport::tcp_channels::max_ip_or_subnetwork_connections (vxlnetwork::tcp_endpoint const & endpoint_a)
{
	return max_ip_connections (endpoint_a) || max_subnetwork_connections (endpoint_a);
}

bool vxlnetwork::transport::tcp_channels::reachout (vxlnetwork::endpoint const & endpoint_a)
{
	auto tcp_endpoint (vxlnetwork::transport::map_endpoint_to_tcp (endpoint_a));
...
std::unique_ptr<vxlnetwork::container_info_component> vxlnetwork::transport::tcp_channels::collect_container_info (std::string const & name)
{
...
void vxlnetwork::transport::tcp_channels::purge (std::chrono::steady_clock::time_point const & cutoff_a)
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> lock (mutex);
...
void vxlnetwork::transport::tcp_channels::ongoing_keepalive ()
{
	vxlnetwork::keepalive message{ node.network_params.network };
	node.network.random_fill (message.peers);
	vxlnetwork::unique_lock<vxlnetwork::mutex> lock (mutex);
	// Wake up channels
	std::vector<std::shared_ptr<vxlnetwork::transport::channel_tcp>> send_list;
	...
	vxlnetwork::tcp_endpoint invalid_endpoint (boost::asio::ip::address_v6::any (), 0);
	if (!node.network_params.network.is_dev_network () && !node.flags.disable_udp)
	{
		...
		for (auto i (0); i <= random_count; ++i)
		{
			...
			{
				start_tcp (vxlnetwork::transport::map_tcp_to_endpoint (tcp_endpoint));
			}
		}
	}
...
void vxlnetwork::transport::tcp_channels::list (std::deque<std::shared_ptr<vxlnetwork::transport::channel>> & deque_a, uint8_t minimum_version_a, bool include_temporary_channels_a)
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> lock (mutex);
	// clang-format off
...
void vxlnetwork::transport::tcp_channels::modify (std::shared_ptr<vxlnetwork::transport::channel_tcp> const & channel_a, std::function<void (std::shared_ptr<vxlnetwork::transport::channel_tcp> const &)> modify_callback_a)
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> lock (mutex);
...
void vxlnetwork::transport::tcp_channels::update (vxlnetwork::tcp_endpoint const & endpoint_a)
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> lock (mutex);
	auto existing (channels.get<endpoint_tag> ().find (endpoint_a));
	if (existing != channels.get<endpoint_tag> ().end ())
...
void vxlnetwork::transport::tcp_channels::start_tcp (vxlnetwork::endpoint const & endpoint_a)
{
	...
	auto socket = std::make_shared<vxlnetwork::client_socket> (node);
	std::weak_ptr<vxlnetwork::socket> socket_w (socket);
	auto channel (std::make_shared<vxlnetwork::transport::channel_tcp> (node, socket_w));
	std::weak_ptr<vxlnetwork::node> node_w (node.shared ());
	socket->async_connect (vxlnetwork::transport::map_endpoint_to_tcp (endpoint_a),
	[node_w, channel, socket, endpoint_a] (boost::system::error_code const & ec) {
		if (auto node_l = node_w.lock ())
		{
			if (!ec && channel)
			{
				...
				vxlnetwork::node_id_handshake message (node_l->network_params.network, cookie, boost::none);
...
void vxlnetwork::transport::tcp_channels::start_tcp_receive_node_id (std::shared_ptr<vxlnetwork::transport::channel_tcp> const & channel_a, vxlnetwork::endpoint const & endpoint_a, std::shared_ptr<std::vector<uint8_t>> const & receive_buffer_a)
{
	std::weak_ptr<vxlnetwork::node> node_w (node.shared ());
	if (auto socket_l = channel_a->socket.lock ())
	{
		auto cleanup_node_id_handshake_socket = [socket_w = channel_a->socket, node_w] (vxlnetwork::endpoint const & endpoint_a) {
			if (auto node_l = node_w.lock ())
			{
		...
		auto cleanup_and_udp_fallback = [socket_w = channel_a->socket, node_w, cleanup_node_id_handshake_socket] (vxlnetwork::endpoint const & endpoint_a) {
			if (auto node_l = node_w.lock ())
			{
				node_l->network.tcp_channels.udp_fallback (endpoint_a);
		...
		socket_l->async_read (receive_buffer_a, 8 + sizeof (vxlnetwork::account) + sizeof (vxlnetwork::account) + sizeof (vxlnetwork::signature), [node_w, channel_a, endpoint_a, receive_buffer_a, cleanup_and_udp_fallback, cleanup_node_id_handshake_socket] (boost::system::error_code const & ec, std::size_t size_a) {
			if (auto node_l = node_w.lock ())
			{
				if (!ec && channel_a)
				{
					node_l->stats.inc (vxlnetwork::stat::type::message, vxlnetwork::stat::detail::node_id_handshake, vxlnetwork::stat::dir::in);
					auto error (false);
					vxlnetwork::bufferstream stream (receive_buffer_a->data (), size_a);
					vxlnetwork::message_header header (error, stream);
					if (!error && header.type == vxlnetwork::message_type::node_id_handshake)
					{
						if (header.version_using >= node_l->network_params.network.protocol_version_min)
						{
							vxlnetwork::node_id_handshake message (error, stream, header);
							if (!error && message.response && message.query)
							{
								...
								if (process)
								{
									...
									boost::optional<std::pair<vxlnetwork::account, vxlnetwork::signature>> response (std::make_pair (node_l->node_id.pub, vxlnetwork::sign_message (node_l->node_id.prv, node_l->node_id.pub, *message.query)));
									vxlnetwork::node_id_handshake response_message (node_l->network_params.network, boost::none, response);
									if (node_l->config.logging.network_node_id_handshake_logging ())
									{
										...
										if (auto node_l = node_w.lock ())
										{
											if (!ec && channel_a)
											{
												...
												if (auto socket_l = channel_a->socket.lock ())
												{
													...
													auto response_server = std::make_shared<vxlnetwork::bootstrap_server> (socket_l, node_l);
													node_l->network.tcp_channels.insert (channel_a, socket_l, response_server);
													// Listen for possible responses
													response_server->socket->type_set (vxlnetwork::socket::type_t::realtime_response_server);
													...
													{
														node_l->telemetry->get_metrics_single_peer_async (channel_a, [] (vxlnetwork::telemetry_data_response const &) {
						...
						else
						{
							...
							cleanup_node_id_handshake_socket (endpoint_a);
							{
								vxlnetwork::lock_guard<vxlnetwork::mutex> lock (node_l->network.tcp_channels.mutex);
								node_l->network.tcp_channels.attempts.get<endpoint_tag> ().erase (vxlnetwork::transport::map_endpoint_to_tcp (endpoint_a));
							}
						}
					}
					else
...
void vxlnetwork::transport::tcp_channels::udp_fallback (vxlnetwork::endpoint const & endpoint_a)
{
	{
		vxlnetwork::lock_guard<vxlnetwork::mutex> lock (mutex);
		attempts.get<endpoint_tag> ().erase (vxlnetwork::transport::map_endpoint_to_tcp (endpoint_a));
	}
