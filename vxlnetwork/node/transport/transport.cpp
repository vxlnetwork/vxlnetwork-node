...
namespace
{
class callback_visitor : public vxlnetwork::message_visitor
{
public:
	void keepalive (vxlnetwork::keepalive const & message_a) override
	{
		result = vxlnetwork::stat::detail::keepalive;
	}
	void publish (vxlnetwork::publish const & message_a) override
	{
		result = vxlnetwork::stat::detail::publish;
	}
	void confirm_req (vxlnetwork::confirm_req const & message_a) override
	{
		result = vxlnetwork::stat::detail::confirm_req;
	}
	void confirm_ack (vxlnetwork::confirm_ack const & message_a) override
	{
		result = vxlnetwork::stat::detail::confirm_ack;
	}
	void bulk_pull (vxlnetwork::bulk_pull const & message_a) override
	{
		result = vxlnetwork::stat::detail::bulk_pull;
	}
	void bulk_pull_account (vxlnetwork::bulk_pull_account const & message_a) override
	{
		result = vxlnetwork::stat::detail::bulk_pull_account;
	}
	void bulk_push (vxlnetwork::bulk_push const & message_a) override
	{
		result = vxlnetwork::stat::detail::bulk_push;
	}
	void frontier_req (vxlnetwork::frontier_req const & message_a) override
	{
		result = vxlnetwork::stat::detail::frontier_req;
	}
	void node_id_handshake (vxlnetwork::node_id_handshake const & message_a) override
	{
		result = vxlnetwork::stat::detail::node_id_handshake;
	}
	void telemetry_req (vxlnetwork::telemetry_req const & message_a) override
	{
		result = vxlnetwork::stat::detail::telemetry_req;
	}
	void telemetry_ack (vxlnetwork::telemetry_ack const & message_a) override
	{
		result = vxlnetwork::stat::detail::telemetry_ack;
	}
	vxlnetwork::stat::detail result;
};
}

vxlnetwork::endpoint vxlnetwork::transport::map_endpoint_to_v6 (vxlnetwork::endpoint const & endpoint_a)
{
	auto endpoint_l (endpoint_a);
	if (endpoint_l.address ().is_v4 ())
	{
		endpoint_l = vxlnetwork::endpoint (boost::asio::ip::address_v6::v4_mapped (endpoint_l.address ().to_v4 ()), endpoint_l.port ());
	}
	return endpoint_l;
}

vxlnetwork::endpoint vxlnetwork::transport::map_tcp_to_endpoint (vxlnetwork::tcp_endpoint const & endpoint_a)
{
	return vxlnetwork::endpoint (endpoint_a.address (), endpoint_a.port ());
}

vxlnetwork::tcp_endpoint vxlnetwork::transport::map_endpoint_to_tcp (vxlnetwork::endpoint const & endpoint_a)
{
	return vxlnetwork::tcp_endpoint (endpoint_a.address (), endpoint_a.port ());
}

boost::asio::ip::address vxlnetwork::transport::map_address_to_subnetwork (boost::asio::ip::address const & address_a)
{
...
boost::asio::ip::address vxlnetwork::transport::ipv4_address_or_ipv6_subnet (boost::asio::ip::address const & address_a)
{
	debug_assert (address_a.is_v6 ());
...
vxlnetwork::transport::channel::channel (vxlnetwork::node & node_a) :
	node (node_a)
{
	set_network_version (node_a.network_params.network.protocol_version);
}

void vxlnetwork::transport::channel::send (vxlnetwork::message & message_a, std::function<void (boost::system::error_code const &, std::size_t)> const & callback_a, vxlnetwork::buffer_drop_policy drop_policy_a)
{
	callback_visitor visitor;
	message_a.visit (visitor);
	auto buffer (message_a.to_shared_const_buffer ());
	auto detail (visitor.result);
	auto is_droppable_by_limiter = drop_policy_a == vxlnetwork::buffer_drop_policy::limiter;
	...
	if (!is_droppable_by_limiter || !should_drop)
	{
		...
		node.stats.inc (vxlnetwork::stat::type::message, detail, vxlnetwork::stat::dir::out);
	}
	else
	{
		if (callback_a)
		...
		node.stats.inc (vxlnetwork::stat::type::drop, detail, vxlnetwork::stat::dir::out);
...
vxlnetwork::transport::channel_loopback::channel_loopback (vxlnetwork::node & node_a) :
...
std::size_t vxlnetwork::transport::channel_loopback::hash_code () const
{
	std::hash<::vxlnetwork::endpoint> hash;
	return hash (endpoint);
}

bool vxlnetwork::transport::channel_loopback::operator== (vxlnetwork::transport::channel const & other_a) const
{
	return endpoint == other_a.get_endpoint ();
}

void vxlnetwork::transport::channel_loopback::send_buffer (vxlnetwork::shared_const_buffer const & buffer_a, std::function<void (boost::system::error_code const &, std::size_t)> const & callback_a, vxlnetwork::buffer_drop_policy drop_policy_a)
{
	release_assert (false && "sending to a loopback channel is not supported");
}

std::string vxlnetwork::transport::channel_loopback::to_string () const
{
	return boost::str (boost::format ("%1%") % endpoint);
}

boost::asio::ip::address_v6 vxlnetwork::transport::mapped_from_v4_bytes (unsigned long address_a)
{
	return boost::asio::ip::address_v6::v4_mapped (boost::asio::ip::address_v4 (address_a));
}

boost::asio::ip::address_v6 vxlnetwork::transport::mapped_from_v4_or_v6 (boost::asio::ip::address const & address_a)
{
	return address_a.is_v4 () ? boost::asio::ip::address_v6::v4_mapped (address_a.to_v4 ()) : address_a.to_v6 ();
}

bool vxlnetwork::transport::is_ipv4_or_v4_mapped_address (boost::asio::ip::address const & address_a)
{
	return address_a.is_v4 () || address_a.to_v6 ().is_v4_mapped ();
}

bool vxlnetwork::transport::reserved_address (vxlnetwork::endpoint const & endpoint_a, bool allow_local_peers)
{
...
vxlnetwork::bandwidth_limiter::bandwidth_limiter (double const limit_burst_ratio_a, std::size_t const limit_a) :
	bucket (static_cast<std::size_t> (limit_a * limit_burst_ratio_a), limit_a)
{
}

bool vxlnetwork::bandwidth_limiter::should_drop (std::size_t const & message_size_a)
{
	return !bucket.try_consume (vxlnetwork::narrow_cast<unsigned int> (message_size_a));
}

void vxlnetwork::bandwidth_limiter::reset (double const limit_burst_ratio_a, std::size_t const limit_a)
{
