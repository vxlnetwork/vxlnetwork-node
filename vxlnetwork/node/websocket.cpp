...
#include <vxlnetwork/boost/asio/dispatch.hpp>
...
vxlnetwork::websocket::confirmation_options::confirmation_options (vxlnetwork::wallets & wallets_a) :
	wallets (wallets_a)
{
}

vxlnetwork::websocket::confirmation_options::confirmation_options (boost::property_tree::ptree const & options_a, vxlnetwork::wallets & wallets_a, vxlnetwork::logger_mt & logger_a) :
	wallets (wallets_a),
	logger (logger_a)
{
...
bool vxlnetwork::websocket::confirmation_options::should_filter (vxlnetwork::websocket::message const & message_a) const
{
	...
	if (destination_opt_l)
	{
		...
		if (all_local_accounts)
		{
			...
			vxlnetwork::account source_l{};
			vxlnetwork::account destination_l{};
...
bool vxlnetwork::websocket::confirmation_options::update (boost::property_tree::ptree const & options_a)
{
...
void vxlnetwork::websocket::confirmation_options::check_filter_empty () const
{
...
vxlnetwork::websocket::vote_options::vote_options (boost::property_tree::ptree const & options_a, vxlnetwork::logger_mt & logger_a)
{
...
bool vxlnetwork::websocket::vote_options::should_filter (vxlnetwork::websocket::message const & message_a) const
{
...
vxlnetwork::websocket::session::session (vxlnetwork::websocket::listener & listener_a, socket_type socket_a, boost::asio::ssl::context & ctx_a) :
...
vxlnetwork::websocket::session::session (vxlnetwork::websocket::listener & listener_a, socket_type socket_a) :
	ws_listener (listener_a), ws (std::move (socket_a))
{
	ws_listener.get_logger ().try_log ("Websocket: session started");
}

vxlnetwork::websocket::session::~session ()
{
	{
		vxlnetwork::unique_lock<vxlnetwork::mutex> lk (subscriptions_mutex);
...
void vxlnetwork::websocket::session::handshake ()
{
...
void vxlnetwork::websocket::session::close ()
{
...
void vxlnetwork::websocket::session::write (vxlnetwork::websocket::message message_a)
{
	vxlnetwork::unique_lock<vxlnetwork::mutex> lk (subscriptions_mutex);
	auto subscription (subscriptions.find (message_a.topic));
	if (message_a.topic == vxlnetwork::websocket::topic::ack || (subscription != subscriptions.end () && !subscription->second->should_filter (message_a)))
	{
		lk.unlock ();
		auto this_l (shared_from_this ());
...
void vxlnetwork::websocket::session::write_queued_messages ()
{
	auto msg (send_queue.front ().to_string ());
	auto this_l (shared_from_this ());

	ws.async_write (vxlnetwork::shared_const_buffer (msg),
...
void vxlnetwork::websocket::session::read ()
{
	auto this_l (shared_from_this ());
...
namespace
{
vxlnetwork::websocket::topic to_topic (std::string const & topic_a)
{
	vxlnetwork::websocket::topic topic = vxlnetwork::websocket::topic::invalid;
	if (topic_a == "confirmation")
	{
		topic = vxlnetwork::websocket::topic::confirmation;
	}
	else if (topic_a == "stopped_election")
	{
		topic = vxlnetwork::websocket::topic::stopped_election;
	}
	else if (topic_a == "vote")
	{
		topic = vxlnetwork::websocket::topic::vote;
	}
	else if (topic_a == "ack")
	{
		topic = vxlnetwork::websocket::topic::ack;
	}
	else if (topic_a == "work")
	{
		topic = vxlnetwork::websocket::topic::work;
	}
	else if (topic_a == "bootstrap")
	{
		topic = vxlnetwork::websocket::topic::bootstrap;
	}
	else if (topic_a == "telemetry")
	{
		topic = vxlnetwork::websocket::topic::telemetry;
	}
	else if (topic_a == "new_unconfirmed_block")
	{
		topic = vxlnetwork::websocket::topic::new_unconfirmed_block;
	}

	return topic;
}

std::string from_topic (vxlnetwork::websocket::topic topic_a)
{
	std::string topic = "invalid";
	if (topic_a == vxlnetwork::websocket::topic::confirmation)
	{
		topic = "confirmation";
	}
	else if (topic_a == vxlnetwork::websocket::topic::stopped_election)
	{
		topic = "stopped_election";
	}
	else if (topic_a == vxlnetwork::websocket::topic::vote)
	{
		topic = "vote";
	}
	else if (topic_a == vxlnetwork::websocket::topic::ack)
	{
		topic = "ack";
	}
	else if (topic_a == vxlnetwork::websocket::topic::work)
	{
		topic = "work";
	}
	else if (topic_a == vxlnetwork::websocket::topic::bootstrap)
	{
		topic = "bootstrap";
	}
	else if (topic_a == vxlnetwork::websocket::topic::telemetry)
	{
		topic = "telemetry";
	}
	else if (topic_a == vxlnetwork::websocket::topic::new_unconfirmed_block)
	{
...
void vxlnetwork::websocket::session::send_ack (std::string action_a, std::string id_a)
{
	vxlnetwork::websocket::message msg (vxlnetwork::websocket::topic::ack);
	boost::property_tree::ptree & message_l = msg.contents;
	message_l.add ("ack", action_a);
	message_l.add ("time", std::to_string (vxlnetwork::milliseconds_since_epoch ()));
	if (!id_a.empty ())
	{
...
void vxlnetwork::websocket::session::handle_message (boost::property_tree::ptree const & message_a)
{
	...
	if (action == "subscribe" && topic_l != vxlnetwork::websocket::topic::invalid)
	{
		auto options_text_l (message_a.get_child_optional ("options"));
		vxlnetwork::lock_guard<vxlnetwork::mutex> lk (subscriptions_mutex);
		std::unique_ptr<vxlnetwork::websocket::options> options_l{ nullptr };
		if (options_text_l && topic_l == vxlnetwork::websocket::topic::confirmation)
		{
			options_l = std::make_unique<vxlnetwork::websocket::confirmation_options> (options_text_l.get (), ws_listener.get_wallets (), ws_listener.get_logger ());
		}
		else if (options_text_l && topic_l == vxlnetwork::websocket::topic::vote)
		{
			options_l = std::make_unique<vxlnetwork::websocket::vote_options> (options_text_l.get (), ws_listener.get_logger ());
		}
		else
		{
			options_l = std::make_unique<vxlnetwork::websocket::options> ();
		}
	...
	else if (action == "unsubscribe" && topic_l != vxlnetwork::websocket::topic::invalid)
	{
		vxlnetwork::lock_guard<vxlnetwork::mutex> lk (subscriptions_mutex);
...
void vxlnetwork::websocket::listener::stop ()
{
	stopped = true;
	acceptor.close ();

	vxlnetwork::lock_guard<vxlnetwork::mutex> lk (sessions_mutex);
...
vxlnetwork::websocket::listener::listener (std::shared_ptr<vxlnetwork::tls_config> const & tls_config_a, vxlnetwork::logger_mt & logger_a, vxlnetwork::wallets & wallets_a, boost::asio::io_context & io_ctx_a, boost::asio::ip::tcp::endpoint endpoint_a) :
...
void vxlnetwork::websocket::listener::run ()
{
	if (acceptor.is_open ())
	{
		accept ();
...
void vxlnetwork::websocket::listener::accept ()
{
	auto this_l (shared_from_this ());
...
void vxlnetwork::websocket::listener::on_accept (boost::system::error_code ec)
{
	if (ec)
	{
	...
	else
	{
		...
		std::shared_ptr<vxlnetwork::websocket::session> session;
		if (tls_config && tls_config->enable_wss)
		{
#ifdef VXLNETWORK_SECURE_RPC
			session = std::make_shared<vxlnetwork::websocket::session> (*this, std::move (socket), tls_config->ssl_context);
#endif
		}
		else
		{
			session = std::make_shared<vxlnetwork::websocket::session> (*this, std::move (socket));
		}
...
void vxlnetwork::websocket::listener::broadcast_confirmation (std::shared_ptr<vxlnetwork::block> const & block_a, vxlnetwork::account const & account_a, vxlnetwork::amount const & amount_a, std::string const & subtype, vxlnetwork::election_status const & election_status_a, std::vector<vxlnetwork::vote_with_weight_info> const & election_votes_a)
{
	vxlnetwork::websocket::message_builder builder;

	vxlnetwork::lock_guard<vxlnetwork::mutex> lk (sessions_mutex);
	boost::optional<vxlnetwork::websocket::message> msg_with_block;
	boost::optional<vxlnetwork::websocket::message> msg_without_block;
	for (auto & weak_session : sessions)
	{
		auto session_ptr (weak_session.lock ());
		if (session_ptr)
		{
			auto subscription (session_ptr->subscriptions.find (vxlnetwork::websocket::topic::confirmation));
			if (subscription != session_ptr->subscriptions.end ())
			{
				vxlnetwork::websocket::confirmation_options default_options (wallets);
				auto conf_options (dynamic_cast<vxlnetwork::websocket::confirmation_options *> (subscription->second.get ()));
...
void vxlnetwork::websocket::listener::broadcast (vxlnetwork::websocket::message message_a)
{
	vxlnetwork::lock_guard<vxlnetwork::mutex> lk (sessions_mutex);
	for (auto & weak_session : sessions)
	{
...
void vxlnetwork::websocket::listener::increase_subscriber_count (vxlnetwork::websocket::topic const & topic_a)
{
	topic_subscriber_count[static_cast<std::size_t> (topic_a)] += 1;
}

void vxlnetwork::websocket::listener::decrease_subscriber_count (vxlnetwork::websocket::topic const & topic_a)
{
...
vxlnetwork::websocket::message vxlnetwork::websocket::message_builder::stopped_election (vxlnetwork::block_hash const & hash_a)
{
	vxlnetwork::websocket::message message_l (vxlnetwork::websocket::topic::stopped_election);
...
vxlnetwork::websocket::message vxlnetwork::websocket::message_builder::block_confirmed (std::shared_ptr<vxlnetwork::block> const & block_a, vxlnetwork::account const & account_a, vxlnetwork::amount const & amount_a, std::string subtype, bool include_block_a, vxlnetwork::election_status const & election_status_a, std::vector<vxlnetwork::vote_with_weight_info> const & election_votes_a, vxlnetwork::websocket::confirmation_options const & options_a)
{
	vxlnetwork::websocket::message message_l (vxlnetwork::websocket::topic::confirmation);
	set_common_fields (message_l);
...
vxlnetwork::websocket::message vxlnetwork::websocket::message_builder::vote_received (std::shared_ptr<vxlnetwork::vote> const & vote_a, vxlnetwork::vote_code code_a)
{
	vxlnetwork::websocket::message message_l (vxlnetwork::websocket::topic::vote);
	set_common_fields (message_l);
	...
	switch (code_a)
	{
		case vxlnetwork::vote_code::vote:
			vote_type = "vote";
			break;
		case vxlnetwork::vote_code::replay:
			vote_type = "replay";
			break;
		case vxlnetwork::vote_code::indeterminate:
			vote_type = "indeterminate";
			break;
		case vxlnetwork::vote_code::invalid:
			debug_assert (false);
			break;
	}
...
vxlnetwork::websocket::message vxlnetwork::websocket::message_builder::work_generation (vxlnetwork::work_version const version_a, vxlnetwork::block_hash const & root_a, uint64_t work_a, uint64_t difficulty_a, uint64_t publish_threshold_a, std::chrono::milliseconds const & duration_a, std::string const & peer_a, std::vector<std::string> const & bad_peers_a, bool completed_a, bool cancelled_a)
{
	vxlnetwork::websocket::message message_l (vxlnetwork::websocket::topic::work);
	set_common_fields (message_l);
	...
	request_l.put ("version", vxlnetwork::to_string (version_a));
	request_l.put ("hash", root_a.to_string ());
	request_l.put ("difficulty", vxlnetwork::to_string_hex (difficulty_a));
	auto request_multiplier_l (vxlnetwork::difficulty::to_multiplier (difficulty_a, publish_threshold_a));
	request_l.put ("multiplier", vxlnetwork::to_string (request_multiplier_l));
	...
	if (completed_a)
	{
		...
		result_l.put ("work", vxlnetwork::to_string_hex (work_a));
		auto result_difficulty_l (vxlnetwork::dev::network_params.work.difficulty (version_a, root_a, work_a));
		result_l.put ("difficulty", vxlnetwork::to_string_hex (result_difficulty_l));
		auto result_multiplier_l (vxlnetwork::difficulty::to_multiplier (result_difficulty_l, publish_threshold_a));
		result_l.put ("multiplier", vxlnetwork::to_string (result_multiplier_l));
...
vxlnetwork::websocket::message vxlnetwork::websocket::message_builder::work_cancelled (vxlnetwork::work_version const version_a, vxlnetwork::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a)
{
	return work_generation (version_a, root_a, 0, difficulty_a, publish_threshold_a, duration_a, "", bad_peers_a, false, true);
}

vxlnetwork::websocket::message vxlnetwork::websocket::message_builder::work_failed (vxlnetwork::work_version const version_a, vxlnetwork::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a)
{
	return work_generation (version_a, root_a, 0, difficulty_a, publish_threshold_a, duration_a, "", bad_peers_a, false, false);
}

vxlnetwork::websocket::message vxlnetwork::websocket::message_builder::bootstrap_started (std::string const & id_a, std::string const & mode_a)
{
	vxlnetwork::websocket::message message_l (vxlnetwork::websocket::topic::bootstrap);
	set_common_fields (message_l);
...
vxlnetwork::websocket::message vxlnetwork::websocket::message_builder::bootstrap_exited (std::string const & id_a, std::string const & mode_a, std::chrono::steady_clock::time_point const start_time_a, uint64_t const total_blocks_a)
{
	vxlnetwork::websocket::message message_l (vxlnetwork::websocket::topic::bootstrap);
	set_common_fields (message_l);
...
vxlnetwork::websocket::message vxlnetwork::websocket::message_builder::telemetry_received (vxlnetwork::telemetry_data const & telemetry_data_a, vxlnetwork::endpoint const & endpoint_a)
{
	vxlnetwork::websocket::message message_l (vxlnetwork::websocket::topic::telemetry);
	set_common_fields (message_l);

	// Telemetry information
	vxlnetwork::jsonconfig telemetry_l;
...
vxlnetwork::websocket::message vxlnetwork::websocket::message_builder::new_block_arrived (vxlnetwork::block const & block_a)
{
	vxlnetwork::websocket::message message_l (vxlnetwork::websocket::topic::new_unconfirmed_block);
	set_common_fields (message_l);

	boost::property_tree::ptree block_l;
	block_a.serialize_json (block_l);
	auto subtype (vxlnetwork::state_subtype (block_a.sideband ().details));
...
void vxlnetwork::websocket::message_builder::set_common_fields (vxlnetwork::websocket::message & message_a)
{
	// Common message information
	message_a.contents.add ("topic", from_topic (message_a.topic));
	message_a.contents.add ("time", std::to_string (vxlnetwork::milliseconds_since_epoch ()));
}

std::string vxlnetwork::websocket::message::to_string () const
{
