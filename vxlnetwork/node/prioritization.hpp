...
class prioritization final
{
	...
	std::vector<vxlnetwork::uint128_t> minimums;
...
public:
	...
	void push (uint64_t time, std::shared_ptr<vxlnetwork::block> block);
	std::shared_ptr<vxlnetwork::block> top () const;
	void pop ();
	...
	std::unique_ptr<vxlnetwork::container_info_component> collect_container_info (std::string const &);
};
}
