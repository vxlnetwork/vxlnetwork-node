...
TEST (rate, basic)
{
	vxlnetwork::rate::token_bucket bucket (10, 10);
...
TEST (rate, network)
{
	...
	vxlnetwork::rate::token_bucket bucket (10, 5);
...
TEST (rate, reset)
{
	vxlnetwork::rate::token_bucket bucket (0, 0);
...
TEST (optional_ptr, basic)
{
	...
	vxlnetwork::optional_ptr<valtype> opt;
	ASSERT_FALSE (opt);
...
TEST (thread, thread_pool)
{
	...
	vxlnetwork::thread_pool workers (1u, vxlnetwork::thread_role::name::unknown);
...
TEST (thread_pool_alarm, one)
{
	...
	vxlnetwork::unique_lock<vxlnetwork::mutex> unique (mutex);
	condition.wait (unique, [&] () { return !!done; });
}

TEST (thread_pool_alarm, many)
{
	vxlnetwork::thread_pool workers (50u, vxlnetwork::thread_role::name::unknown);
...
TEST (filesystem, remove_all_files)
{
	...
	vxlnetwork::remove_all_files_in_dir (path);
...
TEST (filesystem, move_all_files)
{
	...
	vxlnetwork::move_all_files_to_dir (dummy_directory, path);
...
{
	vxlnetwork::relaxed_atomic_integral<uint32_t> atomic{ 0 };
	ASSERT_EQ (0, atomic++);
	ASSERT_EQ (1, atomic);
	ASSERT_EQ (2, ++atomic);
	ASSERT_EQ (2, atomic);
