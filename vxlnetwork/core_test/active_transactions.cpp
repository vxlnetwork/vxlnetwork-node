...
TEST (active_transactions, confirm_election_by_request)
{
	...
	vxlnetwork::node_flags node_flags2{};
	node_flags2.disable_rep_crawler = true;
	auto & node2 = *system.add_node (node_flags2);

	auto send1 = vxlnetwork::state_block_builder{}.make_block ().account (vxlnetwork::dev::genesis_key.pub).representative (vxlnetwork::dev::genesis_key.pub).previous (vxlnetwork::dev::genesis->hash ()).link (vxlnetwork::public_key ()).balance (vxlnetwork::dev::constants.genesis_amount - 100).sign (vxlnetwork::dev::genesis_key.prv, vxlnetwork::dev::genesis_key.pub).work (*system.work.generate (vxlnetwork::dev::genesis->hash ())).build_shared ();
	...
	{
		vxlnetwork::lock_guard<vxlnetwork::mutex> guard (node2.rep_crawler.probable_reps_mutex);
		node2.rep_crawler.probable_reps.emplace (vxlnetwork::dev::genesis_key.pub, vxlnetwork::dev::constants.genesis_amount, *peers.cbegin ());
	}
...
TEST (active_transactions, confirm_frontier)
{
	...
	{
		vxlnetwork::lock_guard<vxlnetwork::mutex> guard (node2.rep_crawler.probable_reps_mutex);
		node2.rep_crawler.probable_reps.emplace (vxlnetwork::dev::genesis_key.pub, vxlnetwork::dev::constants.genesis_amount, *peers.begin ());
	}
	...
	ASSERT_EQ (vxlnetwork::process_result::progress, node2.process (*send_copy).code);
...
TEST (active_transactions, keep_local)
{
	...
	auto const send1 = wallet.send_action (vxlnetwork::dev::genesis_key.pub, key1.pub, node.config.receive_minimum.number ());
	auto const send2 = wallet.send_action (vxlnetwork::dev::genesis_key.pub, key2.pub, node.config.receive_minimum.number ());
	auto const send3 = wallet.send_action (vxlnetwork::dev::genesis_key.pub, key3.pub, node.config.receive_minimum.number ());
	auto const send4 = wallet.send_action (vxlnetwork::dev::genesis_key.pub, key4.pub, node.config.receive_minimum.number ());
	auto const send5 = wallet.send_action (vxlnetwork::dev::genesis_key.pub, key5.pub, node.config.receive_minimum.number ());
	auto const send6 = wallet.send_action (vxlnetwork::dev::genesis_key.pub, key6.pub, node.config.receive_minimum.number ());
	...
	for (auto const & block : { send1, send2, send3, send4, send5, send6 })
	{
		...
		node.process_confirmed (vxlnetwork::election_status{ block });
...
TEST (active_transactions, inactive_votes_cache)
{
	...
	auto send = vxlnetwork::send_block_builder ()
				...
				.build_shared ();
	auto vote (std::make_shared<vxlnetwork::vote> (vxlnetwork::dev::genesis_key.pub, vxlnetwork::dev::genesis_key.prv, vxlnetwork::vote::timestamp_max, vxlnetwork::vote::duration_max, std::vector<vxlnetwork::block_hash> (1, send->hash ())));
	...
	ASSERT_EQ (1, node.stats.count (vxlnetwork::stat::type::election, vxlnetwork::stat::detail::vote_cached));
}
...
TEST (active_transactions, inactive_votes_cache_non_final)
{
	...
	auto send = vxlnetwork::send_block_builder ()
				...
				.work (*system.work.generate (latest))
				.build_shared ();
	auto vote (std::make_shared<vxlnetwork::vote> (vxlnetwork::dev::genesis_key.pub, vxlnetwork::dev::genesis_key.prv, 0, 0, std::vector<vxlnetwork::block_hash> (1, send->hash ()))); // Non-final vote
	node.vote_processor.vote (vote, std::make_shared<vxlnetwork::transport::channel_loopback> (node));
	ASSERT_TIMELY (5s, node.active.inactive_votes_cache_size () == 1);
	node.process_active (send);
	node.block_processor.flush ();
	ASSERT_TIMELY (5s, node.stats.count (vxlnetwork::stat::type::election, vxlnetwork::stat::detail::vote_cached) == 1);
	auto election = node.active.election (send->qualified_root ());
	ASSERT_NE (nullptr, election);
	ASSERT_FALSE (election->confirmed ());
	ASSERT_EQ (vxlnetwork::dev::constants.genesis_amount - 100, election->tally ().begin ()->first);
}
...
TEST (active_transactions, inactive_votes_cache_fork)
{
	...
	auto const vote = std::make_shared<vxlnetwork::vote> (vxlnetwork::dev::genesis_key.pub, vxlnetwork::dev::genesis_key.prv, vxlnetwork::vote::timestamp_max, vxlnetwork::vote::duration_max, std::vector<vxlnetwork::block_hash> (1, send1->hash ()));
	node.vote_processor.vote (vote, std::make_shared<vxlnetwork::transport::channel_loopback> (node));
	ASSERT_TIMELY (5s, node.active.inactive_votes_cache_size () == 1);
...
TEST (active_transactions, inactive_votes_cache_existing_vote)
{
	...
	auto send = builder.send ()
				.previous (latest)
				.destination (key.pub)
				.balance (vxlnetwork::dev::constants.genesis_amount - 100 * vxlnetwork::Gxrb_ratio)
				.sign (vxlnetwork::dev::genesis_key.prv, vxlnetwork::dev::genesis_key.pub)
				.work (*system.work.generate (latest))
				.build_shared ();
	auto open = builder.state ()
				...
				.balance (100 * vxlnetwork::Gxrb_ratio)
				.link (send->hash ())
	...
	// Insert vote
	auto vote1 (std::make_shared<vxlnetwork::vote> (key.pub, key.prv, vxlnetwork::vote::timestamp_min * 1, 0, std::vector<vxlnetwork::block_hash> (1, send->hash ())));
	...
	ASSERT_EQ (1, node.stats.count (vxlnetwork::stat::type::election, vxlnetwork::stat::detail::vote_new));
	auto last_vote1 (election->votes ()[key.pub]);
	ASSERT_EQ (send->hash (), last_vote1.hash);
	ASSERT_EQ (vxlnetwork::vote::timestamp_min * 1, last_vote1.timestamp);
	// Attempt to change vote with inactive_votes_cache
	vxlnetwork::unique_lock<vxlnetwork::mutex> active_lock (node.active.mutex);
	...
	ASSERT_EQ (0, node.stats.count (vxlnetwork::stat::type::election, vxlnetwork::stat::detail::vote_cached));
}

// Test disabled because it's failing intermittently.
// PR in which it got disabled: https://github.com/vxlnetworkcurrency/vxlnetwork-node/pull/3629
// Issue for investigating it: https://github.com/vxlnetworkcurrency/vxlnetwork-node/issues/3632
TEST (active_transactions, DISABLED_inactive_votes_cache_multiple_votes)
{
	...
	node.block_processor.flush ();
	// Process votes
	auto vote1 (std::make_shared<vxlnetwork::vote> (key1.pub, key1.prv, 0, 0, std::vector<vxlnetwork::block_hash> (1, send1->hash ())));
	node.vote_processor.vote (vote1, std::make_shared<vxlnetwork::transport::channel_loopback> (node));
	auto vote2 (std::make_shared<vxlnetwork::vote> (vxlnetwork::dev::genesis_key.pub, vxlnetwork::dev::genesis_key.prv, 0, 0, std::vector<vxlnetwork::block_hash> (1, send1->hash ())));
	node.vote_processor.vote (vote2, std::make_shared<vxlnetwork::transport::channel_loopback> (node));
	...
	ASSERT_EQ (2, node.stats.count (vxlnetwork::stat::type::election, vxlnetwork::stat::detail::vote_cached));
}
...
TEST (active_transactions, inactive_votes_cache_election_start)
{
	...
	vxlnetwork::send_block_builder send_block_builder;
	vxlnetwork::state_block_builder state_block_builder;
	auto send1 = send_block_builder.make_block ()
				 .previous (latest)
				 .destination (key1.pub)
				 .balance (vxlnetwork::dev::constants.genesis_amount - 5000 * vxlnetwork::Gxrb_ratio)
				 .sign (vxlnetwork::dev::genesis_key.prv, vxlnetwork::dev::genesis_key.pub)
				 .work (*system.work.generate (latest))
				 .build_shared ();
	auto send2 = send_block_builder.make_block ()
				 ...
				 .balance (vxlnetwork::dev::constants.genesis_amount - 10000 * vxlnetwork::Gxrb_ratio)
				 .sign (vxlnetwork::dev::genesis_key.prv, vxlnetwork::dev::genesis_key.pub)
				 .work (*system.work.generate (send1->hash ()))
				 .build_shared ();
	auto open1 = state_block_builder.make_block ()
				 ...
				 .previous (0)
				 .representative (key1.pub)
				 .balance (5000 * vxlnetwork::Gxrb_ratio)
				 .link (send1->hash ())
				 .sign (key1.prv, key1.pub)
				 .work (*system.work.generate (key1.pub))
				 .build_shared ();
	...
	auto send3 = send_block_builder.make_block ()
				 .previous (send2->hash ())
				 .destination (vxlnetwork::keypair ().pub)
	...
	auto send4 = send_block_builder.make_block ()
				 ...
				 .build_shared ();
	// Inactive votes
	std::vector<vxlnetwork::block_hash> hashes{ open1->hash (), open2->hash (), send4->hash () };
	auto vote1 (std::make_shared<vxlnetwork::vote> (key1.pub, key1.prv, 0, 0, hashes));
	node.vote_processor.vote (vote1, std::make_shared<vxlnetwork::transport::channel_loopback> (node));
	ASSERT_TIMELY (5s, node.active.inactive_votes_cache_size () == 3);
	ASSERT_TRUE (node.active.empty ());
	ASSERT_EQ (1, node.ledger.cache.cemented_count);
	// 2 votes are required to start election (dev network)
	auto vote2 (std::make_shared<vxlnetwork::vote> (key2.pub, key2.prv, 0, 0, hashes));
	node.vote_processor.vote (vote2, std::make_shared<vxlnetwork::transport::channel_loopback> (node));
	...
	auto vote0 (std::make_shared<vxlnetwork::vote> (vxlnetwork::dev::genesis_key.pub, vxlnetwork::dev::genesis_key.prv, vxlnetwork::vote::timestamp_max, vxlnetwork::vote::duration_max, hashes)); // Final vote for confirmation
	node.vote_processor.vote (vote0, std::make_shared<vxlnetwork::transport::channel_loopback> (node));
...
TEST (active_transactions, vote_replays)
{
	...
	vxlnetwork::blocks_confirm (node, { send1, open1 });
	ASSERT_EQ (2, node.active.size ());
	// First vote is not a replay and confirms the election, second vote should be a replay since the election has confirmed but not yet removed
	auto vote_send1 (std::make_shared<vxlnetwork::vote> (vxlnetwork::dev::genesis_key.pub, vxlnetwork::dev::genesis_key.prv, vxlnetwork::vote::timestamp_max, vxlnetwork::vote::duration_max, send1));
	ASSERT_EQ (vxlnetwork::vote_code::vote, node.active.vote (vote_send1));
	ASSERT_EQ (2, node.active.size ());
	ASSERT_EQ (vxlnetwork::vote_code::replay, node.active.vote (vote_send1));
	...
	ASSERT_EQ (vxlnetwork::vote_code::replay, node.active.vote (vote_send1));
	// Open new account
	auto vote_open1 (std::make_shared<vxlnetwork::vote> (vxlnetwork::dev::genesis_key.pub, vxlnetwork::dev::genesis_key.prv, vxlnetwork::vote::timestamp_max, vxlnetwork::vote::duration_max, open1));
	ASSERT_EQ (vxlnetwork::vote_code::vote, node.active.vote (vote_open1));
	ASSERT_EQ (vxlnetwork::vote_code::replay, node.active.vote (vote_open1));
	ASSERT_TIMELY (3s, node.active.empty ());
	ASSERT_EQ (vxlnetwork::vote_code::replay, node.active.vote (vote_open1));
	ASSERT_EQ (vxlnetwork::Gxrb_ratio, node.ledger.weight (key.pub));

	auto send2 = builder.make_block ()
				 .account (key.pub)
				 .previous (open1->hash ())
				 .representative (key.pub)
				 .balance (vxlnetwork::Gxrb_ratio - 1)
				 .link (key.pub)
				 .sign (key.prv, key.pub)
	...
	node.process_active (send2);
	vxlnetwork::blocks_confirm (node, { send2 });
	ASSERT_EQ (1, node.active.size ());
	auto vote1_send2 (std::make_shared<vxlnetwork::vote> (vxlnetwork::dev::genesis_key.pub, vxlnetwork::dev::genesis_key.prv, vxlnetwork::vote::timestamp_max, vxlnetwork::vote::duration_max, send2));
	auto vote2_send2 (std::make_shared<vxlnetwork::vote> (key.pub, key.prv, 0, 0, send2));
	ASSERT_EQ (vxlnetwork::vote_code::vote, node.active.vote (vote2_send2));
	ASSERT_EQ (1, node.active.size ());
	ASSERT_EQ (vxlnetwork::vote_code::replay, node.active.vote (vote2_send2));
	ASSERT_EQ (1, node.active.size ());
	ASSERT_EQ (vxlnetwork::vote_code::vote, node.active.vote (vote1_send2));
	ASSERT_EQ (1, node.active.size ());
	ASSERT_EQ (vxlnetwork::vote_code::replay, node.active.vote (vote1_send2));
	ASSERT_TIMELY (3s, node.active.empty ());
	...
	ASSERT_EQ (vxlnetwork::vote_code::indeterminate, node.active.vote (vote_send1));
	ASSERT_EQ (vxlnetwork::vote_code::indeterminate, node.active.vote (vote_open1));
	ASSERT_EQ (vxlnetwork::vote_code::indeterminate, node.active.vote (vote1_send2));
	ASSERT_EQ (vxlnetwork::vote_code::indeterminate, node.active.vote (vote2_send2));
}
}
...
TEST (active_transactions, dropped_cleanup)
{
	...
	{
		vxlnetwork::vectorstream stream (block_bytes);
		vxlnetwork::dev::genesis->serialize (stream);
	}
	...
	// Not yet removed
	ASSERT_TRUE (node.network.publish_filter.apply (block_bytes.data (), block_bytes.size ()));
	ASSERT_EQ (1, node.active.blocks.count (vxlnetwork::dev::genesis->hash ()));
	...
	ASSERT_EQ (1, node.stats.count (vxlnetwork::stat::type::election, vxlnetwork::stat::detail::election_drop_all));

	// Block cleared from active
	ASSERT_EQ (0, node.active.blocks.count (vxlnetwork::dev::genesis->hash ()));

	// Repeat test for a confirmed election
	ASSERT_TRUE (node.network.publish_filter.apply (block_bytes.data (), block_bytes.size ()));
	node.block_confirm (vxlnetwork::dev::genesis);
	node.scheduler.flush ();
	election = node.active.election (vxlnetwork::dev::genesis->qualified_root ());
	ASSERT_NE (nullptr, election);
	election->force_confirm ();
...
TEST (active_transactions, republish_winner)
{
	...
	ASSERT_TIMELY (3s, node2.stats.count (vxlnetwork::stat::type::message, vxlnetwork::stat::detail::publish, vxlnetwork::stat::dir::in) == 1);
	...
	for (auto i (0); i < 5; i++)
	{
		auto fork = builder.make_block ()
					...
					.previous (vxlnetwork::dev::genesis->hash ())
					.representative (vxlnetwork::dev::genesis_key.pub)
					.balance (vxlnetwork::dev::constants.genesis_amount - 1 - i)
					.link (key.pub)
					.sign (vxlnetwork::dev::genesis_key.prv, vxlnetwork::dev::genesis_key.pub)
					.work (*system.work.generate (vxlnetwork::dev::genesis->hash ()))
					.build_shared ();
	...
	ASSERT_NE (nullptr, election);
	auto vote = std::make_shared<vxlnetwork::vote> (vxlnetwork::dev::genesis_key.pub, vxlnetwork::dev::genesis_key.prv, vxlnetwork::vote::timestamp_max, vxlnetwork::vote::duration_max, std::vector<vxlnetwork::block_hash>{ fork->hash () });
...
TEST (active_transactions, fork_filter_cleanup)
{
	...
	{
		vxlnetwork::vectorstream stream{ send_block_bytes };
...
TEST (active_transactions, fork_replacement_tally)
{
	...
	std::vector<vxlnetwork::keypair> keys (reps_count);
	...
	for (auto i (0); i < reps_count; i++)
	{
		...
		// Confirmation
		auto vote (std::make_shared<vxlnetwork::vote> (vxlnetwork::dev::genesis_key.pub, vxlnetwork::dev::genesis_key.prv, vxlnetwork::vote::timestamp_max, vxlnetwork::vote::duration_max, std::vector<vxlnetwork::block_hash>{ send->hash (), open->hash () }));
		node1.vote_processor.vote (vote, std::make_shared<vxlnetwork::transport::channel_loopback> (node1));
	}
	node1.block_processor.flush ();
	...
	auto send_last = builder.make_block ()
					 ...
					 .previous (latest)
					 .representative (vxlnetwork::dev::genesis_key.pub)
					 .balance (balance - 2 * vxlnetwork::Gxrb_ratio)
					 .link (key.pub)
					 .sign (vxlnetwork::dev::genesis_key.prv, vxlnetwork::dev::genesis_key.pub)
					 .work (*system.work.generate (latest))
					 .build_shared ();
	...
	for (auto i (0); i < reps_count; i++)
	{
		auto fork = builder.make_block ()
					...
					.previous (latest)
					.representative (vxlnetwork::dev::genesis_key.pub)
					.balance (balance - vxlnetwork::Gxrb_ratio - i)
					.link (key.pub)
					.sign (vxlnetwork::dev::genesis_key.prv, vxlnetwork::dev::genesis_key.pub)
					.work (*system.work.generate (latest))
					.build_shared ();
	...
	for (auto i (0); i < reps_count; i++)
	{
		auto fork = builder.make_block ()
					...
					.work (*system.work.generate (latest))
					.build_shared ();
		auto vote (std::make_shared<vxlnetwork::vote> (keys[i].pub, keys[i].prv, 0, 0, std::vector<vxlnetwork::block_hash>{ fork->hash () }));
		node1.vote_processor.vote (vote, std::make_shared<vxlnetwork::transport::channel_loopback> (node1));
		node1.vote_processor.flush ();
		node1.process_active (fork);
	}
	...
	ASSERT_TIMELY (3s, node1.stats.count (vxlnetwork::stat::type::message, vxlnetwork::stat::detail::publish, vxlnetwork::stat::dir::in) > 0);
	node1.block_processor.flush ();
	...
	auto vote (std::make_shared<vxlnetwork::vote> (vxlnetwork::dev::genesis_key.pub, vxlnetwork::dev::genesis_key.prv, 0, 0, std::vector<vxlnetwork::block_hash>{ send_last->hash () }));
	node1.vote_processor.vote (vote, std::make_shared<vxlnetwork::transport::channel_loopback> (node1));
	node1.vote_processor.flush ();
	node2.network.flood_block (send_last);
	ASSERT_TIMELY (3s, node1.stats.count (vxlnetwork::stat::type::message, vxlnetwork::stat::detail::publish, vxlnetwork::stat::dir::in) > 1);
	node1.block_processor.flush ();
	std::this_thread::sleep_for (50ms);
	...
	ASSERT_TRUE (votes2.find (vxlnetwork::dev::genesis_key.pub) != votes2.end ());
}
...
TEST (active_transactions, confirmation_consistency)
{
	...
	{
		auto block (system.wallet (0)->send_action (vxlnetwork::dev::genesis_key.pub, vxlnetwork::public_key (), node.config.receive_minimum.number ()));
...
// Test disabled because it's failing intermittently.
// PR in which it got disabled: https://github.com/vxlnetworkcurrency/vxlnetwork-node/pull/3629
// Issue for investigating it: https://github.com/vxlnetworkcurrency/vxlnetwork-node/issues/3634
TEST (active_transactions, DISABLED_confirm_new)
{
	...
	auto send = vxlnetwork::send_block_builder ()
				.previous (vxlnetwork::dev::genesis->hash ())
				.destination (vxlnetwork::public_key ())
				.balance (vxlnetwork::dev::constants.genesis_amount - 100)
				.sign (vxlnetwork::dev::genesis_key.prv, vxlnetwork::dev::genesis_key.pub)
				.work (*system.work.generate (vxlnetwork::dev::genesis->hash ()))
				.build_shared ();
...
TEST (active_transactions, conflicting_block_vote_existing_election)
{
	...
	auto fork = builder.make_block ()
				...
				.work (*system.work.generate (vxlnetwork::dev::genesis->hash ()))
				.build_shared ();
	auto vote_fork (std::make_shared<vxlnetwork::vote> (vxlnetwork::dev::genesis_key.pub, vxlnetwork::dev::genesis_key.prv, vxlnetwork::vote::timestamp_max, vxlnetwork::vote::duration_max, fork));
	...
	ASSERT_EQ (vxlnetwork::process_result::fork, node.process_local (fork).code);
...
TEST (active_transactions, activate_account_chain)
{
	...
	auto send3 = builder.make_block ()
				 ...
				 .representative (vxlnetwork::dev::genesis_key.pub)
				 .link (key.pub)
				 .balance (vxlnetwork::dev::constants.genesis_amount - 3)
				 .sign (vxlnetwork::dev::genesis_key.prv, vxlnetwork::dev::genesis_key.pub)
				 .work (*system.work.generate (send2->hash ()))
				 .build ();
	auto open = builder.make_block ()
...
TEST (active_transactions, pessimistic_elections)
{
	...
	{
		vxlnetwork::election election1 (
		node, send, [] (auto const &) {}, [] (auto const &) {}, vxlnetwork::election_behavior::normal);
		vxlnetwork::election election2 (
		node, open, [] (auto const &) {}, [] (auto const &) {}, vxlnetwork::election_behavior::normal);
	...
	ASSERT_EQ (2, node.active.expired_optimistic_election_infos.size ());
	auto election_started_it = node.active.expired_optimistic_election_infos.get<vxlnetwork::active_transactions::tag_election_started> ().begin ();
	ASSERT_EQ (election_started_it->account, vxlnetwork::dev::genesis->account ());
	...
	vxlnetwork::confirmation_height_info genesis_confirmation_height_info;
	vxlnetwork::confirmation_height_info key1_confirmation_height_info;
	{
		auto transaction = node.store.tx_begin_read ();
		node.store.confirmation_height.get (transaction, vxlnetwork::dev::genesis->account (), genesis_confirmation_height_info);
...
TEST (active_transactions, list_active)
{
	...
	vxlnetwork::blocks_confirm (node, { send, send2, open });
...
TEST (active_transactions, fifo)
{
	...
	node.process_confirmed (vxlnetwork::election_status{ send1 });
	ASSERT_TIMELY (5s, node.block_confirmed (send1->hash ()));
	...
	auto receive1 = builder.make_block ()
					...
					.build_shared ();
	ASSERT_EQ (vxlnetwork::process_result::progress, node.process (*receive1).code);
	...
	auto receive2 = builder.make_block ()
					...
					.work (*system.work.generate (key1.pub))
					.build_shared ();
	ASSERT_EQ (vxlnetwork::process_result::progress, node.process (*receive2).code);
	...
	ASSERT_EQ (1, node.stats.count (vxlnetwork::stat::type::election, vxlnetwork::stat::detail::election_drop_overflow));
