...
TEST (conflicts, add_existing)
{
	...
	ASSERT_NE (nullptr, election1);
	ASSERT_EQ (1, node1.active.size ());
	auto vote1 (std::make_shared<vxlnetwork::vote> (key2.pub, key2.prv, 0, 0, send2));
...
TEST (conflicts, add_two)
{
	...
	-> std::pair<std::optional<std::shared_ptr<vxlnetwork::block>>, std::optional<std::shared_ptr<vxlnetwork::block>>> {
		auto const send = vxlnetwork::send_block_builder{}.make_block ().previous (previous).destination (to.pub).balance (0).sign (from.prv, from.pub).work (*system.work.generate (previous)).build_shared ();

		if (vxlnetwork::process_result::progress != node->process (*send).code)
		{
		...
		auto const receive = vxlnetwork::open_block_builder{}.make_block ().account (to.pub).source (send->hash ()).representative (to.pub).sign (to.prv, to.pub).work (*system.work.generate (to.pub)).build_shared ();

		if (vxlnetwork::process_result::progress != node->process (*receive).code)
		{
			return std::make_pair (std::nullopt, std::nullopt);
		}
	...
	//
	vxlnetwork::keypair account1{};
	auto const [send1, receive1] = do_send (vxlnetwork::dev::genesis->hash (), vxlnetwork::dev::genesis_key, account1);
	...
	vxlnetwork::keypair account2{};
	auto const [send2, receive2] = do_send ((*send1)->hash (), vxlnetwork::dev::genesis_key, account2);
...
TEST (vote_uniquer, null)
{
	...
	vxlnetwork::vote_uniquer uniquer (block_uniquer);
...
TEST (vote_uniquer, same_vote)
{
	...
	auto vote1 (std::make_shared<vxlnetwork::vote> (key.pub, key.prv, 0, 0, std::make_shared<vxlnetwork::state_block> (0, 0, 0, 0, 0, key.prv, key.pub, 0)));
	auto vote2 (std::make_shared<vxlnetwork::vote> (*vote1));
...
TEST (vote_uniquer, same_block)
{
	...
	vxlnetwork::keypair key1;
	vxlnetwork::keypair key2;
	auto block1 (std::make_shared<vxlnetwork::state_block> (0, 0, 0, 0, 0, key1.prv, key1.pub, 0));
	auto block2 (std::make_shared<vxlnetwork::state_block> (*block1));
	auto vote1 (std::make_shared<vxlnetwork::vote> (key1.pub, key1.prv, 0, 0, block1));
	auto vote2 (std::make_shared<vxlnetwork::vote> (key1.pub, key1.prv, 0, 0, block2));
	ASSERT_EQ (vote1, uniquer.unique (vote1));
	ASSERT_EQ (vote2, uniquer.unique (vote2));
	ASSERT_NE (vote1, vote2);
	ASSERT_EQ (boost::get<std::shared_ptr<vxlnetwork::block>> (vote1->blocks[0]), boost::get<std::shared_ptr<vxlnetwork::block>> (vote2->blocks[0]));
}
...
TEST (vote_uniquer, vbh_one)
{
	...
	auto vote1 (std::make_shared<vxlnetwork::vote> (key.pub, key.prv, 0, 0, hashes));
	auto vote2 (std::make_shared<vxlnetwork::vote> (*vote1));
	ASSERT_EQ (vote1, uniquer.unique (vote1));
	ASSERT_EQ (vote1, uniquer.unique (vote2));
}
...
TEST (vote_uniquer, vbh_two)
{
	...
	vxlnetwork::vote_uniquer uniquer (block_uniquer);
	vxlnetwork::keypair key;
	auto block1 (std::make_shared<vxlnetwork::state_block> (0, 0, 0, 0, 0, key.prv, key.pub, 0));
	std::vector<vxlnetwork::block_hash> hashes1;
	hashes1.push_back (block1->hash ());
	auto block2 (std::make_shared<vxlnetwork::state_block> (1, 0, 0, 0, 0, key.prv, key.pub, 0));
	std::vector<vxlnetwork::block_hash> hashes2;
	hashes2.push_back (block2->hash ());
	auto vote1 (std::make_shared<vxlnetwork::vote> (key.pub, key.prv, 0, 0, hashes1));
	auto vote2 (std::make_shared<vxlnetwork::vote> (key.pub, key.prv, 0, 0, hashes2));
	ASSERT_EQ (vote1, uniquer.unique (vote1));
	ASSERT_EQ (vote2, uniquer.unique (vote2));
}
...
TEST (vote_uniquer, cleanup)
{
	...
	vxlnetwork::keypair key;
	auto vote1 (std::make_shared<vxlnetwork::vote> (key.pub, key.prv, 0, 0, std::make_shared<vxlnetwork::state_block> (0, 0, 0, 0, 0, key.prv, key.pub, 0)));
	auto vote2 (std::make_shared<vxlnetwork::vote> (key.pub, key.prv, vxlnetwork::vote::timestamp_min * 1, 0, std::make_shared<vxlnetwork::state_block> (0, 0, 0, 0, 0, key.prv, key.pub, 0)));
