...
TEST (toml, diff)
{
	vxlnetwork::tomlconfig defaults, other;
...
{
	vxlnetwork::tomlconfig t;
	boost::filesystem::path data_path (".");
	vxlnetwork::daemon_config c{ data_path, vxlnetwork::dev::network_params };
...
TEST (toml, rpc_config_deserialize_defaults)
{
	...
	vxlnetwork::tomlconfig t;
	t.read (ss);
	vxlnetwork::rpc_config conf{ vxlnetwork::dev::network_params.network };
	vxlnetwork::rpc_config defaults{ vxlnetwork::dev::network_params.network };
...
TEST (toml, daemon_config_deserialize_defaults)
{
	...
	vxlnetwork::tomlconfig t;
	t.read (ss);
	vxlnetwork::daemon_config conf;
	vxlnetwork::daemon_config defaults;
	conf.deserialize_toml (t);
...
TEST (toml, base_override)
{
	...
	ASSERT_TRUE (t.get_error () == vxlnetwork::error_config::missing_value);
	t.get_error ().clear ();

	// Query uint16 that's too big, make sure we have an error
	t.get_required<uint16_t> ("node.too_big", port);
	ASSERT_TRUE (t.get_error ());
	ASSERT_TRUE (t.get_error () == vxlnetwork::error_config::invalid_value);
}

TEST (toml, put)
{
	vxlnetwork::tomlconfig config;
	vxlnetwork::tomlconfig config_node;
	// Overwrite value and add to child node
	//
	config_node.put ("port", "7154");
	config_node.put ("port", "7155");
...
TEST (toml, daemon_config_deserialize_no_defaults)
{
	...
	preconfigured_representatives = ["vxlc_3arg3asgtigae3xckabaaewkx3bzsh7nwz7jkmjos79ihyaxwphhm6qgjps4"]
	receive_minimum = "999"
	...
	ASSERT_EQ (vxlnetwork::rocksdb_config::using_rocksdb_in_tests (), defaults.node.rocksdb_config.enable);
...
TEST (toml, daemon_config_deserialize_errors)
{
...

	{
		...
		ASSERT_EQ (conf.node.frontiers_confirmation, vxlnetwork::frontiers_confirmation_mode::invalid);
	}
...
TEST (toml, daemon_read_config)
{
	...
	vxlnetwork::daemon_config config;
	...
	ASSERT_FALSE (boost::filesystem::exists (vxlnetwork::get_node_toml_config_path (path)));
	ASSERT_FALSE (vxlnetwork::read_node_config_toml (path, config));
	{
		auto error = vxlnetwork::read_node_config_toml (path, config, invalid_overrides1);
		ASSERT_TRUE (error);
		ASSERT_EQ (error.get_message (), expected_message1);
	}
	{
		auto error = vxlnetwork::read_node_config_toml (path, config, invalid_overrides2);
		ASSERT_TRUE (error);
	...
	vxlnetwork::tomlconfig toml;
	toml.write (vxlnetwork::get_node_toml_config_path (path));

	// Reading when there is a config file
	ASSERT_TRUE (boost::filesystem::exists (vxlnetwork::get_node_toml_config_path (path)));
	ASSERT_FALSE (vxlnetwork::read_node_config_toml (path, config));
	{
		auto error = vxlnetwork::read_node_config_toml (path, config, invalid_overrides1);
		ASSERT_TRUE (error);
...
TEST (toml, tls_config_deserialize_no_defaults)
{
	...
	vxlnetwork::tomlconfig toml;
	toml.read (ss);
	vxlnetwork::tls_config conf;
	vxlnetwork::tls_config defaults;
	conf.deserialize_toml (toml);
