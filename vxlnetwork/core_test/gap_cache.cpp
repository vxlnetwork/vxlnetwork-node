...
TEST (gap_cache, add_new)
{
	...
	vxlnetwork::gap_cache cache (*system.nodes[0]);
...
TEST (gap_cache, add_existing)
{
	...
	auto block1 (std::make_shared<vxlnetwork::send_block> (0, 1, 2, vxlnetwork::keypair ().prv, 4, 5));
	cache.add (block1->hash ());
	vxlnetwork::unique_lock<vxlnetwork::mutex> lock (cache.mutex);
...
TEST (gap_cache, comparison)
{
	vxlnetwork::system system (1);
	vxlnetwork::gap_cache cache (*system.nodes[0]);
	auto block1 (std::make_shared<vxlnetwork::send_block> (1, 0, 2, vxlnetwork::keypair ().prv, 4, 5));
	cache.add (block1->hash ());
	vxlnetwork::unique_lock<vxlnetwork::mutex> lock (cache.mutex);
	auto existing1 (cache.blocks.get<1> ().find (block1->hash ()));
	ASSERT_NE (cache.blocks.get<1> ().end (), existing1);
	...
	auto block3 (std::make_shared<vxlnetwork::send_block> (0, 42, 1, vxlnetwork::keypair ().prv, 3, 4));
...
TEST (gap_cache, gap_bootstrap)
{
	...
	vxlnetwork::system system (2, vxlnetwork::transport::transport_type::tcp, node_flags);

	auto & node1 (*system.nodes[0]);
	auto & node2 (*system.nodes[1]);
	vxlnetwork::block_hash latest (node1.latest (vxlnetwork::dev::genesis_key.pub));
	vxlnetwork::keypair key;
	auto send (std::make_shared<vxlnetwork::send_block> (latest, key.pub, vxlnetwork::dev::constants.genesis_amount - 100, vxlnetwork::dev::genesis_key.prv, vxlnetwork::dev::genesis_key.pub, *system.work.generate (latest)));
	ASSERT_EQ (vxlnetwork::process_result::progress, node1.process (*send).code);
	ASSERT_EQ (vxlnetwork::dev::constants.genesis_amount - 100, node1.balance (vxlnetwork::dev::genesis->account ()));
	ASSERT_EQ (vxlnetwork::dev::constants.genesis_amount, node2.balance (vxlnetwork::dev::genesis->account ()));
	...
	auto latest_block (system.wallet (0)->send_action (vxlnetwork::dev::genesis_key.pub, key.pub, 100));
	ASSERT_NE (nullptr, latest_block);
	ASSERT_EQ (vxlnetwork::dev::constants.genesis_amount - 200, node1.balance (vxlnetwork::dev::genesis->account ()));
	ASSERT_EQ (vxlnetwork::dev::constants.genesis_amount, node2.balance (vxlnetwork::dev::genesis->account ()));
	ASSERT_TIMELY (10s, node2.balance (vxlnetwork::dev::genesis->account ()) == vxlnetwork::dev::constants.genesis_amount - 200);
}
...
TEST (gap_cache, two_dependencies)
{
	...
	auto & node1 (*system.nodes[0]);
	vxlnetwork::keypair key;
	auto send1 (std::make_shared<vxlnetwork::send_block> (vxlnetwork::dev::genesis->hash (), key.pub, 1, vxlnetwork::dev::genesis_key.prv, vxlnetwork::dev::genesis_key.pub, *system.work.generate (vxlnetwork::dev::genesis->hash ())));
	auto send2 (std::make_shared<vxlnetwork::send_block> (send1->hash (), key.pub, 0, vxlnetwork::dev::genesis_key.prv, vxlnetwork::dev::genesis_key.pub, *system.work.generate (send1->hash ())));
