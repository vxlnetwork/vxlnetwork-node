#include <vxlnetwork/core_test/fakes/work_peer.hpp>
...
TEST (distributed_work, stopped)
{
	...
	ASSERT_TRUE (system.nodes[0]->distributed_work.make (vxlnetwork::work_version::work_1, vxlnetwork::block_hash (), {}, vxlnetwork::dev::network_params.work.base, {}));
}
...
TEST (distributed_work, no_peers)
{
	...
	ASSERT_FALSE (node->distributed_work.make (vxlnetwork::work_version::work_1, hash, node->config.work_peers, node->network_params.work.base, callback, vxlnetwork::account ()));
	ASSERT_TIMELY (5s, done);
	ASSERT_GE (vxlnetwork::dev::network_params.work.difficulty (vxlnetwork::work_version::work_1, hash, *work), node->network_params.work.base);
...
TEST (distributed_work, no_peers_disabled)
{
	...
	ASSERT_TRUE (node.distributed_work.make (vxlnetwork::work_version::work_1, vxlnetwork::block_hash (), node.config.work_peers, vxlnetwork::dev::network_params.work.base, {}));
}
...
TEST (distributed_work, no_peers_cancel)
{
	...
	ASSERT_FALSE (node.distributed_work.make (vxlnetwork::work_version::work_1, hash, node.config.work_peers, vxlnetwork::difficulty::from_multiplier (1e6, node.network_params.work.base), callback_to_cancel));
	ASSERT_EQ (1, node.distributed_work.size ());
...
TEST (distributed_work, no_peers_multi)
{
	...
	{
		ASSERT_FALSE (node->distributed_work.make (vxlnetwork::work_version::work_1, hash, node->config.work_peers, vxlnetwork::difficulty::from_multiplier (10, node->network_params.work.base), callback));
	}
	...
	for (unsigned i{ 0 }; i < total; ++i)
	{
		vxlnetwork::block_hash hash_i (i + 1);
		ASSERT_FALSE (node->distributed_work.make (vxlnetwork::work_version::work_1, hash_i, node->config.work_peers, node->network_params.work.base, callback));
	}
	ASSERT_TIMELY (5s, count == total);
	system.deadline_set (5s);
	while (node->distributed_work.size () > 0)
...
TEST (distributed_work, peer)
{
	...
	auto work_peer (std::make_shared<fake_work_peer> (node->work, node->io_ctx, vxlnetwork::get_available_port (), work_peer_type::good));
	work_peer->start ();
	decltype (node->config.work_peers) peers;
	peers.emplace_back ("::ffff:127.0.0.1", work_peer->port ());
	ASSERT_FALSE (node->distributed_work.make (vxlnetwork::work_version::work_1, hash, peers, node->network_params.work.base, callback, vxlnetwork::account ()));
	ASSERT_TIMELY (5s, done);
	ASSERT_GE (vxlnetwork::dev::network_params.work.difficulty (vxlnetwork::work_version::work_1, hash, *work), node->network_params.work.base);
...
TEST (distributed_work, peer_malicious)
{
	...
	auto malicious_peer (std::make_shared<fake_work_peer> (node->work, node->io_ctx, vxlnetwork::get_available_port (), work_peer_type::malicious));
	...
	ASSERT_FALSE (node->local_work_generation_enabled ());
	auto malicious_peer2 (std::make_shared<fake_work_peer> (node->work, node->io_ctx, vxlnetwork::get_available_port (), work_peer_type::malicious));
	malicious_peer2->start ();
	peers[0].second = malicious_peer2->port ();
	ASSERT_FALSE (node->distributed_work.make (vxlnetwork::work_version::work_1, hash, peers, node->network_params.work.base, {}, vxlnetwork::account ()));
...
// Issue for investigating it: https://github.com/vxlnetworkcurrency/vxlnetwork-node/issues/3630
TEST (distributed_work, DISABLED_peer_multi)
{
	...
	auto good_peer (std::make_shared<fake_work_peer> (node->work, node->io_ctx, vxlnetwork::get_available_port (), work_peer_type::good));
	auto malicious_peer (std::make_shared<fake_work_peer> (node->work, node->io_ctx, vxlnetwork::get_available_port (), work_peer_type::malicious));
	auto slow_peer (std::make_shared<fake_work_peer> (node->work, node->io_ctx, vxlnetwork::get_available_port (), work_peer_type::slow));
	good_peer->start ();
	malicious_peer->start ();
	slow_peer->start ();
	decltype (node->config.work_peers) peers;
