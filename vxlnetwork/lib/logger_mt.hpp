...
inline boost::log::formatting_ostream & operator<< (boost::log::formatting_ostream & strm, boost::log::to_log_manip<vxlnetwork::severity_level, severity_tag> const & manip)
{
	...
	vxlnetwork::severity_level level = manip.get ();
...
private:
	...
	void output (vxlnetwork::severity_level severity_level, LogItems &&... log_items)
	{
...
public:
	 ...
	 */
	template <typename... LogItems>
	void always_log (vxlnetwork::severity_level severity_level, LogItems &&... log_items)
	{
	...
	{
		always_log (vxlnetwork::severity_level::normal, std::forward<LogItems> (log_items)...);
	}
	 ...
	 */
	template <typename... LogItems>
	bool try_log (vxlnetwork::severity_level severity_level, LogItems &&... log_items)
	{
		auto error (true);
		auto time_now = std::chrono::steady_clock::now ();
		vxlnetwork::unique_lock<vxlnetwork::mutex> lk (last_log_time_mutex);
	...
	{
		return try_log (vxlnetwork::severity_level::normal, std::forward<LogItems> (log_items)...);
	}

	std::chrono::milliseconds min_log_delta{ 0 };

private:
	vxlnetwork::mutex last_log_time_mutex;
