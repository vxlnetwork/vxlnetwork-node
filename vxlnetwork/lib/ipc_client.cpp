...
public:
	...
	virtual void async_write (vxlnetwork::shared_const_buffer const & buffer_a, std::function<void (boost::system::error_code const &, size_t)> callback_a) = 0;
...
class socket_client : public vxlnetwork::ipc::socket_base, public channel, public std::enable_shared_from_this<socket_client<SOCKET_TYPE, ENDPOINT_TYPE>>
{
public:
	...
	void async_write (vxlnetwork::shared_const_buffer const & buffer_a, std::function<void (boost::system::error_code const &, size_t)> callback_a) override
	{
		auto this_l (this->shared_from_this ());
	...
	void write_queued_messages ()
	{
		...
		this_l->timer_start (io_timeout);
		vxlnetwork::async_write (socket, msg.buffer,
...
private:
	...
	{
	public:
		queue_item (vxlnetwork::shared_const_buffer buffer_a, std::function<void (boost::system::error_code const &, size_t)> callback_a) :
			buffer (std::move (buffer_a)), callback (std::move (callback_a))
		{
		}
		vxlnetwork::shared_const_buffer buffer;
 ...
 */
class client_impl : public vxlnetwork::ipc::ipc_client_impl
{
public:
	...
	void connect (std::string const & host_a, uint16_t port_a, std::function<void (vxlnetwork::error)> callback_a)
	{
		...
		tcp_client->async_resolve (host_a, port_a, [this, callback = std::move (callback_a)] (boost::system::error_code const & ec_resolve_a, boost::asio::ip::tcp::endpoint const &) mutable {
			if (!ec_resolve_a)
			{
				this->tcp_client->async_connect ([cbk = std::move (callback)] (boost::system::error_code const & ec_connect_a) {
					cbk (vxlnetwork::error (ec_connect_a));
				});
			}
			else
			{
				callback (vxlnetwork::error (ec_resolve_a));
			}
		});
	}

	vxlnetwork::error connect (std::string const & path_a)
	{
		vxlnetwork::error err;
...
#else
		err = vxlnetwork::error ("Domain sockets are not supported by this platform");
#endif
		return err;
	}
...
vxlnetwork::ipc::ipc_client::ipc_client (boost::asio::io_context & io_ctx_a) :
	io_ctx (io_ctx_a)
{
}

vxlnetwork::error vxlnetwork::ipc::ipc_client::connect (std::string const & path_a)
{
	impl = std::make_unique<client_impl> (io_ctx);
	return boost::polymorphic_downcast<client_impl *> (impl.get ())->connect (path_a);
}

void vxlnetwork::ipc::ipc_client::async_connect (std::string const & host_a, uint16_t port_a, std::function<void (vxlnetwork::error)> callback_a)
{
	impl = std::make_unique<client_impl> (io_ctx);
...
vxlnetwork::error vxlnetwork::ipc::ipc_client::connect (std::string const & host, uint16_t port)
{
	std::promise<vxlnetwork::error> result_l;
	async_connect (host, port, [&result_l] (vxlnetwork::error err_a) {
		result_l.set_value (std::move (err_a));
	});
	return result_l.get_future ().get ();
}

void vxlnetwork::ipc::ipc_client::async_write (vxlnetwork::shared_const_buffer const & buffer_a, std::function<void (vxlnetwork::error, size_t)> callback_a)
{
	auto client (boost::polymorphic_downcast<client_impl *> (impl.get ()));
	client->get_channel ().async_write (buffer_a, [callback = std::move (callback_a)] (boost::system::error_code const & ec_a, size_t bytes_written_a) {
		callback (vxlnetwork::error (ec_a), bytes_written_a);
	});
}

void vxlnetwork::ipc::ipc_client::async_read (std::shared_ptr<std::vector<uint8_t>> const & buffer_a, size_t size_a, std::function<void (vxlnetwork::error, size_t)> callback_a)
{
	auto client (boost::polymorphic_downcast<client_impl *> (impl.get ()));
	client->get_channel ().async_read (buffer_a, size_a, [callback = std::move (callback_a), buffer_a] (boost::system::error_code const & ec_a, size_t bytes_read_a) {
		callback (vxlnetwork::error (ec_a), bytes_read_a);
	});
}

/** Read a length-prefixed message asynchronously. Received length must be a big endian 32-bit unsigned integer. */
void vxlnetwork::ipc::ipc_client::async_read_message (std::shared_ptr<std::vector<uint8_t>> const & buffer_a, std::chrono::seconds timeout_a, std::function<void (vxlnetwork::error, size_t)> callback_a)
{
	auto client (boost::polymorphic_downcast<client_impl *> (impl.get ()));
...
std::vector<uint8_t> vxlnetwork::ipc::get_preamble (vxlnetwork::ipc::payload_encoding encoding_a)
{
...
vxlnetwork::shared_const_buffer vxlnetwork::ipc::prepare_flatbuffers_request (std::shared_ptr<flatbuffers::FlatBufferBuilder> const & flatbuffer_a)
{
	auto buffer_l (get_preamble (vxlnetwork::ipc::payload_encoding::flatbuffers));
	...
	return vxlnetwork::shared_const_buffer{ std::move (buffer_l) };
}

vxlnetwork::shared_const_buffer vxlnetwork::ipc::prepare_request (vxlnetwork::ipc::payload_encoding encoding_a, std::string const & payload_a)
{
	std::vector<uint8_t> buffer_l;
	if (encoding_a == vxlnetwork::ipc::payload_encoding::json_v1 || encoding_a == vxlnetwork::ipc::payload_encoding::flatbuffers_json)
	{
...
std::string vxlnetwork::ipc::request (vxlnetwork::ipc::payload_encoding encoding_a, vxlnetwork::ipc::ipc_client & ipc_client, std::string const & rpc_action_a)
{
	auto req (prepare_request (encoding_a, rpc_action_a));
	auto res (std::make_shared<std::vector<uint8_t>> ());

	std::promise<std::string> result_l;
	ipc_client.async_write (req, [&ipc_client, &res, &result_l] (vxlnetwork::error const &, size_t size_a) {
		// Read length
		ipc_client.async_read (res, sizeof (uint32_t), [&ipc_client, &res, &result_l] (vxlnetwork::error const &, size_t size_read_a) {
			uint32_t payload_size_l = boost::endian::big_to_native (*reinterpret_cast<uint32_t *> (res->data ()));
			// Read json payload
			ipc_client.async_read (res, payload_size_l, [&res, &result_l] (vxlnetwork::error const &, size_t size_read_a) {
