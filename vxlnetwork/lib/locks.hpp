...
#define USING_VXLNETWORK_TIMED_LOCKS (VXLNETWORK_TIMED_LOCKS > 0)
...
{
class mutex;
extern vxlnetwork::mutex * mutex_to_filter;
extern vxlnetwork::mutex mutex_to_filter_mutex;
...
template <typename Mutex>
void output_if_held_long_enough (vxlnetwork::timer<std::chrono::milliseconds> & timer, Mutex & mutex);

#ifndef VXLNETWORK_TIMED_LOCKS_IGNORE_BLOCKED
template <typename Mutex>
void output_if_blocked_long_enough (vxlnetwork::timer<std::chrono::milliseconds> & timer, Mutex & mutex);
#endif
...
template <>
class lock_guard<vxlnetwork::mutex> final
{
public:
	explicit lock_guard (vxlnetwork::mutex & mutex_a);
	~lock_guard () noexcept;
...
private:
	vxlnetwork::mutex & mut;
	vxlnetwork::timer<std::chrono::milliseconds> timer;
};

template <typename Mutex, typename = std::enable_if_t<std::is_same<Mutex, vxlnetwork::mutex>::value>>
...
/** Assumes std implementations of std::condition_variable never actually call vxlnetwork::unique_lock::lock/unlock,
...
public:
	...
	void wait (vxlnetwork::unique_lock<vxlnetwork::mutex> & lt);

	template <typename Pred>
	void wait (vxlnetwork::unique_lock<vxlnetwork::mutex> & lk, Pred pred)
	{
		while (!pred ())
		{
			wait (lk);
	...
	std::cv_status wait_until (vxlnetwork::unique_lock<vxlnetwork::mutex> & lk, std::chrono::time_point<Clock, Duration> const & timeout_time)
	{
		if (!lk.mut || !lk.owns)
		{
	...
	bool wait_until (vxlnetwork::unique_lock<vxlnetwork::mutex> & lk, std::chrono::time_point<Clock, Duration> const & timeout_time, Pred pred)
	{
		while (!pred ())
		{
	...
	void wait_for (vxlnetwork::unique_lock<vxlnetwork::mutex> & lk, std::chrono::duration<Rep, Period> const & rel_time)
	{
		wait_until (lk, std::chrono::steady_clock::now () + rel_time);
	}

	template <typename Rep, typename Period, typename Pred>
	bool wait_for (vxlnetwork::unique_lock<vxlnetwork::mutex> & lk, std::chrono::duration<Rep, Period> const & rel_time, Pred pred)
	{
