...
class configbase : public vxlnetwork::error_aware<>
{
public:
	configbase () = default;
	configbase (std::shared_ptr<vxlnetwork::error> const & error_a) :
		error (error_a)
	{
	}

	/** Returns the current error */
	vxlnetwork::error & get_error () override
	{
		return *error;
	}
...
protected:
	...
	/** We're a vxlnetwork::error_aware type. Child nodes share the error state. */
	std::shared_ptr<vxlnetwork::error> error;
