...
vxlnetwork::error vxlnetwork::rpc_secure_config::serialize_toml (vxlnetwork::tomlconfig & toml) const
{
...
vxlnetwork::error vxlnetwork::rpc_secure_config::deserialize_toml (vxlnetwork::tomlconfig & toml)
{
...
vxlnetwork::rpc_config::rpc_config (vxlnetwork::network_constants & network_constants) :
	rpc_process{ network_constants },
	address{ boost::asio::ip::address_v6::loopback ().to_string () }
{
}

vxlnetwork::rpc_config::rpc_config (vxlnetwork::network_constants & network_constants, uint16_t port_a, bool enable_control_a) :
	rpc_process{ network_constants },
	address{ boost::asio::ip::address_v6::loopback ().to_string () },
	port{ port_a },
...
vxlnetwork::error vxlnetwork::rpc_config::serialize_toml (vxlnetwork::tomlconfig & toml) const
{
	...
	vxlnetwork::tomlconfig rpc_process_l;
	...
	vxlnetwork::tomlconfig rpc_logging_l;
	rpc_logging_l.put ("log_rpc", rpc_logging.log_rpc, "Whether to log RPC calls.\ntype:bool");
	toml.put_child ("logging", rpc_logging_l);
	return toml.get_error ();
}

vxlnetwork::error vxlnetwork::rpc_config::deserialize_toml (vxlnetwork::tomlconfig & toml)
{
	if (!toml.empty ())
	{
		...
		if (rpc_secure_l)
		{
			return vxlnetwork::error ("The RPC secure configuration has moved to config-tls.toml. Please update the configuration.");
		}
...
vxlnetwork::rpc_process_config::rpc_process_config (vxlnetwork::network_constants & network_constants) :
...
{
vxlnetwork::error read_rpc_config_toml (boost::filesystem::path const & data_path_a, vxlnetwork::rpc_config & config_a, std::vector<std::string> const & config_overrides)
{
	vxlnetwork::error error;
	auto toml_config_path = vxlnetwork::get_rpc_toml_config_path (data_path_a);
...
std::string get_default_rpc_filepath ()
{
	...
	// Construct the vxlnetwork_rpc executable file path based on where the currently running executable is found.
