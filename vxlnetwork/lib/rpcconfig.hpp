...
{
public:
	rpc_process_config (vxlnetwork::network_constants & network_constants);
...
{
public:
	explicit rpc_config (vxlnetwork::network_constants & network_constants);
	explicit rpc_config (vxlnetwork::network_constants & network_constants, uint16_t, bool);
	vxlnetwork::error serialize_toml (vxlnetwork::tomlconfig &) const;
	vxlnetwork::error deserialize_toml (vxlnetwork::tomlconfig &);

	vxlnetwork::rpc_process_config rpc_process;
	std::string address;
	...
	vxlnetwork::rpc_logging_config rpc_logging;
	/** Optional TLS config */
	std::shared_ptr<vxlnetwork::tls_config> tls_config;
};

vxlnetwork::error read_rpc_config_toml (boost::filesystem::path const & data_path_a, vxlnetwork::rpc_config & config_a, std::vector<std::string> const & config_overrides = std::vector<std::string> ());
