...
{
vxlnetwork::error vxlnetwork::tls_config::serialize_toml (vxlnetwork::tomlconfig & toml) const
{
...
vxlnetwork::error vxlnetwork::tls_config::deserialize_toml (vxlnetwork::tomlconfig & toml)
{
...
namespace
{
	bool on_verify_certificate (bool preverified, boost::asio::ssl::verify_context & ctx, vxlnetwork::tls_config & config_a, vxlnetwork::logger_mt & logger_a)
	{
	...
	void load_certs (vxlnetwork::tls_config & config_a, vxlnetwork::logger_mt & logger_a)
	{
		try
		{
...
vxlnetwork::error read_tls_config_toml (boost::filesystem::path const & data_path_a, vxlnetwork::tls_config & config_a, vxlnetwork::logger_mt & logger_a, std::vector<std::string> const & config_overrides)
{
	vxlnetwork::error error;
	auto toml_config_path = vxlnetwork::get_tls_toml_config_path (data_path_a);
...
#else
		auto msg ("https or wss is enabled in the TLS configuration, but the node is not built with VXLNETWORK_SECURE_RPC");
