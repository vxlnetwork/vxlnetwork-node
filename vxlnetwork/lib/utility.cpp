...
std::size_t vxlnetwork::get_file_descriptor_limit ()
{
...
void vxlnetwork::set_file_descriptor_limit (std::size_t limit)
{
#ifndef _WIN32
	rlimit fd_limit{};
...
vxlnetwork::container_info_composite::container_info_composite (std::string const & name) :
	name (name)
{
}

bool vxlnetwork::container_info_composite::is_composite () const
{
	return true;
}

void vxlnetwork::container_info_composite::add_component (std::unique_ptr<container_info_component> child)
{
	children.push_back (std::move (child));
}

std::vector<std::unique_ptr<vxlnetwork::container_info_component>> const & vxlnetwork::container_info_composite::get_children () const
{
	return children;
}

std::string const & vxlnetwork::container_info_composite::get_name () const
{
	return name;
}

vxlnetwork::container_info_leaf::container_info_leaf (const container_info & info) :
	info (info)
{
}

bool vxlnetwork::container_info_leaf::is_composite () const
{
	return false;
}

vxlnetwork::container_info const & vxlnetwork::container_info_leaf::get_info () const
{
	return info;
}

void vxlnetwork::dump_crash_stacktrace ()
{
	boost::stacktrace::safe_dump_to ("vxlnetwork_node_backtrace.dump");
}

std::string vxlnetwork::generate_stacktrace ()
{
...
void vxlnetwork::remove_all_files_in_dir (boost::filesystem::path const & dir)
{
...
void vxlnetwork::move_all_files_to_dir (boost::filesystem::path const & from, boost::filesystem::path const & to)
{
...
void assert_internal (char const * check_expr, char const * func, char const * file, unsigned int line, bool is_release_assert, std::string_view error_msg)
{
	...
	auto backtrace_str = vxlnetwork::generate_stacktrace ();
...
#ifdef _WIN32
	{
		...
		std::string filename = is_release_assert ? "vxlnetwork_node_backtrace_release_assert.txt" : "vxlnetwork_node_backtrace_assert.txt";
		...
		vxlnetwork::set_secure_perm_file (filepath);
		file << backtrace_str;
	}
#endif
...
// Issue #3748
void vxlnetwork::sort_options_description (const boost::program_options::options_description & source, boost::program_options::options_description & target)
{
