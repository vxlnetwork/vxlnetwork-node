...
{
	static vxlnetwork::mutex cout_mutex;
	auto stacktrace = vxlnetwork::generate_stacktrace ();
...
void output_if_held_long_enough (vxlnetwork::timer<std::chrono::milliseconds> & timer, Mutex & mutex)
{
	...
	if (time_held >= std::chrono::milliseconds (VXLNETWORK_TIMED_LOCKS))
	{
		std::unique_lock lk (vxlnetwork::mutex_to_filter_mutex);
		if (!vxlnetwork::any_filters_registered () || (vxlnetwork::mutex_to_filter == &mutex))
		{
			lk.unlock ();
			output ("held", time_held, mutex);
		}
	}
	if (timer.current_state () != vxlnetwork::timer_state::stopped)
	{
		timer.stop ();
	}
}
...
void output_if_blocked_long_enough (vxlnetwork::timer<std::chrono::milliseconds> & timer, Mutex & mutex)
{
	...
	if (time_blocked >= std::chrono::milliseconds (VXLNETWORK_TIMED_LOCKS))
	{
		std::unique_lock lk (vxlnetwork::mutex_to_filter_mutex);
		if (!vxlnetwork::any_filters_registered () || (vxlnetwork::mutex_to_filter == &mutex))
		{
...
lock_guard<vxlnetwork::mutex>::lock_guard (vxlnetwork::mutex & mutex) :
	mut (mutex)
{
	timer.start ();
...
lock_guard<vxlnetwork::mutex>::~lock_guard () noexcept
{
	mut.unlock ();
...
template class unique_lock<vxlnetwork::mutex>;
...
void condition_variable::wait (vxlnetwork::unique_lock<vxlnetwork::mutex> & lk)
{
	if (!lk.mut || !lk.owns)
	{
		throw (std::system_error (std::make_error_code (std::errc::operation_not_permitted)));
...
vxlnetwork::mutex * mutex_to_filter{ nullptr };
vxlnetwork::mutex mutex_to_filter_mutex;

bool should_be_filtered (char const * name)
{
	return std::strcmp (name, xstr (VXLNETWORK_TIMED_LOCKS_FILTER)) == 0;
}

bool any_filters_registered ()
{
	return std::strcmp ("", xstr (VXLNETWORK_TIMED_LOCKS_FILTER)) != 0;
}
}
#endif

char const * vxlnetwork::mutex_identifier (mutexes mutex)
{
	switch (mutex)
	{
		case mutexes::active:
