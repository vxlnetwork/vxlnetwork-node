...
REGISTER_ERROR_CODES (vxlnetwork, error_common);
REGISTER_ERROR_CODES (vxlnetwork, error_blocks);
REGISTER_ERROR_CODES (vxlnetwork, error_rpc);
REGISTER_ERROR_CODES (vxlnetwork, error_process);
REGISTER_ERROR_CODES (vxlnetwork, error_config);
...
public:
	error () = default;
	error (vxlnetwork::error const & error_a) = default;
	error (vxlnetwork::error && error_a) = default;
	...
	error & operator= (vxlnetwork::error const & err_a);
	error & operator= (vxlnetwork::error && err_a);
	...
	error & then (std::function<vxlnetwork::error &()> next);
	...
	error & set (std::string message_a, std::error_code code_a = vxlnetwork::error_common::generic);
...
/**
 * A type that manages a vxlnetwork::error.
 * The default return type is vxlnetwork::error&, though shared_ptr<vxlnetwork::error> is a good option in cases
 * where shared error state is desirable.
 */
template <typename RET_TYPE = vxlnetwork::error &>
class error_aware
{
	static_assert (std::is_same<RET_TYPE, vxlnetwork::error &>::value || std::is_same<RET_TYPE, std::shared_ptr<vxlnetwork::error>>::value, "Must be vxlnetwork::error& or shared_ptr<vxlnetwork::error>");
