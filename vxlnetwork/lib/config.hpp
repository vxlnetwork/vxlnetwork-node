...
*/
char const * const VXLNETWORK_VERSION_STRING = xstr (TAG_VERSION_STRING);
char const * const VXLNETWORK_MAJOR_VERSION_STRING = xstr (MAJOR_VERSION_STRING);
char const * const VXLNETWORK_MINOR_VERSION_STRING = xstr (MINOR_VERSION_STRING);
char const * const VXLNETWORK_PATCH_VERSION_STRING = xstr (PATCH_VERSION_STRING);
char const * const VXLNETWORK_PRE_RELEASE_VERSION_STRING = xstr (PRE_RELEASE_VERSION_STRING);
...
enum class networks : uint16_t
{
	...
	vxlnetwork_dev_network = 0x5241, // 'R', 'A'
	// Normal work parameters, secret beta genesis key, beta IP ports
	vxlnetwork_beta_network = 0x5242, // 'R', 'B'
	// Normal work parameters, secret live key, live IP ports
	vxlnetwork_live_network = 0x5243, // 'R', 'C'
	// Normal work parameters, secret test genesis key, test IP ports
	vxlnetwork_test_network = 0x5258, // 'R', 'X'
};
...
public:
	...
	work_thresholds operator= (vxlnetwork::work_thresholds const & other_a)
	{
		return other_a;
	}

	uint64_t threshold_entry (vxlnetwork::work_version const, vxlnetwork::block_type const) const;
	uint64_t threshold (vxlnetwork::block_details const &) const;
	// Ledger threshold
	uint64_t threshold (vxlnetwork::work_version const, vxlnetwork::block_details const) const;
	uint64_t threshold_base (vxlnetwork::work_version const) const;
	uint64_t value (vxlnetwork::root const & root_a, uint64_t work_a) const;
	...
	uint64_t difficulty (vxlnetwork::work_version const, vxlnetwork::root const &, uint64_t const) const;
	uint64_t difficulty (vxlnetwork::block const & block_a) const;
	bool validate_entry (vxlnetwork::work_version const, vxlnetwork::root const &, uint64_t const) const;
	bool validate_entry (vxlnetwork::block const &) const;

	/** Network work thresholds. Define these inline as constexpr when moving to cpp17. */
	static vxlnetwork::work_thresholds const publish_full;
	static vxlnetwork::work_thresholds const publish_beta;
	static vxlnetwork::work_thresholds const publish_dev;
	static vxlnetwork::work_thresholds const publish_test;
};

class network_constants
{
public:
	network_constants (vxlnetwork::work_thresholds & work, vxlnetwork::networks network_a) :
		...
		work{ work }
	{
		...
		default_node_port = is_live_network () ? 7155 : is_beta_network () ? 54000
		: is_test_network ()                                               ? test_node_port ()
																		   : 44000;
		default_rpc_port = is_live_network () ? 7156 : is_beta_network () ? 55000
		: is_test_network ()                                              ? test_rpc_port ()
																		  : 45000;
		default_ipc_port = is_live_network () ? 7157 : is_beta_network () ? 56000
		: is_test_network ()                                              ? test_ipc_port ()
																		  : 46000;
		default_websocket_port = is_live_network () ? 7158 : is_beta_network () ? 57000
	...
	vxlnetwork::networks current_network{ vxlnetwork::network_constants::active_network };
	...
	vxlnetwork::networks network () const
	{
	 ...
	 */
	static void set_active_network (vxlnetwork::networks network_a)
	{
	...
	static bool set_active_network (std::string network_a)
	{
		...
		if (network_a == "live")
		{
			active_network = vxlnetwork::networks::vxlnetwork_live_network;
		}
		else if (network_a == "beta")
		{
			active_network = vxlnetwork::networks::vxlnetwork_beta_network;
		}
		else if (network_a == "dev")
		{
			active_network = vxlnetwork::networks::vxlnetwork_dev_network;
		}
		else if (network_a == "test")
		{
			active_network = vxlnetwork::networks::vxlnetwork_test_network;
		}
		else
		{
			error = true;
	...
	{
		return current_network == vxlnetwork::networks::vxlnetwork_live_network;
	}
	bool is_beta_network () const
	{
		return current_network == vxlnetwork::networks::vxlnetwork_beta_network;
	}
	bool is_dev_network () const
	{
		return current_network == vxlnetwork::networks::vxlnetwork_dev_network;
	}
	bool is_test_network () const
	{
		return current_network == vxlnetwork::networks::vxlnetwork_test_network;
	}

	/** Initial value is ACTIVE_NETWORK compile flag, but can be overridden by a CLI flag */
	static vxlnetwork::networks active_network;
