...
#include <vxlnetwork/boost/asio/deadline_timer.hpp>
#include <vxlnetwork/boost/asio/executor_work_guard.hpp>
#include <vxlnetwork/boost/asio/io_context.hpp>
#include <vxlnetwork/boost/asio/steady_timer.hpp>
#include <vxlnetwork/boost/asio/thread_pool.hpp>
...
namespace thread_role
{
	 ...
	 */
	vxlnetwork::thread_role::name get ();
	void set (vxlnetwork::thread_role::name);

	/*
	 * Get the thread name as a string from enum
	 */
	std::string get_string (vxlnetwork::thread_role::name);
...
{
public:
	explicit thread_pool (unsigned, vxlnetwork::thread_role::name);
	~thread_pool ();
...
private:
	...
	void set_thread_names (unsigned num_threads, vxlnetwork::thread_role::name thread_name);
};

std::unique_ptr<vxlnetwork::container_info_component> collect_container_info (thread_pool & thread_pool, std::string const & name);
}
