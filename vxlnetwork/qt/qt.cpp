...
bool vxlnetwork_qt::eventloop_processor::event (QEvent * event_a)
{
	debug_assert (dynamic_cast<vxlnetwork_qt::eventloop_event *> (event_a) != nullptr);
	static_cast<vxlnetwork_qt::eventloop_event *> (event_a)->action ();
	return true;
}

vxlnetwork_qt::eventloop_event::eventloop_event (std::function<void ()> const & action_a) :
	QEvent (QEvent::Type::User),
	action (action_a)
{
}

vxlnetwork_qt::self_pane::self_pane (vxlnetwork_qt::wallet & wallet_a, vxlnetwork::account const & account_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	self_layout (new QHBoxLayout),
	self_window (new QWidget),
	your_account_label (new QLabel ("Your Vxlnetwork account:")),
	...
	wallet (wallet_a)
{
	...
	version = new QLabel (boost::str (boost::format ("%1% %2% network") % VXLNETWORK_VERSION_STRING % network).c_str ());
...
void vxlnetwork_qt::self_pane::set_balance_text (std::pair<vxlnetwork::uint128_t, vxlnetwork::uint128_t> balance_a)
{
...
vxlnetwork_qt::accounts::accounts (vxlnetwork_qt::wallet & wallet_a) :
...
void vxlnetwork_qt::accounts::refresh ()
{
	...
	{
		vxlnetwork::public_key key (i->first);
...
vxlnetwork_qt::import::import (vxlnetwork_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
...
vxlnetwork_qt::history::history (vxlnetwork::ledger & ledger_a, vxlnetwork::account const & account_a, vxlnetwork_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	model (new QStandardItemModel),
	view (new QTableView),
...
namespace
{
class short_text_visitor : public vxlnetwork::block_visitor
{
public:
	short_text_visitor (vxlnetwork::transaction const & transaction_a, vxlnetwork::ledger & ledger_a) :
	...
	void open_block (vxlnetwork::open_block const & block_a)
	{
		...
		else
		{
			...
			amount = vxlnetwork::dev::constants.genesis_amount;
		}
	}
...
void vxlnetwork_qt::history::refresh ()
{
...
vxlnetwork_qt::block_viewer::block_viewer (vxlnetwork_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
...
void vxlnetwork_qt::block_viewer::rebroadcast_action (vxlnetwork::block_hash const & hash_a)
{
	auto done (true);
...
vxlnetwork_qt::account_viewer::account_viewer (vxlnetwork_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	...
	wallet (wallet_a)
{
	...
	QObject::connect (refresh, &QPushButton::released, [this] () {
		...
		if (!account.decode_account (account_line->text ().toStdString ()))
		{
			...
			auto final_text (std::string ("Balance (VXLNETWORK): ") + wallet.format_balance (balance.first));
...
vxlnetwork_qt::stats_viewer::stats_viewer (vxlnetwork_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	refresh (new QPushButton ("Refresh")),
...
void vxlnetwork_qt::stats_viewer::refresh_stats ()
{
	model->removeRows (0, model->rowCount ());
...
vxlnetwork_qt::status::status (vxlnetwork_qt::wallet & wallet_a) :
	wallet (wallet_a)
{
	wallet.status->setToolTip ("Wallet status, block count (blocks downloaded)");
	active.insert (vxlnetwork_qt::status_types::nominal);
	set_text ();
}

void vxlnetwork_qt::status::erase (vxlnetwork_qt::status_types status_a)
{
	debug_assert (status_a != vxlnetwork_qt::status_types::nominal);
	auto erased (active.erase (status_a));
	(void)erased;
	set_text ();
}

void vxlnetwork_qt::status::insert (vxlnetwork_qt::status_types status_a)
{
	debug_assert (status_a != vxlnetwork_qt::status_types::nominal);
...
std::string vxlnetwork_qt::status::text ()
{
	...
	switch (*active.begin ())
	{
		case vxlnetwork_qt::status_types::disconnected:
			result = "Status: Disconnected";
			break;
		case vxlnetwork_qt::status_types::working:
			result = "Status: Generating proof of work";
			break;
		case vxlnetwork_qt::status_types::synchronizing:
			result = "Status: Synchronizing";
			break;
		case vxlnetwork_qt::status_types::locked:
			result = "Status: Wallet locked";
			break;
		case vxlnetwork_qt::status_types::vulnerable:
			result = "Status: Wallet password empty";
			break;
		case vxlnetwork_qt::status_types::active:
			result = "Status: Wallet active";
			break;
		case vxlnetwork_qt::status_types::nominal:
...
std::string vxlnetwork_qt::status::color ()
{
	debug_assert (!active.empty ());
	std::string result;
	switch (*active.begin ())
...
vxlnetwork_qt::wallet::wallet (QApplication & application_a, vxlnetwork_qt::eventloop_processor & processor_a, vxlnetwork::node & node_a, std::shared_ptr<vxlnetwork::wallet> const & wallet_a, vxlnetwork::account & account_a) :
	rendering_ratio (vxlnetwork::Mxrb_ratio),
	node (node_a),
	wallet_m (wallet_a),
	account (account_a),
...
void vxlnetwork_qt::wallet::ongoing_refresh ()
{
	std::weak_ptr<vxlnetwork_qt::wallet> wallet_w (shared_from_this ());
...
void vxlnetwork_qt::wallet::start ()
{
	ongoing_refresh ();
	std::weak_ptr<vxlnetwork_qt::wallet> this_w (shared_from_this ());
					...
					if (actual <= balance)
					{
						...
						if (this_l->wallet_m->store.valid_password (transaction))
						{
							...
							this_l->node.background ([this_w, account_l, actual] () {
								if (auto this_l = this_w.lock ())
								{
									this_l->wallet_m->send_async (this_l->account, account_l, actual, [this_w] (std::shared_ptr<vxlnetwork::block> const & block_a) {
										if (auto this_l = this_w.lock ())
										{
	...
	node.observers.blocks.add ([this_w] (vxlnetwork::election_status const & status_a, std::vector<vxlnetwork::vote_with_weight_info> const & votes_a, vxlnetwork::account const & account_a, vxlnetwork::uint128_t const & amount_a, bool, bool) {
		if (auto this_l = this_w.lock ())
		{
	...
	node.observers.account_balance.add ([this_w] (vxlnetwork::account const & account_a, bool is_pending) {
		if (auto this_l = this_w.lock ())
		{
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a] () {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (vxlnetwork_qt::status_types::active);
					}
					else
					{
						this_l->active_status.erase (vxlnetwork_qt::status_types::active);
					}
				}
			}));
		}
	});
	node.observers.endpoint.add ([this_w] (std::shared_ptr<vxlnetwork::transport::channel> const &) {
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w] () {
				if (auto this_l = this_w.lock ())
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a] () {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (vxlnetwork_qt::status_types::synchronizing);
					}
					else
					{
						this_l->active_status.erase (vxlnetwork_qt::status_types::synchronizing);
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, working] () {
				if (auto this_l = this_w.lock ())
				{
					if (working)
					{
						this_l->active_status.insert (vxlnetwork_qt::status_types::working);
					}
					else
					{
						this_l->active_status.erase (vxlnetwork_qt::status_types::working);
					}
				}
			}));
		}
...
void vxlnetwork_qt::wallet::refresh ()
{
	{
...
void vxlnetwork_qt::wallet::update_connected ()
{
	if (node.network.empty ())
	{
		active_status.insert (vxlnetwork_qt::status_types::disconnected);
	}
	else
	{
		active_status.erase (vxlnetwork_qt::status_types::disconnected);
	}
}

void vxlnetwork_qt::wallet::empty_password ()
{
...
void vxlnetwork_qt::wallet::change_rendering_ratio (vxlnetwork::uint128_t const & rendering_ratio_a)
{
...
//
std::string vxlnetwork_qt::wallet::format_balance (vxlnetwork::uint128_t const & balance) const
{
	auto balance_str = vxlnetwork::amount (balance).format_balance (rendering_ratio, 3, false);
	auto unit = std::string ("vxlc");
	if (rendering_ratio == vxlnetwork::raw_ratio)
	{
		unit = std::string ("raw");
	}
...
void vxlnetwork_qt::wallet::push_main_stack (QWidget * widget_a)
{
	main_stack->addWidget (widget_a);
	main_stack->setCurrentIndex (main_stack->count () - 1);
}

void vxlnetwork_qt::wallet::pop_main_stack ()
{
	main_stack->removeWidget (main_stack->currentWidget ());
}

vxlnetwork_qt::settings::settings (vxlnetwork_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	password (new QLineEdit),
	...
	wallet (wallet_a)
{
	...
	QObject::connect (change_rep, &QPushButton::released, [this] () {
		vxlnetwork::account representative_l;
...
void vxlnetwork_qt::settings::refresh_representative ()
{
...
void vxlnetwork_qt::settings::activate ()
{
	this->wallet.push_main_stack (window);
}

void vxlnetwork_qt::settings::update_locked (bool invalid, bool vulnerable)
{
	if (invalid)
	{
		this->wallet.active_status.insert (vxlnetwork_qt::status_types::locked);
	}
	else
	{
		this->wallet.active_status.erase (vxlnetwork_qt::status_types::locked);
	}
	if (vulnerable)
	{
		this->wallet.active_status.insert (vxlnetwork_qt::status_types::vulnerable);
	}
	else
	{
		this->wallet.active_status.erase (vxlnetwork_qt::status_types::vulnerable);
	}
}

vxlnetwork_qt::advanced_actions::advanced_actions (vxlnetwork_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	...
	vxlnetwork_unit (new QRadioButton ("vxlc")), //
	...
	bootstrap_label (new QLabel ("IPV6:port \"::ffff:192.168.0.1:7155\"")), //
	...
	wallet (wallet_a)
{
	ratio_group->addButton (vxlnetwork_unit);
	ratio_group->setId (vxlnetwork_unit, ratio_group->buttons ().size () - 1);
	ratio_group->addButton (raw_unit);
	ratio_group->setId (raw_unit, ratio_group->buttons ().size () - 1);
	scale_layout->addWidget (scale_label);
	scale_layout->addWidget (vxlnetwork_unit);
	...
	QObject::connect (vxlnetwork_unit, &QRadioButton::toggled, [this] () {
		if (vxlnetwork_unit->isChecked ())
		{
			this->wallet.change_rendering_ratio (vxlnetwork::Mxrb_ratio);
			QSettings ().setValue (saved_ratio_key, ratio_group->id (vxlnetwork_unit));
		}
	});
	QObject::connect (raw_unit, &QRadioButton::toggled, [this] () {
		if (raw_unit->isChecked ())
		{
			this->wallet.change_rendering_ratio (vxlnetwork::raw_ratio);
	...
	{
		selected_ratio_button = vxlnetwork_unit;
	}
	...
	QObject::connect (peers_bootstrap, &QPushButton::released, [this] () {
		...
		auto error (vxlnetwork::parse_endpoint (bootstrap_line->text ().toStdString (), endpoint));
		if (!error)
		{
...
void vxlnetwork_qt::advanced_actions::refresh_peers ()
{
...
void vxlnetwork_qt::advanced_actions::refresh_ledger ()
{
	...
	for (auto i (wallet.node.ledger.store.account.begin (transaction)), j (wallet.node.ledger.store.account.end ()); i != j; ++i)
	{
		...
		std::string balance;
		vxlnetwork::amount (info.balance.number () / wallet.rendering_ratio).encode_dec (balance);
...
void vxlnetwork_qt::advanced_actions::refresh_stats ()
{
	wallet.stats_viewer.refresh_stats ();
}

vxlnetwork_qt::block_entry::block_entry (vxlnetwork_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	block (new QPlainTextEdit),
	status (new QLabel),
	...
	wallet (wallet_a)
{
	...
	QObject::connect (process, &QPushButton::released, [this] () {
		...
		try
		{
			...
			auto block_l (vxlnetwork::deserialize_block_json (tree));
			if (block_l != nullptr)
			{
...
vxlnetwork_qt::block_creation::block_creation (vxlnetwork_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
...
void vxlnetwork_qt::block_creation::deactivate_all ()
{
	account_label->hide ();
	account->hide ();
	source_label->hide ();
...
void vxlnetwork_qt::block_creation::activate_send ()
{
	account_label->show ();
	account->show ();
	amount_label->show ();
...
void vxlnetwork_qt::block_creation::activate_receive ()
{
	source_label->show ();
	source->show ();
}

void vxlnetwork_qt::block_creation::activate_open ()
{
	source_label->show ();
	source->show ();
...
void vxlnetwork_qt::block_creation::activate_change ()
{
	account_label->show ();
	account->show ();
	representative_label->show ();
...
void vxlnetwork_qt::block_creation::create_send ()
{
	vxlnetwork::account account_l;
	auto error (account_l.decode_account (account->text ().toStdString ()));
	if (!error)
	{
		vxlnetwork::amount amount_l;
			...
			if (!error)
			{
				...
				if (!wallet.wallet_m->store.fetch (transaction, account_l, key))
				{
					...
					if (amount_l.number () <= balance)
					{
						...
						(void)error;
						debug_assert (!error);
						vxlnetwork::state_block send (account_l, info.head, info.representative, balance - amount_l.number (), destination_l, key, account_l, 0);
...
void vxlnetwork_qt::block_creation::create_receive ()
{
	vxlnetwork::block_hash source_l;
	...
	if (!error)
	{
		...
		if (block_l != nullptr)
		{
			...
			{
				vxlnetwork::pending_key pending_key (destination, source_l);
				...
				if (!wallet.node.store.pending.get (block_transaction, pending_key, pending))
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							vxlnetwork::state_block receive (pending_key.account, info.head, info.representative, info.balance.number () + pending.amount.number (), source_l, key, pending_key.account, 0);
							vxlnetwork::block_details details;
...
void vxlnetwork_qt::block_creation::create_change ()
{
	vxlnetwork::account account_l;
	auto error (account_l.decode_account (account->text ().toStdString ()));
	if (!error)
	{
		...
		if (!error)
		{
			...
			if (!error)
			{
				...
				if (!error)
				{
					vxlnetwork::state_block change (account_l, info.head, representative_l, info.balance, 0, key, account_l, 0);
					vxlnetwork::block_details details;
					details.epoch = info.epoch ();
...
void vxlnetwork_qt::block_creation::create_open ()
{
	vxlnetwork::block_hash source_l;
	auto error (source_l.decode_hex (source->text ().toStdString ()));
	if (!error)
				...
				if (!destination.is_zero ())
				{
					...
					if (!wallet.node.store.pending.get (block_transaction, pending_key, pending))
					{
						...
						if (error)
						{
							...
							if (!error)
							{
								vxlnetwork::state_block open (pending_key.account, 0, representative_l, pending.amount, source_l, key, pending_key.account, 0);
								vxlnetwork::block_details details;
								details.is_receive = true;
