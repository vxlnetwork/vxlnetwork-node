...
public:
	...
	QPushButton * back;
	vxlnetwork_qt::wallet & wallet;
};
class advanced_actions
{
public:
	advanced_actions (vxlnetwork_qt::wallet &);
	QWidget * window;
	QVBoxLayout * layout;
...
{
public:
	block_entry (vxlnetwork_qt::wallet &);
	QWidget * window;
	QVBoxLayout * layout;
...
{
public:
	block_creation (vxlnetwork_qt::wallet &);
	void deactivate_all ();
	void activate_send ();
...
{
public:
	self_pane (vxlnetwork_qt::wallet &, vxlnetwork::account const &);
	void set_balance_text (std::pair<vxlnetwork::uint128_t, vxlnetwork::uint128_t>);
	QWidget * window;
	QVBoxLayout * layout;
...
{
public:
	history (vxlnetwork::ledger &, vxlnetwork::account const &, vxlnetwork_qt::wallet &);
	void refresh ();
	QWidget * window;
	QVBoxLayout * layout;
	QStandardItemModel * model;
...
{
public:
	block_viewer (vxlnetwork_qt::wallet &);
	void rebroadcast_action (vxlnetwork::block_hash const &);
	QWidget * window;
	QVBoxLayout * layout;
	QLabel * hash_label;
	QLineEdit * hash;
...
{
public:
	account_viewer (vxlnetwork_qt::wallet &);
	QWidget * window;
	QVBoxLayout * layout;
	QLabel * account_label;
...
{
public:
	stats_viewer (vxlnetwork_qt::wallet &);
	QWidget * window;
	QVBoxLayout * layout;
	QPushButton * refresh;
	QPushButton * clear;
...
{
public:
	status (vxlnetwork_qt::wallet &);
	void erase (vxlnetwork_qt::status_types);
	void insert (vxlnetwork_qt::status_types);
	void set_text ();
	std::string text ();
	std::string color ();
	std::set<vxlnetwork_qt::status_types> active;
	vxlnetwork_qt::wallet & wallet;
};
class wallet : public std::enable_shared_from_this<vxlnetwork_qt::wallet>
{
public:
	wallet (QApplication &, vxlnetwork_qt::eventloop_processor &, vxlnetwork::node &, std::shared_ptr<vxlnetwork::wallet> const &, vxlnetwork::account &);
	void start ();
	void refresh ();
	void update_connected ();
	void empty_password ();
	void change_rendering_ratio (vxlnetwork::uint128_t const &);
	std::string format_balance (vxlnetwork::uint128_t const &) const;
	vxlnetwork::uint128_t rendering_ratio;
	vxlnetwork::node & node;
	std::shared_ptr<vxlnetwork::wallet> wallet_m;
	vxlnetwork::account & account;
	vxlnetwork_qt::eventloop_processor & processor;
	vxlnetwork_qt::history history;
	vxlnetwork_qt::accounts accounts;
	vxlnetwork_qt::self_pane self;
	vxlnetwork_qt::settings settings;
	vxlnetwork_qt::advanced_actions advanced;
	vxlnetwork_qt::block_creation block_creation;
	vxlnetwork_qt::block_entry block_entry;
	vxlnetwork_qt::block_viewer block_viewer;
	vxlnetwork_qt::account_viewer account_viewer;
	vxlnetwork_qt::stats_viewer stats_viewer;
	vxlnetwork_qt::import import;
	...
	vxlnetwork_qt::status active_status;
	void pop_main_stack ();
