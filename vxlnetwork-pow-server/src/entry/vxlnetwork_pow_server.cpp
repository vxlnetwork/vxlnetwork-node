...
int main (int argc, char * argv[])
{
	try
	{
		...
		options.add_options () ("config_path", boost::program_options::value<std::string> ()->default_value ("./config-vxlnetwork-pow-server.toml"), "Path to the optional configuration file, including the file name");
		...
		{
			vxlnetwork_pow_server::config conf;
		...
		vxlnetwork_pow_server::config conf (config_path_parsed.string (), config_overrides);

		// Log configuration
		auto rotating_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt> ("vxlnetwork-pow-server.log", 1024 * 1024 * 5, 5);
		...
		vxlnetwork_pow_server::work_handler work_handler (conf, logger);
		web::config web_conf;
		...
		logger->info ("Vxlnetwork PoW Server version {}", version_string_full);
