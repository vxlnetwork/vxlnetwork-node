...
std::atomic<unsigned> vxlnetwork_pow_server::job::job_id_dispenser{ 1 };

vxlnetwork_pow_server::job::job ()
{
	job_id = job_id_dispenser.fetch_add (1);
}

/** Currently we support work on a single device, hence a single-thread pool */
vxlnetwork_pow_server::work_handler::work_handler (vxlnetwork_pow_server::config const & config_a, std::shared_ptr<spdlog::logger> const & logger_a)
    : config (config_a)
    , logger (logger_a)
    , pool (config.devices.size ())
{
	...
	{
		std::shared_ptr<vxlnetwork_pow::driver> driver;
		if (device.type == vxlnetwork_pow_server::config::device::device_type::cpu)
		{
			driver = std::make_shared<vxlnetwork_pow::cpp_driver> ();
		}
		else if (device.type == vxlnetwork_pow_server::config::device::device_type::gpu)
		{
			driver = std::make_shared<vxlnetwork_pow::opencl_driver> ();
		}
...
vxlnetwork_pow_server::work_handler::~work_handler ()
{
	pool.stop ();
}

void vxlnetwork_pow_server::work_handler::handle_queue_request (std::function<void(std::string)> response_handler)
{
...
void vxlnetwork_pow_server::work_handler::handle_queue_delete_request (std::function<void(std::string)> response_handler)
{
	boost::property_tree::ptree response;
...
void vxlnetwork_pow_server::work_handler::handle_request_async (std::string body, std::function<void(std::string)> response_handler)
{
	...
	try
	{
			...
			boost::asio::post (pool, [this, correlation_id, response_handler, create_error_response, attach_correlation_id] {
				...
				if (!jobs.empty ())
				{
					...
					try
					{
						...
						// TODO: set difficulty when handling request once the vxlnetwork-pow API is set
						//       as well as set_memory based on request?
						// driver->difficulty_set(vxlnetwork_pow::reverse ((1ULL << 40) - 1));
						// driver->memory_set (...);

						// TODO: vxlnetwork-pow API currently takes a 64 bit integer, same with the nonce
						...
						{
							// TODO: use values from vxlnetwork-pow / to_multiplier ()
		...
		else if (action && *action == "work_validate")
		{
			...
			// TODO: update to use vxlnetwork_pow::passes when the API is done
			bool passes (true);

			boost::property_tree::ptree response;
			response.put ("valid", passes ? "1" : "0");

			// TODO: add these when the vxlnetwork_pow API is done
